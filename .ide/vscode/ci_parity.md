...
| Testing         | pytest via bazel                    | CMK Tests Test Explorer (Bazel-driven) for `py_test` / `py_doc_test`; pytest via VSCode test explorer for ad-hoc runs | Match (Bazel) / Partial (pytest) | The CMK Tests group spawns `bazel test` for `py_test` and `py_doc_test`, hermetic. The standard VSCode pytest runner still works for the venv but is not hermetic and needs the venv built first.             |

## TypeScript / JavaScript (.ts, .js, .tsx, .jsx)

| Task           | CI (Bazel)                                       | Local (VSCode)                                        | Parity | Notes                                                                                                                                                                       |
| -------------- | ------------------------------------------------ | ----------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Formatting     | prettier (npm, via bazel format)                 | prettier extension (`esbenp.prettier-vscode`) on save | Match  | VSCode uses `bazel/tools/prettier.config.cjs` directly via `prettier.configPath`. Same config, same output.                                                                 |
...
| Testing        | vitest via bazel                                 | CMK Tests Test Explorer (Bazel-driven)                | Match  | Discovers `vitest_test` rules and shells out to `bazel test`, so the run uses the same hermetic vitest as CI. The standalone `vitest.explorer` extension is no longer used. |

## Vue (.vue)

| Task          | CI (Bazel)                    | Local (VSCode)                                     | Parity | Notes                                                                            |
| ------------- | ----------------------------- | -------------------------------------------------- | ------ | -------------------------------------------------------------------------------- |
| Formatting    | prettier (via bazel format)   | prettier extension on save                         | Match  | Same bazel config as TS/JS.                                                      |
...
| Testing      | `rust_test` via bazel                             | CMK Tests Test Explorer (Bazel-driven) | Match        | Discovers `rust_test` rules and shells out to `bazel test`, parsing libtest output for case-level results. The standalone `swellaby.vscode-rust-test-adapter` extension is no longer used.                                          |
...
| Task         | CI (Bazel)                     | Local (VSCode)                             | Parity         | Notes                                                                                                                                                                                                           |
| ------------ | ------------------------------ | ------------------------------------------ | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Formatting   | clang-format                   | clangd extension (`vscode-clangd`) on save | Version risk   | clangd is pinned to the workspace `bazel_env` binary (`clangd.path = bazel-bin/bazel/tools/bazel_env/bin/clangd`). Build `bazel_env` once via the C++ profile so the path resolves.                             |
| Linting      | clang-tidy (bazel lint aspect) | clangd in-editor diagnostics               | Match          | clangd is launched with `--clang-tidy` and reads `compile_commands.json` from the workspace root.                                                                                                               |
| IWYU         | `--config=iwyu` (bazel)        | Not configured                             | Not configured |                                                                                                                                                                                                                 |
| Intellisense | N/A                            | clangd                                     | Match          | clangd uses `compile_commands.json` regenerated via `CMK ▸ Cmd: Refresh C++ compile_commands.json` (`bazel build //bazel/tools:bazel_env && bazel run //:refresh_compile_commands`). No automatic regeneration. |
| Testing      | `cc_test` via bazel            | CMK Tests Test Explorer (Bazel-driven)     | Match          | Discovers `cc_test` rules and shells out to `bazel test`, hermetic toolchain.                                                                                                                                   |
...
| **Version drift**                     | VSCode extensions may bundle their own tool binaries.                                                                                                                    | Mitigated: ruff uses venv binary (`ruff.path`), buildifier uses explicit path, TypeScript pinned via `js/ts.tsdk.path`, prettier uses bazel config directly, mypy config auto-generated. Remaining risk: Rust toolchain (rustfmt/clippy). |
...
| **C++ compile_commands.json**         | clangd needs `compile_commands.json` to resolve includes / run clang-tidy; Bazel doesn't update it automatically when BUILD files change.                                | Run `CMK ▸ Cmd: Refresh C++ compile_commands.json` after BUILD changes. The C++ profile must be enabled and `bazel_env` built first.                                                                                                      |
| **Missing integrations**              | Shell, Groovy, TOML have no VSCode tooling configured.                                                                                                                   | Add extensions or accept CI-only coverage.                                                                                                                                                                                                |
