...
/**
 * The direct launcher runs the tool from the checkout's sources: unlike the
 * former `bazel run` target, it neither queues behind other Bazel commands
 * nor needs an edition pinned on an outer bazel invocation (the tool pins
 * the site edition on its own bazel calls and reports edition mismatches
 * itself).
 */
const DEPLOY_LAUNCHER = './scripts/cmk-dev-deploy.py'
...
export async function deployToSite(siteName: string): Promise<void> {
  ...
  const cmd = `${DEPLOY_LAUNCHER} ${flags.join(' ')}`
