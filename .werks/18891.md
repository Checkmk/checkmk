...
```shell
sudo omd stop␋sudo omd update <site name>
```
...
This command is even safe, if `omd` has not been updated, yet.

*Question*: Is it sufficient to install the new version of the Checkmk package?

*Answer*: No, only the aforementioned update procedure is safe. This is because the version used by `omd` can be manipulated by the site user. As long as the vulnerable Checkmk version is present on the host, the system is not any safer. The old packages have to be uninstalled. The patched `omd` executable can safely execute the following commands against a target site:

```shell
omd su <site name>␋omd stop <site name>␋omd start <site name>␋omd update <site name>␋omd rm <site name>
omd cleanup
omd version <site name>␋omd setversion
```

The following commands are not safe to execute as a root user until old Checkmk versions have been uninstalled:

```shell
omd mv <site name> <new site name>␋omd cp <site name> <new site name>␋omd disable <site name>␋omd enable <site name>␋omd update-apache-config <site name>␋omd restore <backup archive>␋omd backup <site name>␋omd init <site name>␋omd create <site name>
```

*Question*: What are the alternatives if commands like omd restore, omd backup, omd mv, or omd cp trigger the vulnerability?

*Answer*: There is no direct workaround for `omd` commands, which require root permissions, on unpatched hosts. The old `Checkmk` package has to be uninstalled to use these commands safely again. `omd restore` and `omd backup` can be used safely by executing these commands as a site user. If you need to find the original site name from your backup archive, you can extract it like this:

```shell
tar -tzf <backup archive> | head -1 | cut -d/ -f1
```

Once you have the exact name, create the site and restore the site from within the site context:
