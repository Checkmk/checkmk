...
Pipfile.lock: Pipfile .python-$(PYTHON_MAJOR_DOT_MINOR)-stamp
	@if [[ "$?" == "Pipfile" && "${CI}" == "true" ]]; then \
	...
	@( \
		if [ "$(CI)" == "true" ]; then \
			echo "$? would trigger locking of Pipfile.lock."; \
			echo "Skipping regenerating the Pipfile.lock from within a CI Job."; \
		else \
