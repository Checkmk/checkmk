...
class PostgresLinux(PostgresBase):
    ...
    def _filter_instances(self, procs_list, procs_to_match, proc_sensitive_filter):
        ...
        return [
            proc for proc in procs_list if self._matches_postgres_process(proc, procs_to_match) and
            (proc_sensitive_filter(proc) or self._matches_main(proc))
        ]
    ...
    def get_instances(self):
        ...
        procs = self._filter_instances(procs_list,
                                       procs_to_match,
                                       proc_sensitive_filter=lambda p: self.name in p)
        if not procs:
