...
def testconfig(request, testconfig_suffixes):
    ...
    if request.param != 'sync':
        ...
        testconfig_suffixes.set(Globals.plugintype, 'execution %s' % Globals.pluginname, 'async')
        testconfig_suffixes.set(Globals.plugintype, 'timeout %s' % Globals.pluginname, '10')
        if request.param == 'async+cached':
            testconfig_suffixes.set(Globals.plugintype, 'cache_age %s' % Globals.pluginname, '300')
        testconfig_suffixes.set(Globals.plugintype, 'retry_count %s' % Globals.pluginname, '3')
...
def expected_output():
    ...
    if Globals.pluginname == 'netstat_an.bat':
        ...
        plugin_variadic = repeat(
            ...
            r'|^$'
            r'%s' % (r'|%s' % re.escape(r'<<<>>>') if Globals.plugintype == 'plugins' else ''))
