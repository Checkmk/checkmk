...
void print_perf_counter(const wchar_t *counter_pattern)
{
    ...
    for (const std::wstring &object : query.enumerateObjects()) {
        ...
        if (globmatch(counter_pattern, object.c_str())
            ...
            std::tie(counter_names, instance_names) = query.enumerateObject(object.c_str());
            for (const std::wstring &counter_name : counter_names) {
                for (const std::wstring &instance_name : instance_names) {
                    std::wstring counter_path = query.makePath(object, instance_name, counter_name);
                    std::wstring path_en = query.makePath(object_en, instance_name, query.trans(counter_name));
                    counters.push_back(std::make_pair(path_en, query.addCounter(counter_path)));
                }
            }
        }
    }
