...
def actual_output_no_tcp(request, write_config):
    if platform.system() == 'Windows':
        ...
        try:
            ...
            p = subprocess.Popen(
                [src_agent_exe] + request.param, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def expected_output():
    ...
    expected = [
        ...
        r'WorkingDirectory: %s%s' % (drive_letter, re.escape(src_exec_dir)),
        r'ConfigFile: %s%s' % (drive_letter, re.escape(os.path.join(src_exec_dir, 'check_mk.ini'))),
        r'LocalConfigFile: %s%s' % (drive_letter,
                                    re.escape(os.path.join(src_exec_dir, 'check_mk_local.ini'))),
        r'AgentDirectory: %s%s' % (drive_letter, re.escape(src_exec_dir)),
        r'PluginsDirectory: %s%s' % (drive_letter, re.escape(os.path.join(src_exec_dir,
                                                                          'plugins'))),
        r'StateDirectory: %s%s' % (drive_letter, re.escape(os.path.join(src_exec_dir, 'state'))),
        r'ConfigDirectory: %s%s' % (drive_letter,
                                    re.escape(os.path.join(src_exec_dir, 'make_ini_config'))),
        r'TempDirectory: %s%s' % (drive_letter, re.escape(os.path.join(src_exec_dir, 'temp'))),
        r'LogDirectory: %s%s' % (drive_letter, re.escape(os.path.join(src_exec_dir, 'log'))),
        r'SpoolDirectory: %s%s' % (drive_letter, re.escape(os.path.join(src_exec_dir, 'spool'))),
        r'LocalDirectory: %s%s' % (drive_letter, re.escape(os.path.join(src_exec_dir, 'local'))),
