...
exec_dir = os.path.join(os.sep, 'Users', remoteuser, 'Tests')
sshopts = '-o StrictHostKeyChecking=no'
host = 'localhost'
port = 9999
agent_exe = os.path.join(exec_dir, 'check_mk_agent-64.exe')
ini_filename = os.path.join(exec_dir, 'check_mk.ini')
...
def localtest(expected_output, actual_output, testfile, testname=None, testclass=None):
    ...
    if platform.system() != 'Windows':
        cmd = [
            ...
            'py.test %s%s%s' % (os.path.join(exec_dir, testfile),
