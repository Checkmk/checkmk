...
constexpr bool g_kill_updater_on_end = false;
bool TheMiniBox::waitForUpdater(std::chrono::milliseconds timeout) {
    using namespace std::chrono_literals;
    if (stop_set_) return false;
    ON_OUT_OF_SCOPE(readWhatLeft());

    constexpr std::chrono::milliseconds time_grane = 250ms;
    auto waiting_processes = getProcessId();  // NOLINT
    auto* read_handle = getReadHandle();
    int safety_poll_count = 5;
    for (;;) {
        auto buf = cma::tools::ReadFromHandle<std::vector<char>>(read_handle);
        if (buf.empty()) {
            // we have nothing from the cmk-update
        ...
        if (timeout >= time_grane) {
            std::unique_lock lk(lock_);
            auto stop_time = std::chrono::steady_clock::now() + time_grane;
        ...
        if (timeout < time_grane) failed_ = true;

        if constexpr (g_kill_updater_on_end) {
