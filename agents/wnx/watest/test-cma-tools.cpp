...
namespace cma {
namespace ntfs {
extern bool G_SimulateBadRemove;
TEST(Ntfs, Remove) {
    ...
    auto fname = work / "check";

    // check global settings
    ASSERT_FALSE(G_SimulateBadRemove);
    ON_OUT_OF_SCOPE(G_SimulateBadRemove = false;);

    // normal delete
    tst::ConstructFile(fname, "aaaa");
    std::error_code ec;
    ASSERT_TRUE(fs::exists(fname, ec));
    ASSERT_TRUE(Remove(fname, ec));
    EXPECT_EQ(ec.value(), 0);
    EXPECT_TRUE(!fs::exists(fname, ec));
    EXPECT_FALSE(Remove(fname, ec));
    EXPECT_EQ(ec.value(), 0);

    // control shot in the remove
    EXPECT_FALSE(fs::remove(fname, ec));
    EXPECT_EQ(ec.value(), 0);

    // simulated delete
    G_SimulateBadRemove = true;
    tst::ConstructFile(fname, "aaaa");
    ASSERT_TRUE(fs::exists(fname, ec));
    ASSERT_TRUE(Remove(fname, ec));
    EXPECT_TRUE(!fs::exists(fname, ec));
