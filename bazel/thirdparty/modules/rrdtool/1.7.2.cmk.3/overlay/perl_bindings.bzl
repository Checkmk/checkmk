...
def _rrdtool_perl_bindings_impl(ctx):
    ...
    # The build host's perl is NOT a declared input -- it comes from PATH -- so nothing else in this
    # action's cache key varies with it.  Two things it decides are host-specific:
    #   * $Config{{archname}} picks the install dir: x86_64-linux-gnu-thread-multi on Debian/Ubuntu
    #     but x86_64-linux-thread-multi on el/sles;
    #   * RRDs.so is an XS module, ABI-bound to the perl it was compiled against (5.38 on
    #     ubuntu-24.04, 5.36 on debian-12, 5.26 on almalinux-8, ...).
    # Without the distro in the key, a remote-cache hit would hand one distro's build to another and
    # ship RRDs.pm in a directory that distro's perl never searches -- and pnp4nagios swallows the
    # resulting load failure silently.  Keep this line.
    command = """
# distro: {distro}
set -eu
root="$(pwd)"
...
rrdtool_perl_bindings = rule(
    ...
    attrs = {
        "distro": attr.string(
            mandatory = True,
            doc = "Target distro, folded into the action key: the host perl this builds against " +
                  "differs per distro in both archname and XS ABI, and Bazel cannot see it.",
        ),
        "librrd": attr.label(
            mandatory = True,
            allow_files = True,
