...
/// file: test-system-ai-features.groovy
...
void main() {
    ...
    def make_target = "test-system-ai-features";
    ...
    dir("${checkout_dir}") {
        ...
        /// Same execution path as test-system-singlesite-single: the site runs in
        /// this pod, no nested container.
        helper.execute_test([
            ...
            force_build: force_build,
            disable_cache: disable_cache,
            // both suites run in one session; tests/Makefile allows them 3600s,
            // so leave headroom for pytest to time out on its own first
            timeout: 90,
        ]);
    }
