...
class ServiceDiscoveryPreviewResult(ABCAutomationResult):
    ...
    def serialize(self, for_cmk_version: cmk_version.Version) -> SerializedResult:
        if for_cmk_version < cmk_version.Version(
            "2.1.0p27"
        ...
        if for_cmk_version < cmk_version.Version("2.2.0b2"):  # no source results, no labels by host
            return SerializedResult(repr(astuple(self)[:6]))

        if for_cmk_version < cmk_version.Version("2.2.0b6"):  # source_results, no labels by host
