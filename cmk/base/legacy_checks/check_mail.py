...
def check_mail_arguments(params):
    ...
    if "forward" in params:
        ...
        if method := forward.get("method"):
            forward_method = method
            if isinstance(method, tuple) and len(method) == 3:
                protocol, address, port = method
                # if statement instead pattern matching due to specific protocol string values which
                # mypy doesn't like
                if (
                    protocol in ("udp", "tcp")
                    ...
                    forward_method = f"{protocol},{address},{port}"

            args.append(f"--forward-method={forward_method}")
