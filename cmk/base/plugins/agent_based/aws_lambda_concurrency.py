...
def parse_aws_lambda_provisioned_concurrency(
        string_table: StringTable) -> SectionProvisionedConcurrencyAliases:
...
def discover_aws_lambda_concurrency(
        ...
        section_aws_lambda_region_limits: Optional[LambdaRegionLimitsSection],  #pylint: disable=unused-argument
) -> DiscoveryResult:
    if not section_aws_lambda_provisioned_concurrency:
        return
    for lambda_function, provisioned_concurrency_function_arns in section_aws_lambda_provisioned_concurrency.items(
    ):
...
def _check_concurrent_executions_in_percent(
        concurrent_executions_in_percent: float,
        levels_upper) -> Generator[Union[Result, Metric], None, None]:
...
def check_aws_lambda_concurrency(
    ...
    if (section_aws_lambda_provisioned_concurrency is None or section_aws_lambda is None or
            section_aws_lambda.get(item) is None):
    ...
    if (metrics.ConcurrentExecutions is None and
            metrics.ProvisionedConcurrencyInvocations is None and
            metrics.ProvisionedConcurrencySpilloverInvocations is None and
            metrics.ProvisionedConcurrencyUtilization is None and
            metrics.ProvisionedConcurrentExecutions is None and
            metrics.UnreservedConcurrentExecutions is None):
    ...
    if region_limits:
        ...
        if metrics.UnreservedConcurrentExecutions:
            yield from check_levels(
                metrics.UnreservedConcurrentExecutions * 100.0 /
...
_DEFAULT_PARAMETERS: Mapping[str, Tuple[float, float]] = {
    ...
    "levels_provisioned_concurrency_spillover_invocations":
        (_MORE_THAN_ONE_PER_HOUR, _MORE_THAN_ONE_PER_HOUR),
