...
def compute_detect_spec_if64adm(
    ...
    >>> compute_detect_spec_if64adm([True], [])
    [[('.1.3.6.1.2.1.1.2.0', '(?!x)x', True)]]
    >>> compute_detect_spec_if64adm([], [True])
    [[('.1.3.6.1.2.1.31.1.1.1.6.*', '.*', True)]]
    >>> compute_detect_spec_if64adm([True], [True])
    [[('.1.3.6.1.2.1.1.2.0', '(?!x)x', True)]]
    """
    if if64.is_disabled(if_disable_if64_hosts) or not if64.need_if64adm(use_if64adm):
        return never_detect
