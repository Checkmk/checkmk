...
def parse_ipmi_sensors(  # pylint: disable=too-many-branches
        string_table: StringTable,) -> ipmi_utils.Section:
    ...
    for line in string_table:
        ...
        if not (sensor := (section.setdefault(
                sensorname,
                ipmi_utils.Sensor(
                    status_txt=status_txt,
                    unit="",
                ),
        ) if status_txt_ok else section.get(sensorname))):
            continue
...
def _status_txt_mapping(status_txt: str) -> State:
    ...
    if (status_txt.lower() in [
            ...
            "present",
            "transition to ok",
    ] or status_txt.startswith("Fully Redundant") or status_txt.endswith("is connected") or
            status_txt.endswith("Presence detected") or status_txt.endswith("Device Present")):
        return State.OK
    return State.CRIT
