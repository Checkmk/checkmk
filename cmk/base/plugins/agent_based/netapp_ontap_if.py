...
PortsSection = Mapping[str, models.PortModel]  # same as Section in netapp_ontap_ports.py
...
def parse_netapp_interfaces(string_table: StringTable) -> InterfacesSection:
    return {
        ...
        if (interface_obj := models.IpInterfaceModel(**json.loads(line[0])))
    }
...
def parse_netapp_interfaces_counters(
    ...
    return {
        ...
        for line in string_table
        if (counter_obj := models.InterfaceCounters(**json.loads(line[0])))
    }
...
def _merge_interface_counters(
    ...
    if counters:
        return interface | counters.dict()
    return interface
