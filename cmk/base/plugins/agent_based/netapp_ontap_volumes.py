...
def parse_netapp_ontap_volumes(
    ...
    return {
        ...
        if (vol_obj := models.VolumeModel(**json.loads(vol[0])))
    }
...
def _generate_volume_metrics(
    ...
    for protocol in params.get("perfdata", []):
        ...
        yield from single_volume_metrics(counters_keys, volume_counter.dict(), value_store, now)
...
def _serialize_volumes(
    ...
    if not section_netapp_ontap_volumes_counters:
        return {key: volume.dict() for key, volume in section_netapp_ontap_volumes.items()}
    ...
    for key, volume in section_netapp_ontap_volumes.items():
        ...
        merged_section[key] = volume.dict() | (
            volume_counter.dict() if volume_counter is not None else {}
        )
...
def _deserialize_volume(
    ...
    if combined_volumes.get("id", None) is not None:
        return models.VolumeModel.validate(combined_volumes), models.VolumeCountersModel.validate(
            combined_volumes
        )

    return models.VolumeModel.validate(combined_volumes), None
...
def check_netapp_ontap_volumes(
    ...
    if "patterns" in params:
        yield from _check_volumes_pattern(
            item, params, section_netapp_ontap_volumes, section_netapp_ontap_volumes_counters
        )
        return
