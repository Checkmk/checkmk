    ...
    for line in string_table:
        ...
        if len(stripped_line) == 10:
            ...
            if _is_deprecated_oracle_asm_plugin_from_1_2_6(fg_min_repair_time, fg_disks):
                # We run in here in case the deprecated plug-in "mk_oracle_asm" from 1.2.6 is still
                ...
                # ignore data from newer agent plug-in.
...
def check_oracle_asm_diskgroup(  # pylint: disable=too-many-branches
    ...
    if section.found_deprecated_agent_output:
        yield Result(
            state=State.WARN,
            summary="The deprecated Oracle Agent plug-in 'mk_oracle_asm' from Checkmk Version 1.2.6 "
            "is still executed on this host. The section 'oracle_asm_diskgroup' is now "
            "generated by the plug-in 'mk_oracle'. Please remove the deprecated plug-in",
        )
    ...
    if data.fail_groups:
        ...
        if dgtype == "EXTERN":
            ...
            # old plug-in format has limitations when NORMAL or HIGH redundancy is found
