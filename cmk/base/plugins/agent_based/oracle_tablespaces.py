...
def check_oracle_tablespaces(
    ...
    try:
        file_online_states = oracle.check_unavailable_datafiles(tablespace["datafiles"], sid,
                                                                params)
    ...
    if stats is not None:
        ...
        if autoext is not None and ts_status != "READONLY":
            ...
            if autoext_info:
                yield Result(state=state(params.get("autoextend_severity", 2)),
        ...
        if ts_status != "READONLY" and (ts_type == "PERMANENT" or
                                        (ts_type == "TEMPORARY" and params.get("temptablespace"))):
