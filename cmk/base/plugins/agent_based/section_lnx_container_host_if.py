...
from cmk.base.plugins.agent_based.utils.interfaces import Interface, mac_address_from_hexstring
...
def _create_interface(raw_stats: Mapping[str, str]) -> Interface:
    ...
    return Interface(
        ...
        in_mcast=multicast,
        in_discards=float(raw_stats["rx_dropped"]),
        in_errors=float(raw_stats["rx_errors"]),
        out_octets=float(raw_stats["tx_bytes"]),
        out_ucast=float(raw_stats["tx_packets"]),
        out_discards=float(raw_stats["tx_dropped"]),
        out_errors=float(raw_stats["tx_errors"]),
    )
