...
def parse_ups_capacity(string_table: List[StringTable]) -> Battery:
    return Battery(
        seconds_left=optional_int(string_table[0][0][0], factor=60),
        percent_charged=optional_int(string_table[0][0][1]),
    )


def parse_ups_seconds_on_battery(string_table: List[StringTable]) -> Battery:
    return Battery(seconds_on_bat=optional_int(string_table[0][0][0]),)
