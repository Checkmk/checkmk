...
def validate_autocompleter_data(api_request: dict[str, object]) -> None:
    ...
    if params is None:
        raise MKUserError(
            "params", _('You need to set the "%(parameter)s" parameter.') % {"parameter": "params"}
        )

    value = api_request.get("value")
    if value is None:
        raise MKUserError(
            "params", _('You need to set the "%(parameter)s" parameter.') % {"parameter": "value"}
        )

    ident = api_request.get("ident")
    if ident is None:
        raise MKUserError(
            "ident", _('You need to set the "%(parameter)s" parameter.') % {"parameter": "ident"}
        )
