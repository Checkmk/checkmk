...
def convert_tree_to_frozen_diff_tree(row: Row) -> tuple[Row, bool]:
    ...
    found_aggr = bi_manager.compiler.get_aggregation_by_name(row["aggr_id"])
    if not found_aggr:
        raise MKGeneralException("Unable to find source aggregation for diff tree")
    _frozen_aggr, frozen_branch = found_aggr
    ...
    try:
        live_aggr = storage.AggregationStore(bi_fs.cache).get(frozen_info.based_on_aggregation_id)
    ...
    if combine_branches(live_branch, frozen_branch):
...
def combine_branches(live_branch: BICompiledRule, frozen_branch: BICompiledRule) -> bool:
    ...
    for pid in affected_parent_ids:
        if (ref := live_ids.get(pid)) or (ref := frozen_ids.get(pid)):
            if (marker := ref.frozen_marker) is None or marker.status != "changed":
