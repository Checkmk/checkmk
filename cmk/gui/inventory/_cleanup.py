...
class ConfigVariableInventoryCleanup(ConfigVariable):
...
def _compute_params(
    ...
    match params[0]:
        ...
        case "combined":
            ...
            assert isinstance(combined_params, dict)
...
def _compute_classified_file_paths(
    omd_root: Path, host_names: Sequence[HostName]
) -> _ClassifiedFilePaths:
    if not host_names:
        return _ClassifiedFilePaths(by_host={}, abandoned_files=[], abandoned_host_files=[])

    inventory_dir = omd_root / "var/check_mk/inventory"
    status_data_dir = omd_root / "tmp/check_mk/status_data"
    archive_dir = omd_root / "var/check_mk/inventory_archive"
    delta_cache_dir = omd_root / "var/check_mk/inventory_delta_cache"

    # Construct all files of known hosts
    file_paths_by_host = {
        h: _FilePathsOfHost(
            inventory_tree=inventory_dir / h,
            inventory_tree_gz=inventory_dir / f"{h}.gz",
            status_data_tree=status_data_dir / h,
            archive_file_paths=_collect_files_from_directory(archive_dir / h),
            delta_cache_file_paths=_collect_files_from_directory(delta_cache_dir / h),
        )
        for h in host_names
    }
    ...
    for file_path in set(archive_dir.glob("*/*")).difference(
    ...
    for file_path in set(delta_cache_dir.glob("*/*")).difference(
    ...
    for raw_host_name in abandoned_folders_and_files_by_host:
        ...
        for file_path in [
            inventory_dir / host_name,
            inventory_dir / f"{host_name}.gz",
            status_data_dir / host_name,
        ]:
    ...
    return _ClassifiedFilePaths(
        ...
        abandoned_files=[
            _File(path=file_path, timestamp=timestamp)
            for file_path in (
                set(inventory_dir.glob("[!.]*"))
                .union(status_data_dir.glob("*"))
                .difference(
                    fp
                    for fps in file_paths_by_host.values()
                    for fp in [
                        fps.inventory_tree,
                        fps.inventory_tree_gz,
                        fps.status_data_tree,
                    ]
                )
...
def _compute_classified_history_files(
    *,
    ...
    inventory_tree_ts = _compute_timestamp_from_file_path(inventory_tree)
...
class InventoryCleanup:
    ...
    def _run(self, config: Config, *, host_names: Sequence[HostName], now: int) -> None:
        ...
        classified_file_paths = _compute_classified_file_paths(self.omd_root, host_names)
    ...
    def __call__(self) -> None:
        self._run(
            ...
            host_names=[
                HostName(h)
                for h in _collect_all_raw_host_names(is_wato_slave_site=gui_is_wato_slave_site())
                if h
            ],
            now=int(time.time()),
        )
