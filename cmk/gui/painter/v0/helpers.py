...
def render_cache_info(what: str, row: Row) -> str:
    ...
    text = _("Cache generated %(cache_age)s ago, cache interval: %(cache_interval)s") % {
        "cache_age": cmk.utils.render.approx_age(cache_age),
        "cache_interval": cmk.utils.render.approx_age(cache_interval),
    }

    if cache_interval:
        percentage = 100.0 * cache_age / cache_interval
        text += _(", elapsed cache lifespan: %(percentage)s") % {
            "percentage": cmk.utils.render.percent(percentage)
        }
...
def format_plugin_output(output: str, *, request: Request, row: Row) -> HTML:
    return cmk.gui.view_utils.format_plugin_output(
        ...
        request=request,
        row=row,
        must_escape=cmk.gui.view_utils.determine_must_escape(active_config.sites, row),
