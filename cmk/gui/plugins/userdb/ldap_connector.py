...
class LDAPUserConnector(UserConnector):
    ...
    def check_credentials(self, user_id, password) -> CheckCredentialsResult:
        ...
        try:
            ...
            self._logger.warning('Unable to authenticate user %s. Reason: %s', user_id,
                                 e.args[0].get("desc", e))
            result = False
        except Exception:
    ...
    def do_sync(self, add_to_changelog, only_username, load_users_func, save_users_func):
        ...
        for user_id, ldap_user in ldap_users.items():
            ...
            if not mode_create and (not is_known_connection or
                                    (is_known_connection and
                                     user_connection_id not in all_active_connections)):
