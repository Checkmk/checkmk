...
def _migrate_group_patterns_discovery(group_pattern: object) -> Mapping[str, object]:
    """
    Migrate the group patterns from a tuple to a dictionary format for service discovery.

    >>> _migrate_group_patterns_discovery(("TESTGROUP", ("*gwia*", "*gwia*e")))
    {'group_name': 'TESTGROUP', 'pattern_configs': {'include_pattern': '*gwia*', 'exclude_pattern': '*gwia*e'}}
    >>> _migrate_group_patterns_discovery({"group_name": "group_name", "pattern_configs": {"include_pattern": "include_pattern", "exclude_pattern": "exclude_pattern"}})
    {'group_name': 'group_name', 'pattern_configs': {'include_pattern': 'include_pattern', 'exclude_pattern': 'exclude_pattern'}}
    """
    if isinstance(group_pattern, tuple):
        return {
            "group_name": group_pattern[0],
            "pattern_configs": {
                "include_pattern": group_pattern[1][0],
                "exclude_pattern": group_pattern[1][1],
            },
        }
    return cast(Mapping[str, object], group_pattern)
            ...
            "group_patterns": DictElement(
                ...
                parameter_form=List(
                    element_template=Dictionary(
                        migrate=_migrate_group_patterns_discovery,
