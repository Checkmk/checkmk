...
def register(permission_registry: PermissionRegistry) -> None:
    permission_registry.register(PERMISSION_DOWNLOAD_PRODUCT_USAGE)


PERMISSION_DOWNLOAD_PRODUCT_USAGE = Permission(
    section=PermissionSectionGeneral,
    name="download_product_usage_analytics",
    title=_l("Download product usage data"),
    description=_l("Allows users to download the product usage analytics data as a JSON file."),
    defaults=["admin"],
)
