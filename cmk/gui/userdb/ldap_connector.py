        ...
        *,
        ...
        def create_temp_user(uid: UserId) -> tuple[UserId, UserSpec] | None:
            ...
            if self._has_suffix() and user_id_with_suffix not in users:
                return (
                    user_id_with_suffix,
                    self._create_checkmk_user_for_this_ldap_connection(user_id_with_suffix, users),
                )

            return None

        def get_checkmk_user_for_this_ldap_user_id(
            uid: UserId,
        ...
        for ldap_user_id, ldap_user in ldap_users.items():
            ...
            if not mode_create:
                set_new, set_old = (
                    set(checkmk_user.keys()),
                    set(users[checkmk_user_id].keys()),
                )
