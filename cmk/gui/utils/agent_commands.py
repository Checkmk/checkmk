...
WINDOWS_AGENT_INSTALL_CMD = """curl.exe -fOG {fqdn}/{site}/check_mk/agents/windows/check_mk_agent.msi && ^
msiexec /i check_mk_agent.msi"""

LINUX_DEBIAN_AGENT_INSTALL_CMD = """curl -fOG {fqdn}/{site}/check_mk/agents/check-mk-agent_{version}-1_all.deb && \\
sudo dpkg -i check-mk-agent_{version}-1_all.deb"""

LINUX_RPM_AGENT_INSTALL_CMD = """curl -fOG {fqdn}/{site}/check_mk/agents/check-mk-agent_{version}-1_noarch.rpm && \\
...
def build_agent_install_cmds(
    ...
    return AgentInstallCmds(
        windows=WINDOWS_AGENT_INSTALL_CMD.format(fqdn=fqdn, site=site),
        linux_deb=LINUX_DEBIAN_AGENT_INSTALL_CMD.format(fqdn=fqdn, site=site, version=version),
        linux_rpm=LINUX_RPM_AGENT_INSTALL_CMD.format(fqdn=fqdn, site=site, version=version),
    )
...
def build_agent_registration_cmds(site: SiteId, fqdn: str) -> AgentRegistrationCmds:
    return AgentRegistrationCmds(
        windows=WINDOWS_AGENT_REGISTRATION_CMD.format(fqdn=fqdn, site=site),
        linux=LINUX_REGISTRATION_CMD.format(fqdn=fqdn, site=site),
        aix=AIX_REGISTRATION_CMD.format(fqdn=fqdn, site=site),
        solaris=SOLARIS_REGISTRATION_CMD.format(fqdn=fqdn, site=site),
    )
