...
from cmk.gui.plugins.views.utils import (  # noqa: F401 # pylint: disable=unused-import
    ...
    SorterSpec, transform_action_url, view_hooks, view_is_enabled, view_title, SorterListEntry,
)
...
class GUIViewRenderer(ABCViewRenderer):
    ...
    def _page_menu_entries_view_layout(self) -> Iterator[PageMenuEntry]:
        ...
        if display_options.enabled(display_options.E) and config.user.may("general.edit_views"):
            ...
            is_foreign_view: bool = view_owner != config.user.id
            is_own_view: bool = not is_builtin_view and not is_foreign_view

            if not is_builtin_view and (is_own_view or
                                        (is_foreign_view and
                                         config.user.may("general.edit_foreign_views"))):
                if is_own_view:
    ...
    for tag_group in config.tags.tag_groups:
        ...
        cls = type(
            ...
            (Painter,),
            {
                ...
                "group_by": lambda self, row, cell: _paint_host_tag(row, self._tag_group_id)[1],
            })
...
class PageRescheduleCheck(AjaxPage):
    ...
    @staticmethod
    def _force_check(now: int, cmd: str, spec: str, site: str) -> None:
        sites.live().command(
            "[%d] SCHEDULE_FORCED_%s_CHECK;%s;%d" % (now, cmd, livestatus.lqencode(spec), now),
            site)

    @staticmethod
    def _wait_for(site: str, host: str, what: str, wait_spec: str, now: int,
                  add_filter: str) -> livestatus.LivestatusRow:
        with sites.only_sites(site):
            return sites.live().query_row(
                ("GET %ss\n"
                 ...
                 "Filter: host_name = %s\n%s") %
                (what, livestatus.lqencode(wait_spec), now, config.reschedule_timeout * 1000,
                 livestatus.lqencode(host), add_filter))
    ...
    def _do_reschedule(self, request):
        ...
        if service in ("Check_MK Discovery", "Check_MK Inventory"):
            ...
            _row = self._wait_for(site, host, "service", f"{host};Check_MK", now,
