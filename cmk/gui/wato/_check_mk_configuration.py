...
def _migrate_use_new_descriptions_for_from_list_to_selection(
    value: object, available_plugin_names: Sequence[str]
) -> Mapping[str, bool]:
    match value:
        case list(list_items) if all(isinstance(item, str) for item in list_items):
            return {
                plugin_name: True if plugin_name in value else False
                for plugin_name in available_plugin_names
            }
        case dict(dict_items) if all(
            isinstance(k, str) and isinstance(v, bool) for k, v in dict_items.items()
        ):
            return {
                # enable newly added service names per default and drop vanished ones
                plugin_name: dict_items.get(plugin_name, True)
                for plugin_name in available_plugin_names
            }

        case _:
            raise TypeError("Invalid data format for 'use_new_descriptions_for': %s" % value)
    ...
    def valuespec(self) -> ValueSpec:
        available_selection = [
            ...
            (
                "casa_cpu_temp",
                Checkbox(
                    title=_("Casa module: CPU temperature"), label=_("Use the new service name")
                ),
            ),
            (
                "cisco_mem",
            ...
            (
                "cmciii_psm_current",
                Checkbox(
                    title=_("Rittal CMC-III Units: Current"), label=_("Use the new service name")
                ),
            ),
            (
                "cmciii_temp",
            ...
            (
                "enterasys_temp",
                Checkbox(
                    title=_("Enterasys Switch: Temperature"), label=_("Use the new service name")
                ),
            ),
            (
            ...
            (
                ...
                Checkbox(
                    title=_("Memory usage of ESX Clusters"), label=_("Use the new service name")
                ),
            ),
            (
                "etherbox_temp",
            ...
            (
                "hyperv_vms",
                Checkbox(
                    title=_("Hyper-V Server: State of VMs"), label=_("Use the new service name")
                ),
            ),
            (
                "ibm_svc_mdiskgrp",
            ...
            (
                "ibm_svc_system",
                Checkbox(
                    title=_("IBM SVC / V7000: System Info"), label=_("Use the new service name")
                ),
            ),
            (
            ...
            (
                "logwatch",
                Checkbox(
                    title=_("Check logfiles for relevant new messages"),
                    label=_("Use the new service name"),
                ),
            ),
            (
                "logwatch_groups",
                Checkbox(title=_("Check logfile groups"), label=_("Use the new service name")),
            ),
            (
                "megaraid_pdisks",
                Checkbox(
                    title=_("LSI MegaRAID: Physical Disks"), label=_("Use the new service name")
                ),
            ),
            (
                "megaraid_ldisks",
                Checkbox(
                    title=_("LSI MegaRAID: Logical Disks"), label=_("Use the new service name")
                ),
            ),
            (
                "megaraid_bbu",
            ...
            (
                "ps",
                Checkbox(
                    title=_("State and Count of Processes"), label=_("Use the new service name")
                ),
            ),
            (
                "qmail_stats",
            ...
            (
                "raritan_emx",
                Checkbox(
                    title=_("Raritan EMX Rack: Temperature"), label=_("Use the new service name")
                ),
            ),
            (
                "raritan_pdu_inlet",
            ...
            (
                "tplink_mem",
                Checkbox(
                    title=_("TP Link: Used memory via SNMP"), label=_("Use the new service name")
                ),
            ),
            (
                "ups_bat_temp",
        ...
        return Migrate(
            ...
            migrate=lambda x: _migrate_use_new_descriptions_for_from_list_to_selection(
                x, [plugin_name for plugin_name, title in available_selection] + ignored_keys
            ),
        )
