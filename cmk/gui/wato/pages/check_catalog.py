...
class ModeCheckPluginTopic(WatoMode):
    ...
    def breadcrumb(self) -> Breadcrumb:
        ...
        breadcrumb = _add_breadcrumb_topic_items(
            parent_cls(self._edition, self._ctx).breadcrumb(), self._titles, self._path
        )
    ...
    def _from_vars(self) -> None:
        ...
        if (check_info := checks.get(self._check_plugin_name)) is not None:
            ...
            raise MKUserError(
                None,
                _(
                    "Could not detect type of manual page: %(check_plugin_name)s. Maybe the check is missing "
                )
