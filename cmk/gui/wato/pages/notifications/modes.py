...
def get_disabled_notifications_infos() -> tuple[int, list[str]]:
    ...
    with sites.prepend_site():
        notification_status = sites.live().query(query_str)
...
class ABCNotificationsMode(ABCEventsMode[EventRule]):
    ...
    def _render_notification_rules(
        ...
        with table_element(title=title, limit=0) as table:
            ...
            for nr, rule in enumerate(rules):
                    ...
                    if idx < len(analyse_rules):  # type: ignore[possibly-undefined]
                        ...
                        if what == "match":
                            ...
                            html.static_icon(
                                ...
                                title=_("This rule does not match: %(reason)s")
                                % {"reason": reason},
                            )
                    else:
                        html.static_icon(
                ...
                if num_conditions:
                    title = _("%(num_conditions)d conditions") % {"num_conditions": num_conditions}
    ...
    def _table_title(self, show_title: bool, profilemode: bool, userid: str) -> str | HTML:
        ...
        if userid:
            ...
            return code + _("Notification rules of user %(userid)s") % {"userid": userid}
        ...
        if rule.get("contact_users"):
            ...
            if (
                ...
                info += html.render_static_icon(
                    StaticIcon(IconNames.warning),
                    title=_("User %(explicit_user)s does not exist.")
    ...
    for section, ruleset in entries.items():
        ...
        for rule_id in ruleset:
            ...
            rule_list.append(
                Rule(
                    i18n=rule.title()
                    or _("Unknown rule with ID %(rule_id)s") % {"rule_id": rule_id},
...
class ModeAnalyzeNotifications(ModeNotifications):
    ...
    def _render_bulks(self, *, only_ripe: bool, debug: bool, table_row_limit: int) -> bool:
        try:
            ...
            html.show_error(_("Could not retrieve bulk notifications: %(exc)s") % {"exc": exc})
            return False
        if not bulks:
            return False
...
class ModeTestNotifications(ModeNotifications):
    ...
    def _add_missing_host_context(self, context: NotificationContext, event_date: float) -> None:
        ...
        if len(resp) < 1:
            raise MKUserError(
                None,
                _("Host '%(hostname)s' is not known in the activated monitoring configuration")
                % {"hostname": hostname},
            )
        ...
        for script_name, title in notification_script_choices():
            ...
            vs: DropdownChoice[None] = DropdownChoice(
                ...
                empty_text=_(
                    ...
                    '<a href="wato.py?mode=notification_parameters&method=%(script_name)s">create</a> '
                    "at least one first."
                )
...
class ModeEditNotificationRule(ABCEditNotificationRuleMode):
    ...
    def _log_text(self, edit_nr: int) -> str:
        ...
        return _("Changed notification rule %(edit_nr)d") % {"edit_nr": edit_nr}


def _log_text(new: bool, user_id: UserId, edit_nr: int) -> str:
    if new:
        return _("Created new notification rule for user %(user_id)s") % {"user_id": user_id}
    return _("Changed notification rule %(edit_nr)d of user %(user_id)s") % {
        "edit_nr": edit_nr,
        "user_id": user_id,
    }
    ...
    def action(self, config: Config) -> ActionResult:
        ...
        if request.has_var("_delete"):
            ...
            self._add_change(
                ...
                text=_("Deleted notification parameter %(parameter_number)d")
            ...
            self._add_change(
                ...
                text=_("Changed position of notification parameter %(from_pos)d")
                % {"from_pos": from_pos},
                pending_changes=_pending_changes(config, omd_site(), user.id),
                site_configs=config.sites,
            )
        ...
        if nondescript_rule_count := sum(not bool(rule["description"]) for rule in related_rules):
            links_to_related_rules += html.render_li(
                HTML.with_escaping("").join(
                    (
                        _("%(nondescript_rule_count)d notification rule(s) ")
                        % {"nondescript_rule_count": nondescript_rule_count},
                        html.render_a(
    ...
    def _render_notification_parameters(
        ...
        with table_element(title=_("Parameters"), limit=0) as table:
            for nr, (parameter_id, parameter) in enumerate(parameters.items()):
                ...
                title = _("%(num_properties)d defined parameter properties") % {
...
class ModeEditNotificationParameter(ABCNotificationParameterMode):
    ...
    def _log_text(self, edit_nr: int) -> str:
        if self._new:
            return _("Created new notification parameter")
        return _("Changed notification parameter #%(edit_nr)s") % {"edit_nr": edit_nr}
