...
class ModeTags(ABCTagMode):
                ...
                if del_id in grouped_tag.aux_tag_ids:
                    raise MKUserError(
                        ...
                        _(
                            ...
                            "It is being used in the tag group <b>%(title)s</b>."
                        )
                        % {"title": group.title},
                    )
    ...
    def _show_customized_builtin_warning(self) -> None:
        ...
        html.show_warning(
            _(
                "You have customized the tag group(s) <tt>%(tag_groups)s</tt> in your tag configuration. "
    ...
    def _render_tag_group_list(
        ...
        *,
        ...
        with table_element(
            ...
            help=(
                _(
                    "Tags are the basis of the rule based configuration of Checkmk. "
    ...
    def _show_tag_icons(
        ...
        delete_url = make_confirm_delete_link(
            ...
            message=_("ID: %(id)s") % {"id": tag_group.id},
        )
    ...
    def _show_aux_tag_icons(self, aux_tag: cmk.ruleset_matcher.tags.AuxTag) -> None:
        ...
        delete_url = make_confirm_delete_link(
            ...
            message=_("ID: %(id)s") % {"id": aux_tag.id},
        )
...
class ModeEditTagGroup(ABCEditTagMode):
    ...
    def action(self, config: Config) -> ActionResult:
        ...
        if self._new:
            ...
            pending_changes.add(
                Change(
                    ...
                    text=_("Created new host tag group '%(id)s'") % {"id": changed_tag_group.id},
                    domains=[CORE],
                ),
                ChangeScope.all_activation_sites(),
            )
            flash(_("Created new host tag group '%(title)s'") % {"title": changed_tag_group.title})
            return redirect(mode_url("tags"))
        ...
        pending_changes.add(
            Change(
                action_name="edit-hosttags",
                text=_("Edited host tag group %(message)s (%(id)s)")
                % {"message": message, "id": self._id},
                domains=[CORE],
            ),
            ChangeScope.all_activation_sites(),
        )
...
def _rename_tags_after_confirmation(
    ...
    *,
    ...
    if repair_mode is not None:
        ...
        return _(
            "Modified folders: %(folders)d, modified hosts: %(hosts)d, modified rule sets: %(rulesets)d"
        ) % {
            "folders": len(affected_folders),
            "hosts": len(affected_hosts),
            "rulesets": len(affected_rulesets),
        }
...
def _show_affected_hosts(affected_hosts: list[Host]) -> None:
    ...
    for nr, host in enumerate(affected_hosts):
        if nr > 20:
            html.write_text_permissive(
                _("... (%(count)d more)") % {"count": len(affected_hosts) - 20}
            )
            break
