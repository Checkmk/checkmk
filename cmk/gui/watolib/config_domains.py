...
class ConfigDomainCore(ABCConfigDomain):
    ...
    @classmethod
    def generate_hosts_to_update_settings(cls, hostnames: Sequence[HostName]):
        return {cls.ident(): {"hosts_to_update": hostnames}}

    @classmethod
    def get_domain_request(cls, settings: List[SerializedSettings]) -> DomainRequest:
        hosts_to_update = set()
        for setting in settings:
            hosts_to_update.update(setting.get("hosts_to_update", {}))
        return DomainRequest(cls.ident(), {"hosts_to_update": list(hosts_to_update)})
