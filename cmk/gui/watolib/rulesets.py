...
def _paths_with_rules_file(folder_paths: Sequence[str]) -> list[str]:
    """Filter folder paths (with trailing slash) to those that have a rules.mk file"""
    root_dir = str(wato_root_dir())
    return [path for path in folder_paths if os.path.exists(f"{root_dir}/{path}rules.mk")]
...
class AllRulesets(RulesetCollection):
    ...
    def _load_rulesets_recursively(self, folder: Folder) -> None:
        ...
        if (
            subfolder_paths := folder.tree.cache.recursive_subfolders_for_path(
                f"{folder.path()}/".lstrip("/")
            )
        ) is not None:
            for folder_path in _paths_with_rules_file(subfolder_paths):
                self._load_folder_rulesets(folder.tree.folder(folder_path.strip("/")))
            return
...
class SingleRulesetRecursively(RulesetCollection):
    ...
    def _load_rulesets_recursively(self, folder: Folder, only_varname: RulesetName) -> None:
        ...
        if (
            ...
            for folder_path in _paths_with_rules_file(subfolder_paths):
                self._load_folder_rulesets(folder.tree.folder(folder_path.strip("/")), only_varname)
            return
...
class Ruleset:
    ...
    def matches_ruleset_search_options(self, search_options: SearchOptions) -> bool:
        return (
            ...
            and _match_search_expression(search_options, "ruleset_title", lambda: str(self.title()))
            and _match_search_expression(search_options, "ruleset_help", lambda: str(self.help()))
        )
...
class EnabledDisabledServicesEditor:
    ...
    def _save_service_enable_disable_rules(
        ...
        *,
        ...
        modified_folders += self._update_rule_of_host(
            ruleset, service_patterns, value=value, pending_changes=pending_changes
        )
        ...
        *,
        ...
        if other_rule and isinstance(other_rule.conditions.service_description, list):
            ...
            if not other_rule.conditions.service_description:
                ...
                self._log_edit_rule(
                    ruleset, original_rule, other_rule, pending_changes=pending_changes
                )
        ...
        if rule and isinstance(rule.conditions.service_description, list):
            ...
            for service_condition in service_patterns:
                if service_condition not in rule_service_conditions:
                    ...
                    modified = True
            if modified:
                self._log_edit_rule(ruleset, original_rule, rule, pending_changes=pending_changes)
    ...
    @staticmethod
    def _log_edit_rule(
        ruleset: Ruleset, original_rule: Rule, rule: Rule, *, pending_changes: PendingChanges
    ) -> None:
        ...
        pending_changes.add(
            Change(
                ...
                % {"index": index, "ruleset": ruleset.title(), "folder": rule.folder.alias_path()},
                object_ref=rule.object_ref(),
...
class RuleConfigFile(WatoConfigFile[Mapping[RulesetName, Any]]):
    ...
    def _save_content_to_rules_file(folder: Folder, content: list[str]) -> None:
        ...
        try:
            ...
            folder.tree.cache.folder_updated(folder.filesystem_path())
