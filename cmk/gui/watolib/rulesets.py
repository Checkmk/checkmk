...
class RuleConditions:
    ...
    def has_only_explicit_service_conditions(self):
        ...
        service_name_conditions = (self.service_description.get("$nor", []) if isinstance(
            self.service_description, dict) else self.service_description)

        return service_name_conditions and all(
...
class Rule:
    ...
    def is_discovery_rule(self):
        return (self.conditions.host_name and len(self.conditions.host_name) == 1 and
                isinstance(self.conditions.host_name, list) and
                isinstance(self.conditions.host_name[0], str) and
