...
def _make_form() -> Dictionary:
    return Dictionary(
        help_text=Help(
            "This ruleset allows you to configure levels for Azure Redis server load monitoring"
        ),
        elements={
            ...
            "levels": DictElement(
                required=False,
                parameter_form=SimpleLevels(
                    title=Title("Server load levels"),
                    level_direction=LevelDirection.UPPER,
...
rule_spec_azure_redis_server_load = CheckParameters(
    name="azure_redis_server_load",
    title=Title("Azure Redis server load"),
