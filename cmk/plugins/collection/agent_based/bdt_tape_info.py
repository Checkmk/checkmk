...
def inventory_bdt_tape_info(section: StringTable) -> DiscoveryResult:
    yield Service()


def check_bdt_tape_info(section: StringTable) -> CheckResult:
    for name, value in zip(["Name", "Description", "Vendor", "Agent Version"], section[0]):
...
snmp_section_bdt_tape_info = SimpleSNMPSection(
    name="bdt_tape_info",
...
check_plugin_bdt_tape_info = CheckPlugin(
    name="bdt_tape_info",
