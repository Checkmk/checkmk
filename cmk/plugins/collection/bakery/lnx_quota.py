...
def get_lnx_quota_files(conf: _Config) -> Iterable[Plugin]:
    if conf.deployment[0] == "do_not_deploy":
        return

    yield Plugin(base_os=OS.LINUX, source=Path("lnx_quota"), interval=conf.deployment[1])


bakery_plugin_lnx_quota = BakeryPlugin(
    name="lnx_quota",
