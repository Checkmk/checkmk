...
def get_db2_mem_files(conf: _Config) -> Iterable[Plugin]:
    if conf.deployment[0] == "do_not_deploy":
        return

    for base_os in (OS.LINUX, OS.SOLARIS, OS.AIX):
        yield Plugin(base_os=base_os, source=Path("db2_mem"), interval=conf.deployment[1])


bakery_plugin_db2_mem = BakeryPlugin(
    name="db2_mem",
