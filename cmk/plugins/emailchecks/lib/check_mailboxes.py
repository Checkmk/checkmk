...
def check_mailboxes(args: Args) -> CheckResult:
    # Currently only EWS uses fetch_tenant_id, don't use getpass() if set
    # because of "warning: password input may be echoed."
    args.fetch_password = (
        args.fetch_password if args.fetch_tenant_id or args.fetch_password else getpass()
    )
