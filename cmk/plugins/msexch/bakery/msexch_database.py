...
def get_msexch_database_files(conf: _Config) -> Iterable[Plugin]:
    if conf.deployment[0] == "do_not_deploy":
        return

    yield Plugin(
        base_os=OS.WINDOWS, source=Path("msexch_database.ps1"), interval=conf.deployment[1]
    )


bakery_plugin_msexch_database = BakeryPlugin(
