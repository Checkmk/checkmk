...
def get_nfsexports_files(conf: _Config) -> Iterable[Plugin]:
    if conf.deployment[0] == "do_not_deploy":
        return

    interval = conf.deployment[1]
    yield Plugin(base_os=OS.LINUX, source=Path("nfsexports"), interval=interval)
    yield Plugin(
        base_os=OS.SOLARIS,
...
bakery_plugin_nfsexports = BakeryPlugin(
    name="nfsexports",
