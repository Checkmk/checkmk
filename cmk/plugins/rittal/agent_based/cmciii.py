...
def parse_devices_and_states(device_table: StringTable) -> tuple[Devices, Sensors]:
...
def parse_cmciii(string_table: Sequence[StringTable]) -> Sensors:
...
snmp_section_cmciii = SNMPSection(
    name="cmciii",
...
def discover_cmciii(params: DiscoveryParams, section: Section) -> DiscoveryResult:
...
def check_cmciii(item: str, params: CheckParams, section: Section) -> CheckResult:
...
check_plugin_cmciii = CheckPlugin(
    name="cmciii",
