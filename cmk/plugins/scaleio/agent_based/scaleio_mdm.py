...
type Section = dict[str, dict[str, Any]]


def _add_key_values(data_dict: dict[str, Any], line: list[str]) -> None:
    """
...
def parse_scaleio_mdm(string_table: StringTable) -> Section:
    ...
    for line in string_table:
        ...
        # cluster. This is handled by the entry "Role"!
            ...
            _add_key_values(data[node], line)

    return parsed


def discover_scaleio_mdm(section: Section) -> DiscoveryResult:
    if section.get("Cluster"):
        yield Service()


def check_scaleio_mdm(section: Section) -> CheckResult:
    translate_status = {
        "Normal": 0,
        "Degraded": 1,
        "Error": 2,
    ...
    if data:
        ...
        yield Result(
            state=State(translate_status[status]),
            summary="Mode: {}, State: {}".format(data["Mode"], status),
        )

        active_state = (
            State.CRIT if (active[0] != active[1] or replicas[0] != replicas[1]) else State.OK
        )
        yield Result(
            state=active_state,
            summary="Active: {}, Replicas: {}".format("/".join(active), "/".join(replicas)),
        )

    for role in ["Master MDM", "Slave MDMs", "Tie-Breakers", "Standby MDMs"]:
        role_state = 0
        nodes = []
        for node in sorted(section.get(role, {})):
            nodes.append(node)
            node_status = section[role][node].get("Status", "Normal")
            if node_status != "Normal":
                role_state = max(role_state, translate_status[node_status])

        if nodes:
            infotext = "{}: {}".format(role, ", ".join(nodes))
        elif role != "Standby MDMs":
            role_state, infotext = 2, "%s not found in agent output" % role
        else:
            infotext = "%s: no" % role

        yield Result(state=State(role_state), summary=infotext)


agent_section_scaleio_mdm = AgentSection(
    name="scaleio_mdm",
    parse_function=parse_scaleio_mdm,
)


check_plugin_scaleio_mdm = CheckPlugin(
    name="scaleio_mdm",
