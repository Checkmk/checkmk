#!/usr/bin/env python3
# Copyright (C) 2019 Checkmk GmbH - License: GNU General Public License v2
# This file is part of Checkmk (https://checkmk.com). It is subject to the terms and
# conditions defined in the file COPYING, which is part of this source code package.


from cmk.agent_based.v2 import (
    CheckPlugin,
    CheckResult,
    DiscoveryResult,
    Metric,
    Result,
    Service,
    SimpleSNMPSection,
    SNMPTree,
    State,
    StringTable,
)
from cmk.plugins.sni_octopuse.lib import DETECT_SNI_OCTOPUSE


def discover_octopus_cpu(section: StringTable) -> DiscoveryResult:
    if len(section[0]) == 1:
        yield Service()


def check_octopus_cpu(section: StringTable) -> CheckResult:
    cpu_perc = int(section[0][0])
    yield Result(state=State.OK, summary=f"CPU utilization is {cpu_perc}%")
    yield Metric("util", float(cpu_perc))


def parse_sni_octopuse_cpu(string_table: StringTable) -> StringTable | None:
    return string_table or None


snmp_section_sni_octopuse_cpu = SimpleSNMPSection(
    name="sni_octopuse_cpu",
    detect=DETECT_SNI_OCTOPUSE,
    fetch=SNMPTree(
        base=".1.3.6.1.4.1.231.7.2.9.1",
        oids=["7"],
    ),
    parse_function=parse_sni_octopuse_cpu,
)


check_plugin_sni_octopuse_cpu = CheckPlugin(
    name="sni_octopuse_cpu",
    service_name="CPU utilization",
    discovery_function=discover_octopus_cpu,
    check_function=check_octopus_cpu,
)
