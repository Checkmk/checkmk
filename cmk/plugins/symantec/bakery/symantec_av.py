...
def get_symantec_av_files(conf: _Config) -> Iterable[Plugin]:
    if conf.deployment[0] == "do_not_deploy":
        return

    yield Plugin(base_os=OS.LINUX, source=Path("symantec_av"), interval=conf.deployment[1])


bakery_plugin_symantec_av = BakeryPlugin(
    name="symantec_av",
