...
def get_unitrends_files(conf: _Config) -> Iterable[Plugin]:
    if conf.deployment[0] == "do_not_deploy":
        return

    interval = conf.deployment[1]
    yield Plugin(base_os=OS.LINUX, source=Path("unitrends_backup"), interval=interval)
    yield Plugin(base_os=OS.LINUX, source=Path("unitrends_replication.py"), interval=interval)


bakery_plugin_unitrends = BakeryPlugin(
    name="unitrends",
