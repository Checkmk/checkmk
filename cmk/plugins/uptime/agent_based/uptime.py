...
def parse_solaris_uptime(info: StringTable, from_boot_time: float) -> uptime.Section:
    """
     Solaris agent Version>= 1.5.0p15 delivers a lot of context information

    This was necessary because Solaris returns very inconsistent output for
    ...
    assert uptime_match
    uptime_parsed = parse_human_read_uptime(uptime_match.group(1))
    snaptime = float(info[3][1])

    if abs(uptime_parsed - from_boot_time) < 600:
        uptime_sec = from_boot_time
    elif abs(uptime_parsed - snaptime) < 600:
        uptime_sec = snaptime
    elif abs(from_boot_time - snaptime) < 600:
        uptime_sec = from_boot_time
    else:
