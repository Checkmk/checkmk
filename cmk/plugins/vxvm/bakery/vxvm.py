...
def get_vxvm_files(conf: _Config) -> Iterable[Plugin]:
    if conf.deployment[0] == "do_not_deploy":
        return

    yield Plugin(base_os=OS.LINUX, source=Path("vxvm"), interval=conf.deployment[1])


bakery_plugin_vxvm = BakeryPlugin(
    name="vxvm",
