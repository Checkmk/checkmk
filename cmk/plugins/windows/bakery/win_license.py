...
def get_win_license_files(conf: _Config) -> Iterable[Plugin]:
    if conf.deployment[0] == "do_not_deploy":
        return

    yield Plugin(
        base_os=OS.WINDOWS,
        source=Path("win_license.bat"),
        interval=conf.deployment[1],
    )


bakery_plugin_win_license = BakeryPlugin(
    name="win_license",
    parameter_parser=_Config.model_validate,
    default_parameters=None,
    files_function=get_win_license_files,
)
