...
def get_windows_multipath_files(conf: _Config) -> Iterable[Plugin]:
    if conf.deployment[0] == "do_not_deploy":
        return

    yield Plugin(
        base_os=OS.WINDOWS,
        source=Path("windows_multipath.ps1"),
        interval=conf.deployment[1],
    )


bakery_plugin_windows_multipath = BakeryPlugin(
    name="windows_multipath",
    parameter_parser=_Config.model_validate,
    default_parameters=None,
    files_function=get_windows_multipath_files,
)
