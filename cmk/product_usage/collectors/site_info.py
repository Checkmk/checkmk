...
def collect(cmk_config_dir: Path, var_dir: Path) -> SiteInfo:
    telemetry_id_fp = site_id_file_path(var_dir)
    site_id = get_or_create_site_id(telemetry_id_fp)

    checkmk_info = get_checkmk_info()
...
def site_id_file_path(var_dir: Path) -> Path:
    return var_dir / "product_usage" / "site_id"


def get_site_id(file_path: Path) -> ProductUsageSiteId | None:
    try:
...
def store_site_id(file_path: Path, site_id: ProductUsageSiteId) -> None:
    file_path.parent.mkdir(parents=True, exist_ok=True)
    file_path.write_text(str(site_id))


def get_or_create_site_id(file_path: Path) -> ProductUsageSiteId:
    site_id = get_site_id(file_path)
    if site_id is None:
        site_id = ProductUsageSiteId(uuid4())
        store_site_id(file_path, site_id)
    return site_id
...
def get_checkmk_info() -> CheckmkInfo:
    general_version_infos = cmk_version.get_general_version_infos()
    return CheckmkInfo(
