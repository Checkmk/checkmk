...
# so that every variant (.info or .rrd) + the RRD backend temp suffix fits within MAX_FILENAME_LENGTH.
...
class Storage:
    ...
    def path(self, suffix: str) -> Path | None:
        # MAX_FILENAME_LENGTH limits a single filename component (not the full path).
