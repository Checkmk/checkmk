...
def _freeipmi_additional_args(args: Args,) -> Iterable[str]:
    yield from chain.from_iterable((freeipmi_opt, value) for argname, freeipmi_opt in [
        ("driver", "-D"),
    ...
    yield from (f"--{bool_arg.replace('_', '-')}" for bool_arg in [
        "quiet_cache",
        "sdr_cache_recreate",
        "interpret_oem_data",
        "output_sensor_state",
...
def _prepare_freeipmi_call(
    args: Args,) -> Tuple[Sequence[str], Mapping[str, Tuple[Iterable[str], Iterable[str]]]]:
    return (
        [
            "ipmi-sensors",
            "-h",
...
def _ipmitool_additional_args(args: Args,) -> Iterable[str]:
