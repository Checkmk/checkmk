...
def fetch_interfaces(connection: HostConnection) -> Iterable[netapp_ontap_models.IpInterfaceModel]:
    ...
    yield from (
        netapp_ontap_models.IpInterfaceModel.validate(element.to_dict())
...
def fetch_interfaces_counters(
...
) -> Iterable[netapp_ontap_models.InterfaceCountersRowModel]:
    """
    ...
    for interface in interfaces:
        ...
        yield from (
            netapp_ontap_models.InterfaceCountersRowModel.validate(element.to_dict())
            for element in NetAppResource.CounterRow.get_collection(
                "lif",
                id=interface_id,
                connection=connection,
