...
def write_section(section_header: str, generator: Iterable, logger: logging.Logger) -> None:
    ...
    with SectionWriter(section_header) as writer:
        for element in generator:
            logger.debug("Element data: %r", element.json(exclude_unset=True, exclude_none=False))
...
def fetch_volumes_counters(
    connection: HostConnection, volumes: Sequence[netapp_ontap_models.VolumeModel]
) -> Iterable[netapp_ontap_models.VolumeCountersModel]:
    """
    ...
    for volume in volumes:
        ...
        for element in NetAppResource.CounterRow.get_collection(
            ...
            **{"counters.name": "|".join(volumes_counters_field_query)},
            ...
            yield netapp_ontap_models.VolumeCountersModel(
...
def fetch_luns(connection: HostConnection) -> Iterable[netapp_ontap_models.LunModel]:
    ...
    for element in NetAppResource.Lun.get_collection(
        ...
        yield netapp_ontap_models.LunModel(
            name=element_data["name"],
...
def fetch_interfaces(connection: HostConnection) -> Iterable[netapp_ontap_models.IpInterfaceModel]:
    ...
    for element in NetAppResource.IpInterface.get_collection(
        ...
        yield netapp_ontap_models.IpInterfaceModel(
            uuid=element_data["uuid"],
            name=element_data["name"],
            enabled=element_data["enabled"],
            state=element_data.get("state"),
...
def fetch_interfaces_counters(
    connection: HostConnection, interfaces: Sequence[netapp_ontap_models.IpInterfaceModel]
) -> Iterable[netapp_ontap_models.InterfaceCounters]:
    """
    ...
    for interface in interfaces:
        ...
        for element in NetAppResource.CounterRow.get_collection(
            ...
            **{"counters.name": "|".join(interfaces_counters_field_query)},
            ...
            yield netapp_ontap_models.InterfaceCounters(
                id=element_data["id"],
...
def fetch_nodes(connection: HostConnection) -> Iterable[netapp_ontap_models.NodeModel]:
    ...
    for element in NetAppResource.Node.get_collection(
        ...
        yield netapp_ontap_models.NodeModel(
            name=element_data["name"],
            uuid=element_data["uuid"],
...
def fetch_alerts(
...
) -> Iterable[netapp_ontap_models.AlertModel]:
    ...
    yield from (netapp_ontap_models.AlertModel.validate(record) for record in records["records"])
    ...
    for key, values in query_data.items():
        for element in NetAppResource.CounterRow.get_collection(
            ...
            **{"counters.name": "|".join(values)},
            ...
            yield netapp_ontap_models.SvmTrafficCountersModel(
    ...
    for key, values in query_data.items():
        for element in NetAppResource.CounterRow.get_collection(
            ...
            **{"counters.name": "|".join(values)},
            ...
            yield netapp_ontap_models.FcInterfaceTrafficCountersModel(
                svm_name=svm_name,
                table=key,
...
def fetch_environment(connection):
    ...
    for element in NetAppResource.Sensors.get_collection(
        ...
        element_data = element.to_dict()
        yield netapp_ontap_models.EnvironmentThresholdSensorModel(
            name=element_data["name"],
            node_name=element_data["node"]["name"],
    ...
    for element in NetAppResource.Sensors.get_collection(
        ...
        element_data = element.to_dict()
        yield netapp_ontap_models.EnvironmentDiscreteSensorModel(
            name=element_data["name"],
            node_name=element_data["node"]["name"],
            sensor_type=element_data["type"],
...
def fetch_snapmirror(
    ...
    for element in NetAppResource.SnapmirrorRelationship.get_collection(
        ...
        yield netapp_ontap_models.SnapMirrorModel(
...
def fetch_fc_ports(connection: HostConnection) -> Iterable[netapp_ontap_models.FcPortModel]:
    ...
    for element in NetAppResource.FcPort.get_collection(
        ...
        element_data = element.to_dict()
        yield netapp_ontap_models.FcPortModel(
