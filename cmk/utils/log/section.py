...
def section_step(text: str, add_info: str = "", verbose: bool = True) -> None:
    (console.verbose if verbose else console.info)(
        f"{tty.yellow}+{tty.normal} {text.upper()}{f' ({add_info})' if add_info else add_info}"
    )
...
def section_error(text: str, verbose: bool = True) -> None:
    (console.verbose if verbose else console.info)(f"{tty.error} - {text}")
