...
class Version:
    ...
    def _parse_release_version(cls, vstring: str) -> Self:
        ...
        match match.group(1, 2, 3, 4, 5, 6, 7):
            ...
            case major, minor, sub, r_type, patch, None, None:
                return cls(
                    _BaseVersion(int(major), int(minor), int(sub)),
            ...
            case major, minor, sub, r_type, patch, rc, None:
                return cls(
                    _BaseVersion(int(major), int(minor), int(sub)),
                    _Release(RType[r_type], int(patch)),
                    _ReleaseCandidate(int(rc)),
            ...
            case major, minor, sub, r_type, patch, None, meta:
                return cls(
                    _BaseVersion(int(major), int(minor), int(sub)),
                    _Release(RType[r_type], int(patch)),
                    _ReleaseCandidate(None),
            ...
            case major, minor, sub, r_type, patch, rc, meta:
                return cls(
                    _BaseVersion(int(major), int(minor), int(sub)),
                    _Release(RType[r_type], int(patch)),
                    _ReleaseCandidate(int(rc)),
