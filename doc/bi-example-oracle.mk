...
aggregation_rules["oracle_tbs"] = ("Tablespaces", ["HOST", "DB"], "worst", [
    ("$HOST$", "Tablespace $DB$_"),
])

# Filesystems and IO relevant for DB
aggregation_rules["db_filesystems"] = ("Filesystems", ["HOST", "DB"], "worst", [
    ("$HOST$", "fs_/OraBase"),
    ("$HOST$", "Mount|Disk"),
])
# State of host that is relevant for a certain ORACLE DB
aggregation_rules["db_host_state"] = ("$HOST$", ["HOST"], "worst", [
    ("$HOST$", HOST_STATE),
    ("performance", ["$HOST$"]),
...
aggregation_rules["cluster_db_hosts"] = ("Hoststates", ["HOSTA", "HOSTB"], "worst", [
    ("db_host_state", ["$HOSTA$:$HOSTB$"]),
    ("db_host_state", ["$HOSTA$"]),
    ("db_host_state", ["$HOSTB$"]),
])

aggregation_rules["noncluster_db"] = ("$DB$", ["HOST", "DB"], "worst", [
    ("$HOST$", "DB_$DB$"),
    ("oracle_log", ["$HOST$", "$DB$"]),
    ("oracle_tbs", ["$HOST$", "$DB$"]),
    ("db_host_state", ["$HOST$"]),
])
aggregation_rules["cluster_db"] = ("$DB$", ["HOSTA", "HOSTB", "DB"], "worst", [
    ("$HOSTA$:$HOSTB$", "DB_$DB$"),
