...
void BoolColumn::output(Row row, RowRenderer &r, const contact * /*auth_user*/,
                        std::chrono::seconds /*timezone_offset*/) const {
    r.output(getValue(row));
}

std::unique_ptr<Filter> BoolColumn::createFilter(
...
std::unique_ptr<Aggregator> BoolColumn::createAggregator(
