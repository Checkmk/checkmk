...
void TableContacts::addColumns(Table *table, const std::string &prefix,
    ...
    table->addColumn(std::make_unique<StringLambdaColumn>(
        ...
        Column::Offsets{}, GetAttr(&contact::name, ""s)));
    table->addColumn(std::make_unique<StringLambdaColumn>(
        prefix + "alias", "The full name of the contact", Column::Offsets{},
        GetAttr(&contact::alias, ""s)));
    table->addColumn(std::make_unique<StringLambdaColumn>(
        prefix + "email", "The email address of the contact", Column::Offsets{},
        GetAttr(&contact::email, ""s)));
    table->addColumn(std::make_unique<StringLambdaColumn>(
        prefix + "pager", "The pager address of the contact", Column::Offsets{},
    ...
    table->addColumn(std::make_unique<StringLambdaColumn>(
        ...
        Column::Offsets{}, GetAttr(&contact::host_notification_period, ""s)));
    table->addColumn(std::make_unique<StringLambdaColumn>(
    ...
    for (int i = 0; i < MAX_CONTACT_ADDRESSES; ++i) {
        ...
        table->addColumn(std::make_unique<StringLambdaColumn>(
            prefix + b, "The additional field " + b, Column::Offsets{},
    ...
    table->addColumn(std::make_unique<IntLambdaColumn>(
        ...
        Column::Offsets{}, GetAttr(&contact::can_submit_commands, 0)));
    table->addColumn(std::make_unique<IntLambdaColumn>(
        prefix + "host_notifications_enabled",
        "Wether the contact will be notified about host problems in general (0/1)",
        Column::Offsets{}, GetAttr(&contact::host_notifications_enabled, 0)));
    table->addColumn(std::make_unique<IntLambdaColumn>(
    ...
    table->addColumn(std::make_unique<ListLambdaColumn>(
        ...
        "A list of all custom variables of the contact", Column::Offsets{},
    ...
    table->addColumn(std::make_unique<ListLambdaColumn>(
        prefix + "tag_values",
        "A list of the values of all tags of the contact", Column::Offsets{},
        GetCustomAttributeElem<1>{table->core(), AttributeKind::tags}));
    table->addColumn(std::make_unique<AttributesLambdaColumn>(
        prefix + "tags", "A dictionary of the tags", Column::Offsets{},
    ...
    table->addColumn(std::make_unique<ListLambdaColumn>(
        prefix + "label_values",
        "A list of the values of all labels of the contact", Column::Offsets{},
        GetCustomAttributeElem<1>{table->core(), AttributeKind::labels}));
    table->addColumn(std::make_unique<AttributesLambdaColumn>(
        prefix + "labels", "A dictionary of the labels", Column::Offsets{},
    ...
    table->addColumn(std::make_unique<ListLambdaColumn>(
        ...
        "A list of the values of all sources of the contact", Column::Offsets{},
        GetCustomAttributeElem<1>{table->core(),
                                  AttributeKind::label_sources}));
    table->addColumn(std::make_unique<AttributesLambdaColumn>(
    ...
    table->addColumn(std::make_unique<AttributeBitmaskLambdaColumn>(
        ...
        Column::Offsets{}, GetAttr{&contact::modified_attributes, 0}));
    table->addColumn(std::make_unique<AttributeListColumn2>(
        prefix + "modified_attributes_list",
        "A list of all modified attributes", Column::Offsets{},
