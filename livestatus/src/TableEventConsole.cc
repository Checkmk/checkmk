...
// static
std::unique_ptr<ListColumnCallback<ECRow, std::string>> ECRow::makeListColumn(
    const std::string &name, const std::string &description,
    const ColumnOffsets &offsets) {
    return std::make_unique<ListColumnCallback<ECRow, std::string>>(
        name, description, offsets, [name](const ECRow &r) {
...
bool TableEventConsole::isAuthorizedForEventViaContactGroups(
    ...
    auto col = std::static_pointer_cast<ListColumnCallback<ECRow, std::string>>(
