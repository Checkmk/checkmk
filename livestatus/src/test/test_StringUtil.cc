...
TEST(StringUtilTest, EscapeNonprintableTest) {
    EXPECT_EQ(R"(\x01\xFFtoto 42~\x7F\x80)",
              mk::escape_nonprintable("\1\xfftoto 42\x7e\x7f\x80"));
    EXPECT_EQ(R"(\x01\x02\x03)", mk::escape_nonprintable("\1\2\3"));
    // No UTF-8 support.
    EXPECT_EQ(R"(\xE5\x87\x84\xE3\x81\x8F\xE3\x81\xAA\xE3\x81\x84)",
              mk::escape_nonprintable("凄くない"));
    EXPECT_EQ(R"(Bl\xC3\xB6dsinn)", mk::escape_nonprintable("Blödsinn"));
}
