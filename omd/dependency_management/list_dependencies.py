    ...
    def components(self) -> list[Component]:
        return [
            Component(
                ...
                hashes=frozenset(Hash(alg="SHA-256", content=h) for h in info.hashes),
...
def read_cargo_lock(path: Path) -> list[Component]:
    ...
    with path.open() as cargofile_fh:
        cargofile = toml.load(cargofile_fh)
    ...
    return [
        Component(
            ...
            hashes=frozenset({Hash(alg="SHA-256", content=details["checksum"])}),
            files=frozenset({path}),
        )
...
def read_manifests(paths: list[Path]) -> list[Component]:
    ...
    for path in paths:
        match path:
            case (
                Path(name="runtime-requirements.txt")
                | Path(name="requirements.txt")
                | Path(name="requirements-windows.txt")
            ):
