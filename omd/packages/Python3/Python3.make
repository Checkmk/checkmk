...
# Package definition
PYTHON3 := Python3
PYTHON3_DIR := Python-$(PYTHON_VERSION)
...
PACKAGE_PYTHON3_PYTHONPATH      := $(PACKAGE_PYTHON3_DESTDIR)/lib/python$(PYTHON_MAJOR_DOT_MINOR)
...
PACKAGE_PYTHON3_INCLUDE_PATH    := $(PACKAGE_PYTHON3_DESTDIR)/include/python$(PYTHON_MAJOR_DOT_MINOR)
...
PYTHON3_SITECUSTOMIZE_COMPILED := $(PYTHON3_WORK_DIR)/__pycache__/sitecustomize.cpython-$(PYTHON_MAJOR_MINOR).pyc
...
# Ensure that the rpath of the python binary and dynamic libs always points to the current version path
	for i in $(PACKAGE_PYTHON3_EXECUTABLE) \
	         $(PACKAGE_PYTHON3_LD_LIBRARY_PATH)/libpython$(PYTHON_MAJOR_DOT_MINOR).so.1.0 \
...
# Native modules built based on this version need to use the correct rpath
	...
	LD_LIBRARY_PATH="$(PACKAGE_PYTHON3_LD_LIBRARY_PATH)" \
	    ...
	    $(PACKAGE_PYTHON3_PYTHONPATH)/_sysconfigdata__linux_x86_64-linux-gnu.py
	if [ -d "$(PACKAGE_PYTHON3_PYTHONPATH)/test" ] ; then \
...
# Fix python interpreter
	$(SED) -i '1s|^#!.*/python3\.'"$PYTHON_VERSION_MINOR"'$$|#!/usr/bin/env python3|' $(addprefix $(PYTHON3_INSTALL_DIR)/bin/,2to3-$(PYTHON_MAJOR_DOT_MINOR) easy_install-$(PYTHON_MAJOR_DOT_MINOR) idle$(PYTHON_MAJOR_DOT_MINOR) pip3 pip$(PYTHON_MAJOR_DOT_MINOR) pydoc$(PYTHON_MAJOR_DOT_MINOR))
