...
# We're using here only the relative folder for snmp commands: the full path will be dynamically calculated
# in the check_snmp binary. That way we can use cached builds in multiple checkmk versions.
