...
from typing import assert_never, cast, IO, Literal, NoReturn
...
from omdlib.finalize import CommandType, finalize_site, finalize_site_as_user
...
def start_site(version_info: VersionInfo, site: SiteContext) -> None:
    ...
    if not get_instance_id_file_path(Path(site_home)).exists():
...
def main_update(  # pylint: disable=too-many-branches
    ...
    call_scripts(site.name, "post-update", open_pty=is_tty)
...
def _omd_to_check_mk_version(omd_version: str) -> Version:
    ...
    >>> f("2.0.0p3.cee")
    Version(_BaseVersion(major=2, minor=0, sub=0), _Release(r_type=RType.p, value=3), _ReleaseCandidate(value=None), _ReleaseMeta(value=None))
    >>> f("1.6.0p3.cee.demo")
    Version(_BaseVersion(major=1, minor=6, sub=0), _Release(r_type=RType.p, value=3), _ReleaseCandidate(value=None), _ReleaseMeta(value=None))
    >>> f("2.0.0p3.cee")
    Version(_BaseVersion(major=2, minor=0, sub=0), _Release(r_type=RType.p, value=3), _ReleaseCandidate(value=None), _ReleaseMeta(value=None))
    >>> f("2021.12.13.cee")
    Version(None, _Release(r_type=RType.daily, value=_BuildDate(year=2021, month=12, day=13)), _ReleaseCandidate(value=None), _ReleaseMeta(value=None))
    """
    parts = omd_version.split(".")
...
def _restore_backup_from_tar_root(
    ...
    *,
    ...
    with tarfile.open(fileobj=buffer_with_copy, mode="r|*") as tar:
