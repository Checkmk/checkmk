...
def sitename_must_be_valid(site: SiteContext, reuse: bool = False) -> None:
    ...
    if not reuse and site.exists():
        sys.exit("Site '%s' already existing." % site.name)
    if not reuse and group_exists(site.name):
        sys.exit("Group '%s' already existing." % site.name)
    if not reuse and user_exists(site.name):
        sys.exit("User '%s' already existing." % site.name)
