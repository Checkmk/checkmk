...
PACKAGE_PERL_MODULES_PERL5LIB := $(PERL_MODULES_BUILD_DIR)/dest/lib/perl5
...
$(PERL_MODULES_BUILD): $(PACKAGE_DIR)/$(PERL_MODULES)/src/Crypt-SSLeay-0.72-patched.tar.gz
	...
	set -e; unset LANG; \
	    ...
	    unset PERL_MM_OPT; \
	    export PATH=$(PERL_MODULES_BUILD_DIR)/dest/bin:$$PATH; \
	    export PERL_MM_OPT=INSTALL_BASE=$(PERL_MODULES_BUILD_DIR)/dest; \
	    export PERL_MB_OPT=--install_base=$(PERL_MODULES_BUILD_DIR)/dest; \
	    export MODULEBUILDRC=$(PERL_MODULES_BUILD_DIR)/dest/.modulebuildrc; \
	    export PERL5LIB=$(PACKAGE_PERL_MODULES_PERL5LIB):$(PERL_MODULES_BUILD_DIR)/src/lib:$(PERL_MODULES_BUILD_DIR)/src/Crypt-SSLeay-0.72; \
	    cd $(PERL_MODULES_BUILD_DIR)/src ; \
		FORCE=1 ./build_module.pl -d "$(DISTRO_INFO)" -p $(PERL_MODULES_BUILD_DIR)/dest $(PERL_MODULES_LIST1); \
	    export PERL_JSON_BACKEND='JSON::XS'; \
	    cd $(PERL_MODULES_BUILD_DIR)/src ; \
	    ./build_module.pl -d "$(DISTRO_INFO)" -p $(PERL_MODULES_BUILD_DIR)/dest $(PERL_MODULES_LIST2)
...
$(PERL_MODULES_INSTALL): $(PERL_MODULES_BUILD)
	...
	$(RSYNC) $(PERL_MODULES_BUILD_DIR)/dest/lib $(DESTDIR)$(OMD_ROOT)/lib/perl5/
	$(RSYNC) $(PERL_MODULES_BUILD_DIR)/dest/bin $(DESTDIR)$(OMD_ROOT)/lib/perl5/
...
perl-modules-clean:
	$(RM) -r src/inc $(PERL_MODULES_BUILD_DIR) $(BUILD_HELPER_DIR) $(PACKAGE_DIR)/$(PERL_MODULES)/src/*-patched.tar.gz
