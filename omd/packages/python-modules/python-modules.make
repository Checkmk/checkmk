...
# Fix python interpreter for kept scripts
	$(SED) -i '1s|^#!.*/python$$|#!/usr/bin/env python2|' $(addprefix $(DESTDIR)$(OMD_ROOT)/bin/,chardetect easy_install easy_install-2.7 fakebmc futurize jirashell pasteurize pbr pip pip2 pip2.7 pyghmicons pyghmiutil pyjwt pyrsa-decrypt pyrsa-encrypt pyrsa-keygen pyrsa-priv2pub pyrsa-sign pyrsa-verify virshbmc)
	$(TOUCH) $@
