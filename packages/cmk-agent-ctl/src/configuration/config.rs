...
#[derive(StringEnum, PartialEq, Eq, Deserialize, Serialize)]
...
pub mod test_helpers {
    ...
    impl TestRegistryDir {
        pub fn new() -> Self {
            let dir = Some(tempfile::tempdir().unwrap());
            Self { dir }
        }
    ...
    // Clippy is happy
    impl Default for TestRegistryDir {
        fn default() -> Self {
            Self::new()
        }
    }

    impl Drop for TestRegistryDir {
        fn drop(&mut self) {
