...
class AgentReceiverRunner:
    ...
    def _log_file(self) -> Path:
        return self._config.log_path.parent / "runner.log"

    def _default_ssl_context(self) -> ssl.SSLContext:
        return ssl.create_default_context(cafile=str(self._config.agent_cert_store_path))
    ...
    def start(self) -> None:
        ...
        self._process = subprocess.Popen(
            ...
            env=os.environ
            | {"OMD_ROOT": str(self._config.omd_root), "OMD_SITE": self._config.site_name},
        )
    ...
    def _print_logs(self) -> None:
        print("\n==== agent-receiver runner logs ====")  # nosemgrep: disallow-print
        if self._log_file.exists():
            content = self._log_file.read_text()
            print(content if content.strip() else "(empty)")  # nosemgrep: disallow-print
        else:
            print(f"No log file at {self._log_file}")  # nosemgrep: disallow-print
            ...
            try:
                for attempt in Retrying(
                    ...
                    with attempt:
                        try:
                            resp = client.get(
                                f"{self.base_url}/{self._config.site_name}"
