...
def test_update_task(
    ...
    # add some tasks
    task_ids = add_tasks(5, agent_receiver, relay_id)

    # pick the testing task
    task_id = task_ids[2]

    # the task is not updated yet, should be pending
    ...
    # the task is no longer in the list of pending tasks

    all_pending_tasks = get_pending_tasks(agent_receiver, relay_id)
    assert_task_not_in_the_list(task_id, all_pending_tasks)

    # TODO assert the other tasks have not been touched
    ...
    for tid in other_task_ids:
        ...
        t2 = find_task_with_id(tid, stored_tasks)
        assert t1 == t2
