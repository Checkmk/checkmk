...
"""Environment/version snapshot types and collection shared by the version subsystem and
crash reporting.

``VersionInfo`` is produced by the ``general_version_infos*`` functions below and consumed
by the crash-reporting library to build a ``CrashInfo``. It lives here, in the foundational
``cmk.ccc`` layer, so that higher layers (``cmk.crash``) depend downward on it rather than
the reverse.
"""
