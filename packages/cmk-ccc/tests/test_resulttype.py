...
class TestOk:
    ...
    def test_map_error(self, result: Result[int, object]) -> None:
        error = "error"
        assert result.map_error(lambda _: error) == result

    def test_fold(self, result: Result[int, object]) -> None:
        assert result.fold(ok=lambda _: "ok", error=lambda _: "error") == "ok"
...
class TestError:
    ...
    def test_map(self, result: Result[object, int]) -> None:
        ok = "ok"
        assert result.map(lambda _: ok) == result
    ...
    def test_fold(self, result: Result[object, int]) -> None:
        assert result.fold(ok=lambda _: "ok", error=lambda _: "error") == "error"
