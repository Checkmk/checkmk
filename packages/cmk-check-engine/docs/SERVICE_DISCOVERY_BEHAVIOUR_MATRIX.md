...
otherwise              → sync_changes_before_remote_automation(site)   # ⚠️ syncs; does not activate
...
1. **The remote path pushes the central configuration to the remote first; the local path does
   not.** `sync_changes_before_remote_automation` (`services.py:get_check_table`) runs before the
   automation, and it is a _sync_, not an activation. It drives the full activation machinery with
   `prevent_activate=True`, so `_sync_and_activate` transfers the config snapshot while
   `activate_changes.py:activate_site_changes` skips `_do_activate` and calls
   `_confirm_synchronized_changes` in place of `_confirm_activated_changes`, which would remove the
   changes. The pending changes therefore survive, the central change list still shows them, and the
   remote core is not reloaded — `_do_activate` is the only path to the core-reload automation.
   Clearing `force_sync` is not bookkeeping, though: `_needs_sync` returns that flag as an override
   when it is set (`activate_changes.py:_needs_sync`), and `is_sync_needed` — what
   `sync_changes_before_remote_automation` returns early on — is `any(_needs_sync(...))` once
   `site_is_local` has been ruled out (`activate_changes.py:_is_sync_needed_specific_changes`). So
   the
   pre-sync **consumes the sync obligation while leaving the change pending**: a second read of the
   same remote host pushes nothing new, and the eventual real "Activate changes" finds nothing to
   sync and proceeds straight to activation.
   _Consequence:_ **any read** of a remote host's discovery state copies an unrelated, half-finished
   configuration change onto that remote's **live** tree — the pre-sync sits in `get_check_table`'s
   remote branch with no action guarding it — the branch is on the automation config, and the one
   action `get_check_table` does examine first, `TABULA_RASA`, is examined ahead of both — so
   `DiscoveryAction.NONE` reaches it too: merely opening the page, or a
   `GET /objects/service_discovery/{host_name}`, which passes exactly that. And
   `_update_config_on_remote_site` unpacks into `base_dir = omd_root`. Monitoring there does not see
   the change. The discovery automation does: it runs in the long-lived `cmk-automation-helper` —
   the `AUTOMATION_HELPER` OMD hook defaults to `on`, so
   `automations.py:check_mk_local_automation_serialized` selects `HelperExecutor` over
   `SubprocessExecutor` — and that helper calls `reload_if_required` before every automation
   (`automation_helper/_app.py`), reloading when the watcher's last detected change is newer than
   the last reload; the watcher's schedule covers `etc/check_mk/conf.d` for `*.mk`
   (`automation_helper/_config.py`). So a pending "Disabled services" or discovery-parameters rule
   takes effect on **the scan that triggered the sync** — but only eventually, and by way of three
   hops with no happens-before edge between them: `_update_config_on_remote_site` writes the file,
   `_AutomationWatcherHandler._store_last_detected_change` records it into redis asynchronously, and
   `reload_if_required` compares `last_reload` against `get_last_detected_change()`
   (`automation_helper/_cache.py`) whenever the next automation happens to arrive. Note what
   `_watcher.py`'s comment above `InotifyBuffer.delay = 0.0` is and is not: it describes watchdog's
   move-pairing delay and then sets it to zero, so it documents a hazard the code **eliminates**,
   not this residual one — its value here is as evidence that the maintainers thought the window
   real enough to tune away.

   A second and unrelated mechanism can defeat the same expectation.
   `ServiceDiscoveryBackgroundJob.get_result` prefers a stored preview over recomputing (`elif
(last_result := self._load_last_preview()) is not None:`), `_store_last_preview` is written only
   at the end of a `REFRESH` scan, and `_load_last_preview` unlinks the file after reading it, so
   it is consumed exactly once. A T4.3 sequence of _refresh → PUT ignored → read_ therefore gets a
   preview computed **before** the rule existed, for reasons that have nothing to do with inotify.
   Between the two, T4.3 must assert this rather than assume it, and the assertion may need a retry;
   neither window has been measured. On a local host the same read copies nothing. Two consequences
   for T4.3 either way:
   the effect on the preview precedes activation while the effect on monitoring follows it, so the
   two cannot be tested as one before/after contrast; and the test cannot establish a "before the
   sync" state by reading, because **the read is the sync**.
...
`analyze_service_rule_matches` hard-codes `LocalAutomationConfig()` and takes no
`automation_config` at all (`check_mk_automations.py`), so the disabled-rule arithmetic in
`EnabledDisabledServicesEditor` runs on the **central** site even for a remote host. Unlike B-F1
this is not a constraint to work around — it is correct, and Tier 2's "remote work goes to the
remote site" assertion carves it out on purpose. Recorded because an unexplained exception in a
...
(`cmk/base/config/_impl.py:ConfigCache.__init__`): the loaded config's host list verbatim, with no
site filter, and WATO writes every host of the folder tree into `all_hosts` and every cluster into
`clusters`, both of which that property unions
(`cmk/ccc/hostaddress.py:Hosts.all_configured_hosts`). `is_active` does not enter at this point —
it is applied one call later, to narrow `_all_processed_hosts`.

**The data the analysis reads is central data.** Folder path, host tags, explicit host labels and
the "Host labels" ruleset all come from the central config, of which the remote's copy is
downstream; the `site` **tag** in particular is written explicitly per host into `hosts.mk` and
does not drift. Service labels are not read from local config at all: they arrive as an argument.
`EnabledDisabledServicesEditor._save_service_enable_disable_rules` (`rulesets.py`) fetches them
from the owning site with `get_services_labels(automation_config, …)` and, five lines later, passes
them into the `Ruleset.analyse_ruleset` call that reaches this automation. One call dispatched
...
so `matcher.py:_all_matching_hosts_computation` — which filters within whatever scope it is handed
— returns the same verdict for it. Without the handler's call the flag would come out **`True`** on
a distributed central site, not `False`: `ConfigCache.__init__` seeds the processed set with
`is_active`-filtered hosts (`cmk/base/config/_impl.py:ConfigCache.__init__`) and the automation
helper re-applies the same filter before every call
(`automation_helper:_clear_caches_before_each_call`), while `is_active` is `False` centrally for a
host assigned to another site — `_update_distributed_wato_file` (`watolib/sites.py`) writes
`distributed_wato_site` into the **central** site's own `distributed_wato.mk` as well, not only
...
1. _Discovered_ host labels — `labels_of_host` → `discovered_labels_of_host` →
   `DiscoveredHostLabelsStore` — are replicated to the centre on purpose by
   `cmk/gui/watolib/host_label_sync.py`, whose module docstring is _"Synchronize discovered host
   labels from remote site to central site"_: a cron job once a minute plus an on-demand per-host
   variant. Their residual freshness gap is below.
2. **Builtin** host labels are not replicated, and they win (B-F5). `labels_of_host` merges its
   four sources with builtin **last**, so `_builtin_host_labels()` overrides everything beneath it.
   That reads `var/check_mk/builtin_host_labels.json`, one file per site holding `cmk/site` (plus
   `cmk/customer` under managed services), written as `str(omd_site())` by a `pre-activate-changes`
   hook whose docstring says so outright: _"The file is never synced from the central site (like
   the discovered host labels file)"_ (`watolib/builtin_host_labels.py`). So on the central site
   `labels_of_host(<a remote host>)` reports `cmk/site = <the central site>`. This is not a
   freshness window but a permanent misread, and `execute_host_label_sync` cannot repair it — that
   copies only `discovered_host_labels/*.mk`. **Accepted risk, not ticketed** (§9.1) — three facts
   scope it, and they are recorded so the acceptance can be re-evaluated without re-deriving the
   mechanism. It is the `cmk/site` **label**, not the `site` **tag** above, so tag-conditioned
   rules are unaffected. It can only bite where a site evaluates rules for a host it does **not**
   monitor, and never config generation, where each site labels its own hosts correctly. That is
   wider than this path: B-F5 is a property of `labels_of_host`, so **host**-rule analysis is
   affected too — `check_mk.py:_automation_analyze_host_rule_effectiveness` filters its host list
   by `config_cache.is_online` **only, not `is_active`**, over hosts, clusters and shadow hosts,
   and calls `labels_of_host` for each, so on a central site it evaluates every remote host with
   `cmk/site = <central>` and drives a user-visible "rule is ineffective" verdict from it (reached
   from `wato/pages/rulesets.py:1330` and `watolib/rulesets.py:1020`, and `Rule.matches_search` →
   `_match_rule_host_list` walks the folder tree the same way). On the service side the affected
   callers are the three of `analyze_service_rule_matches`: this one, `ModeEditRuleset`'s match
   simulation and `ModePatternEditor`. And on this path it inherits exactly the narrowness of the
   freshness gap below: a disabled-services rule written by discovery carries no label condition at
   all, so only a hand-written `cmk/site`-conditioned `ignored_services` rule could be affected.

**Residual freshness gap in the discovered labels — out of scope, deliberately not ticketed
(§9.1).** `perform_fix_all` calls `_perform_update_host_labels`, which writes the new labels on the
**host's** site, and then runs the arithmetic above against the central copy, with nothing in
between to sync. `analyze_service_rule_matches` has three callers and only one forces a sync:
`ModeEditRuleset._get_host_labels_from_remote_site` calls `execute_host_label_sync` for a remote
host, returns early for a local one, and explains itself in a docstring — _"To be able to execute
the match simulation we need the discovered host labels to be present in the central site"_
(`wato/pages/rulesets.py:ModeEditRuleset._get_host_labels_from_remote_site`). `ModePatternEditor`
does not, and neither does the discovery path, so **two of the three** analyse a remote host
against whatever the cron last left behind. It is judged too narrow to ticket because a
disabled-services rule _written by discovery_ always targets the host and names the service, never
a host label — a stale label can only matter for a hand-written label-conditioned rule, which
`analyse_ruleset` does evaluate (it walks the whole ruleset via `get_rules()`) but which stacks a
second precondition on an already narrow case.

One note for §7, stated no more strongly than it has been established. The gap is asymmetric **in
the steady state**: on a local host the labels land in the very directory the analysis reads and
the automation helper watches it (`automation_helper/_config.py` schedules
`var/check_mk/discovered_host_labels` for `*.mk`), whereas on a remote host the central copy is
simply older and no reload can repair a file that has not been written. Whether a Tier 4 assertion
could **see** that is a separate question and is not established: Tier 4 observes a single request,
and within one `perform_fix_all` the local path is racy too, by the same watcher race B-F2.1
documents. Should the local window outlast the request, both sites are stale and parity holds
vacuously — the §7 hazard this would otherwise be a counterexample to. Neither window has been
measured, and no Tier 4 row as specified sets up the label-conditioned `ignored_services` rule that
would make any of this observable. The in-process form of the same symptom — analysis seeing
pre-update labels because the matcher cache was not invalidated — is recorded under CMK-22846 and
CMK-19632; whether either is live on this path has not been checked here.
...
| T4.2 | `test_autochecks_written_on_the_owning_site`                               | `var/check_mk/autochecks/<host>.mk` changes on the **remote** site and **not** on the central. Direct regression test for the PoC's worst defect. Assert file _content_ — see §10.8 below.                                                                                                                                                                                                  |
| T4.3 | `test_disabled_services_rule_is_central_and_reaches_the_remote`            | `ignored → unchanged` and `unchanged → ignored` on a remote host: the `ignored_services` rule lands in the **central** config and the remote honours it — the preview from the moment the pre-sync copies the rule across (B-F2.1), monitoring only after activation. Two observations, not one — and neither of them is that the remote autochecks file stops holding the service (§10.1). |
...
#### What §10 does to these tests

A parity assertion says _central ≡ remote_, which makes it blind to every defect that behaves the
same way on both sites — and **every item of §10 is symmetric except §10.10** (§6.1's B-F2
collects the asymmetries separately, and so do two findings of its own: B-F4's residual freshness
gap, whose observability from a Tier 4 assertion §6.1 deliberately leaves open, and B-F5, which §6.1
calls a permanent misread rather than a window — though no Tier 4 row as specified sets up the
`cmk/site`-conditioned rule that would make B-F5 fire here). One qualification:
§10.15 is symmetric only because both sites of a test setup run the same monitoring core, which is a
property of the setup and not of the code. So the hazard for Tier 4 is not a red test, it is a
**green** one. Three items produce a pass that proves nothing:

- **§10.19** — an unknown `ServiceID` is answered `204` with nothing written. T4.4 addresses one
  service per site, so an identifier matching **neither** host — a typo in the fixture, or an item
  spelling that has drifted from the agent output — yields `204` on both sides, no write anywhere,
  and "identical result" holds. **T4.4 must assert a positive change per site**, never equality
  alone. Note which half of that is the hazard: an identifier that matches one host and not the
  other is the benign case, since one side writes and one answers `204` and the assertion goes red.
  It is the identifier matching nothing that passes while proving nothing. The item is low priority
  for users and load-bearing for this suite.
- **§10.18** — a write issued while a scan is active is answered `204` and discarded. The green
  pass is the case where a scan is active on both sites: the write is dropped on each, nothing
  changes anywhere, and equality holds. Where only one site is scanning the test goes red or
  nondeterministic instead, which is the flakiness this contributes in a suite driving real scans —
  and the two are not equally likely, because the remote path inserts a pre-sync bounded at 30
  seconds plus an HTTP round trip before the table is read. Either way the remedy is the same: use
  `run_discovery_and_wait_for_completion` throughout. Note the `409` guard does cross the site
...
- **§10.8** — `fix_all` rewrites the autochecks with an empty diff on any host carrying an active,
  ...
  expect the no-op case to flip when §10.8 lands.
...
- **§10.1** leaves a disabled service in the autochecks file. T4.3 must not assert that the remote
  file stops containing it: today it does not, for a reason that has nothing to do with remoteness.
...
endpoints, so none of the tests above reaches it; T2.15 covers it with a mock, and an end-to-end
version would have to be a further case here. Everything is driven as an administrator, so B-F2.4
stays untested — the remote `AutomationServiceDiscoveryJob._check_permissions` requires only
`wato.hosts` where the central path demanded `wato.services`. §10.4 does **not** belong on this
list: it is the _central_ endpoint's missing `wato.services` / `wato.edit` gate, not a remoteness
story and is already covered by T2.12 and T3.3a. And both sites run the same version, so
B-F2.3's version-truncated wire format never fires: T4.1 establishes parity **at equal versions**,
which belongs in its docstring so that it is not read as covering the supported skew.
...
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| A1-F1 | `FIX_ALL` retargets **11 of 13** sources to `unchanged`; the guarding comment is false and never was true (since 2020)                                                                                                                                  | Spurious pending change + automation round trip per click on most real hosts; also prunes enforced-shadowed autochecks, so their survival depends on an unrelated click — §10.8 retracts the data-loss reading of that, and domain model §12.8 owns the remaining question | ✅ **confirmed** → §10.8                                                         |
...
| B-F4      | **Not an asymmetry — intentional, out of scope for CMK-34150, no ticket.** Central execution is correct; §6.1 records the one fact it rests on and why the rest is fallback. The residual gap — `perform_fix_all` writes discovered labels on the host's site and then analyses against the central copy with nothing in between to sync, where `ModeEditRuleset` forces a sync — is judged too narrow to ticket, because a disabled-services rule written by discovery targets the host and names the service, never a label.                                                                     |
| B-F5      | **Accepted risk, no ticket.** Not staleness and not a consequence of a host moving between sites: the central site's `builtin_host_labels.json` holds the central site's own id and is read for every host it evaluates, so `labels_of_host(<any remote host>)` reports `cmk/site = <central>` permanently. Accepted on that reading. Scoped by three facts in §6.1 — the label not the tag, GUI-side analysis only, and no label condition on a discovery-written rule.                                                                                                                           |
