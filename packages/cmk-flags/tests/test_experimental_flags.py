...
class _ExampleFlags(BaseModel):
    """Stand-in with a real flag, since ``ExperimentalFlagConfig`` may declare none."""
...
def test_experimental_field_records_metadata() -> None:
...
def test_no_expired_flags() -> None:
    ...
    for name, field_info in ExperimentalFlagConfig.model_fields.items():
        ...
        assert Version.from_str(remove_after) > current, (
            f"Experimental flag '{name}' passed its removal deadline {remove_after} "
...
def test_load_ignores_unknown_flags(tmp_path: Path) -> None:
    (tmp_path / "experimental_flag.json").write_text('{"already_removed_flag": true}')
    flags = load_experimental_flags(tmp_path)
