...
function labelFont(reference: Element | null): LabelFont {
  if (!reference) {
    return { spec: `${FALLBACK_FONT_SIZE_PX}px sans-serif`, sizePx: FALLBACK_FONT_SIZE_PX }
  }
  ...
  const font = { spec: `${sizePx}px ${style.fontFamily || 'sans-serif'}`, sizePx }
...
export function measureAxisLabel(text: string, reference: Element | null): number {
  ...
  if (context) {
    ...
    width = context.measureText(text)?.width ?? 0
  }
  if (!width) {
    width = text.length * font.sizePx * FALLBACK_EM_PER_CHAR
  }
