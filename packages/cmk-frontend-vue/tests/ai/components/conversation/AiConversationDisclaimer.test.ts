...
const LEGAL = {
  ...
  disclaimer_body_template: 'Powered by {models} via {provider}.',
...
describe('AiConversationDisclaimer — AI info interpolation', () => {
  test('interpolates model and provider names into the body when info is already available', async () => {
    ...
    // a single element.
    await waitFor(() => {
      expect(document.body.textContent).toContain('Powered by GPT-4 via OpenAI.')
    })
  })
  ...
  test('updates the body text once info loads via onInfoLoaded callback', async () => {
    ...
    await waitFor(() => {
      expect(document.body.textContent).toContain('Powered by Claude via Anthropic.')
    })
  })
  ...
  test('handles multiple models by joining them with a comma', async () => {
    ...
    await waitFor(() => {
      expect(document.body.textContent).toContain('Powered by GPT-4, GPT-3.5 via OpenAI.')
    })
  })
})
