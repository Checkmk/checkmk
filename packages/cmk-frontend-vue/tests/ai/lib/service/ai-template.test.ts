...
vi.mock('@/lib/keyShortcuts', () => ({
  KeyShortcutService: vi.fn().mockImplementation(() => ({ on: vi.fn(), remove: vi.fn() }))
}))
...
beforeEach(() => {
  ...
  MockAiApiClient.mockImplementation(
    () =>
      ({
        getInfo: mockGetInfo,
...
describe('execAiAction — streaming', () => {
  ...
  test('onError does NOT push an alert when the stream signal was already aborted', () => {
    ...
    let firstElement!: IAiConversationElement
    ...
    const service = makeService()
    firstElement = cast(service).execAiAction(ACTION) as IAiConversationElement
