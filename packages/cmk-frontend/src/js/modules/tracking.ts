...
export async function activate_tracking(url: string, startTime: number, backendDuration: number) {
  ...
  const mutationWaiter = waitForMutationsToStop(document.body, startTime, 2000, isMouseoverMutation)
  ...
  await storePageLoadMetric(db, url, `${prefix}rendered[backend]`, backendDuration)
...
async function getRepeat(db: IDBDatabase, url: string): Promise<repeatUrlsEntry | null> {
...
async function setRepeat(db: IDBDatabase, repeat: repeatUrlsEntry): Promise<repeatUrlsEntry> {
  const transaction = db.transaction([repeatUrlsTable], 'readwrite')
  const store = transaction.objectStore(repeatUrlsTable)
...
export async function repeat(times: number, prefix = ''): Promise<repeatUrlsEntry> {
  const db = await openDatabase()
...
export async function nukeDataFromOrbit() {
  ...
  return new Promise((resolve, reject) => {
    ...
    clearRequest.onsuccess = onSuccess(resolve, () => console.warn('Nuking data: done.'))
