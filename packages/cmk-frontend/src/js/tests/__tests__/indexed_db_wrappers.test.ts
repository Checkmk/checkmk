...
import { getRecord, openDatabase, putRecord, repeatUrlsTable } from '@/modules/tracking_database'
...
describe('When the indexeddb is empty', () => {
  ...
  test('we want to reload the page 5 times', async () => {
    ...
    const storedEntry = await getRecord<repeatUrlsEntry>(store, 'localhost')
