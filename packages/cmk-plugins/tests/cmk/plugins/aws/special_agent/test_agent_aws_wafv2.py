...
def test_search_string_bytes_handling_in_get_wafv2_web_acls() -> None:
    ...
    res = _get_wafv2_web_acls(fake_wafv2_client, "us-east-1", get_response_content, None, None)
...
def test_agent_aws_wafv2_regional_cloudfront() -> None:
    ...
    assert wafv2_limits_regional._region_report == region  # noqa: SLF001

    wafv2_limits_regional = WAFV2Limits(None, "us-east-1", config, "CLOUDFRONT")
    assert wafv2_limits_regional._region_report == "CloudFront"  # noqa: SLF001
    ...
    assert len(WAFV2WebACL(None, "region", config, True)._static_metric_dimensions) == 2  # noqa: SLF001
