    ...
    def validate(self) -> None:
        ...
        for aux_tag in self._tags:
            ...
            if builtin_config.aux_tag_list.exists(aux_tag.id):
                raise MKGeneralException(
                    _('You cannot override the built-in auxiliary tag "%(aux_tag_id)s".')
                    % {"aux_tag_id": aux_tag.id}
                )

            if builtin_config.tag_group_exists(TagGroupID(aux_tag.id)):
                raise MKGeneralException(
                    _('You cannot override the built-in tag group "%(aux_tag_id)s".')
                    % {"aux_tag_id": aux_tag.id}
                )

            if aux_tag.id in seen:
                raise MKGeneralException(
                    _('Duplicate tag ID "%(aux_tag_id)s" in auxiliary tags')
                    % {"aux_tag_id": aux_tag.id}
                )
...
class TagConfig:
        ...
        if tag_groups_using_aux_tag:
            raise AuxTagInUseError(
                _(
                    ...
                    'It is being used by the following tag groups: "%(tag_groups)s"'
                )
                % {"tag_groups": ", ".join(tag_groups_using_aux_tag)},
            )
    ...
    def update_tag_group(self, tag_group: TagGroup) -> None:
        for idx, group in enumerate(self.tag_groups):
            ...
            raise MKGeneralException(
                _('Unknown tag group "%(tag_group_id)s"') % {"tag_group_id": tag_group.id}
            )
        self._validate_group(tag_group)
    ...
    def _validate_ids(self) -> None:
        ...
        for tag_group in self.tag_groups:
            if tag_group.id in seen_ids:
                raise MKGeneralException(
                    _('The tag group ID "%(tag_group_id)s" is used twice.')
        ...
        for aux_tag in self.aux_tag_list.get_tags():
            if aux_tag.id in seen_ids:
                raise MKGeneralException(
                    _('The tag ID "%(aux_tag_id)s" is used twice.') % {"aux_tag_id": aux_tag.id}
                )
    ...
    def _validate_group(self, tag_group: TagGroup) -> None:
        ...
        if tag_group.id == "site":
            raise MKGeneralException(
                _('The tag group "%(tag_group_id)s" is reserved for internal use.')
                % {"tag_group_id": tag_group.id}
            )
        ...
        if builtin_config.tag_group_exists(tag_group.id):
            raise MKGeneralException(
                _('You cannot override the built-in tag group "%(tag_group_id)s".')
                % {"tag_group_id": tag_group.id}
            )

        if builtin_config.aux_tag_list.exists(TagID(tag_group.id)):
            raise MKGeneralException(
                _('You cannot override the built-in auxiliary tag "%(tag_group_id)s".')
                % {"tag_group_id": tag_group.id}
            )

        if not tag_group.title:
            raise MKGeneralException(
                _('Please specify a title for your tag group "%(tag_group_id)s".')
                % {"tag_group_id": tag_group.id}
            )
            ...
            if tag.id or tag.title:
                ...
                for n, x in enumerate(tag_group.tags):
                    if n != nr and x.id == tag.id:
                        raise MKGeneralException(
                            _('Tags IDs must be unique. You\'ve used "%(tag_id)s" twice.')
                            % {"tag_id": tag.id}
                        )
