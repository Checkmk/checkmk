...
class ActiveCheckCommand:
    ...
    Example:
        ...
        ...     command_arguments=["--user", "example-user", "--password", StoredSecret("stored_password_id")]
        ...     )
        ActiveCheckCommand(service_description='Example description', command_arguments=['--user', 'example-user', '--password', StoredSecret(value='stored_password_id', format='%s')])
    """
...
class ActiveCheckConfig(Generic[_ParsedParameters]):
    ...
    Args:
        name: Active check name. Has to match active check executable name without the prefix ´check_´.
