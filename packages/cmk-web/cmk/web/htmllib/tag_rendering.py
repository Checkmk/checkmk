...
from cmk.web.utils import escaping, urls
...
def normalize_css_spec(css_classes: CSSSpec | str | None) -> list[str]:
    match css_classes:
        case [*css_specs]:
            # Callers pass None entries despite CSSSpec = list[str]; filter defensively.
            return [c for c in css_specs if c is not None]  # type: ignore[redundant-expr,comparison-overlap]
