...
def test_a_healthy_setup_has_no_problems(tmp_path: Path) -> None:
    status = _collect(_paths(tmp_path, [20_251, 20_252]))
...
def test_reserved_ids_without_any_id(tmp_path: Path) -> None:
    status = _collect(_paths(tmp_path))
...
def test_no_reserved_ids_and_unreachable_server_is_a_problem(tmp_path: Path) -> None:
    status = _collect(_paths(tmp_path), server_status=ServerStatus.UNREACHABLE)
...
def test_unreachable_server_with_reserved_ids_reports_no_problem(tmp_path: Path) -> None:
    status = _collect(_paths(tmp_path, [20_251]), server_status=ServerStatus.UNREACHABLE)
...
def test_rejected_secret_is_a_problem(tmp_path: Path) -> None:
    status = _collect(_paths(tmp_path, [20_251]), server_status=ServerStatus.UNAUTHORIZED)
...
def test_server_error_is_a_problem(tmp_path: Path) -> None:
    status = _collect(_paths(tmp_path, [20_251]), server_status=ServerStatus.ERROR)
...
def test_a_missing_secret_is_a_problem(tmp_path: Path) -> None:
    status = _collect(_paths(tmp_path, [20_251], secret_mode=None), server_status=None)
...
def test_a_leftover_legacy_stash_is_a_problem(tmp_path: Path) -> None:
    ...
    assert status.legacy_stash.path == str(paths.legacy_stash_file)
...
def test_secret_with_restrictive_mode_is_no_problem(tmp_path: Path) -> None:
    status = _collect(_paths(tmp_path, [20_251], secret_mode=0o600))
...
def test_any_group_or_other_access_to_the_secret_is_a_problem(tmp_path: Path, mode: int) -> None:
    status = _collect(_paths(tmp_path, [20_251], secret_mode=mode))
...
def test_server_status_tokens_are_machine_readable(
    ...
    status = _collect(_paths(tmp_path, [20_251]), server_status=server_status)
...
def _status(
    *,
    ...
    return Status(
        ...
        secret=secret or FileInfo(path="/h/secret", exists=True, mode="0600"),
        reserved_ids=reserved_ids
        or StashInfo(path="/h/reserved", exists=True, mode="0600", count=3, next_id=20_251),
        legacy_stash=legacy_stash
        or StashInfo(path="/h/legacy", exists=False, mode=None, count=0, next_id=None),
        problems=list(problems),
    )


def _rendered(status: Status) -> str:
    stream = io.StringIO()
    render_status(status, Console(file=stream, width=120, no_color=True, highlight=False))
    return "\n".join(line.rstrip() for line in stream.getvalue().split("\n"))


def test_render_shows_both_sections() -> None:
    output = _rendered(_status())
...
def test_render_hides_the_legacy_stash_while_it_does_not_exist() -> None:
    assert "legacy stash" not in _rendered(_status())
    ...
    output = _rendered(
        _status(
            legacy_stash=StashInfo(
                path="/h/.cmk-werk-ids", exists=True, mode="0600", count=2, next_id=11_120
            )
        )
    )
...
def test_render_does_not_fold_a_long_path_when_wide_enough() -> None:
    long_path = "/home/user/.local/state/cmk-werks/reserved-ids"
    output = _rendered(
        _status(
            reserved_ids=StashInfo(
                path=long_path, exists=True, mode="0600", count=1, next_id=20_251
            )
        )
    )
...
def test_json_is_a_single_document_with_a_schema_version() -> None:
    document = json.loads(render_json(_status()))
...
def test_json_has_no_problems_when_healthy() -> None:
    assert json.loads(render_json(_status()))["problems"] == []


def test_json_contains_no_ansi_escapes() -> None:
    assert "\x1b" not in render_json(
        _status(problems=[Problem(item="secret", problem="p", fix="f")])
    )
...
def test_a_legacy_stash_on_its_own_is_the_one_in_use(tmp_path: Path) -> None:
    ...
    status = _collect(paths, server_status=None)
