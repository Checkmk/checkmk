...
onBeforeMount(() => {
  ...
  props.component.config.elements.forEach((element) => {
    if (element.is_active) component_value[element.name] = element.component.config.value
  })
})
onMounted(() => {
  props.component.config.elements.forEach((element) => {
    if (element.is_active || element.required) element_active.value[element.name] = true
  })
...
<template>
  <table class="dictionary">
    <tbody>
      <tr v-for="element in get_elements_from_props()" v-bind:key="element.name">
        <td class="dictleft">
          <span class="checkbox">
            <input
              type="checkbox"
            ...
            />
            <label :onclick="(event) => clicked_dictionary_checkbox_label(event, element.name)">
          ...
          <div class="dictelement indent">
            <DForm
              ...
              "
              @update-value="(new_value) => update_key(element.name, new_value)"
            />
          </div>
        </td>
      </tr>
