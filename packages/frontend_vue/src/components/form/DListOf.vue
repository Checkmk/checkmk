...
function get_dragged_row(event: D3DragEvent<HTMLImageElement, null, HTMLImageElement>) {
...
function drag_end(event: D3DragEvent<HTMLImageElement, null, HTMLImageElement>) {
...
function add_element() {
  ...
  let new_element = JSON.parse(JSON.stringify(props.component.config.template))
...
function update_actual_value() {
  ...
  table.querySelectorAll(':scope > tr.' + class_listof_element).forEach((child_node) => {
    actual_value.push(component_value[child_node.attributes.uuid.value])
  })
