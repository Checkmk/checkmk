...
flowchart LR
    ...
    C --> I["Integration tests<br/>(Docker DB + both plugins,<br/>tests/system/mk_oracle)"]
...
- **Integration** — `tests/system/mk_oracle/` at the repo root: Dockerised Oracle Free, exercises the built plugin end-to-end including the legacy-vs-new comparison harness.
