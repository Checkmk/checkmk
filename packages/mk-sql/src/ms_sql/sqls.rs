...
SELECT InstanceNames, InstanceIds, EditionNames, VersionNames, ClusterNames,Ports, DynamicPorts FROM @GetAll;";
    ...
    pub const COUNTERS_ENTRIES: &str =
        "SELECT cast(counter_name as NVARCHAR(128)) as counter_name, \
...
cast(DATABASEPROPERTYEX(name, 'Status') as nvarchar(128)) AS Status, \
  cast(DATABASEPROPERTYEX(name, 'Recovery') as nvarchar(128)) AS Recovery, \
...
FROM master.dbo.sysdatabases";
    ...
    pub const CLUSTER_NODES: &str =
        "SELECT cast(nodename as NVARCHAR(128)) as nodename FROM sys.dm_os_cluster_nodes";
...
FROM sys.databases";
  ...
  sj.job_id AS job_id, \
  cast(sj.name  as NVARCHAR(128)) AS job_name, \
  ...
  CAST(sjserver.last_outcome_message as NVARCHAR(250)) as last_outcome_message, \
...
INNER JOIN master.sys.availability_groups GroupsName ON Groups.group_id = GroupsName.group_id";
    ...
    cast(SERVERPROPERTY( 'ComputerNamePhysicalNetBIOS' ) as nvarchar(128)) as NetBios";
