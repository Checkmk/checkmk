...
@pytest.mark.parametrize("info,discovery_expected,check_expected", [
    (  # no info
        [],
        [],
    ...
    (  # Mountpoint with spaces and permission denied
        ...
        [('/var/dba export', {}, BasicCheckResult(2, 'Permission denied', None)),
         ('/var/dbaexport', {}, BasicCheckResult(0, '15.2% used (931.48 GB of 6.00 TB)', None))]
    ),
    (  # with perfdata
        ...
        [('/PERFshare', {'has_perfdata': True},
          BasicCheckResult(0, "6.4% used (1.91 GB of 30.00 GB)", [
              PerfValue('fs_size', 491520 * 65536), PerfValue('fs_used', 491520 * 65536 - 460182 * 65536)
          ]))]
    ),
    (  # state == 'hanging'
