...
def _change_password(page: PPage, old_password: str, new_password: str) -> None:
    ...
    page.page.locator("text=Change password").click()
    ...
    expect(page.main_frame.locator("div.success")).to_have_text("Successfully changed password.")
...
def test_v2_1_5(logged_in_page: PPage) -> None:
    ...
    expect(page.page.locator("div.error")).to_have_text("Invalid login")

    # changing it back for other tests
    page.login("cmkadmin", "not-cmk")
    expect(page.main_frame.locator("div.titlebar > a")).to_have_text(
        "Main dashboard"
    )
