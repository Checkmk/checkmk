...
def _legacy_lib_file(site: Site) -> Iterator[Path]:
    ...
    try:
        site.makedirs(site_rel_lib_file.parent)
        site.write_text_file(site_rel_lib_file, "")
        yield lib_file
    finally:
        site.delete_file(site_rel_lib_file)  # should be gone anyway
        site.delete_dir(site_rel_lib_file.parent)
