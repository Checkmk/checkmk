...
def default_cfg(site: Site) -> Generator[None, None, None]:
    ...
    try:
        ...
        for i in range(EXPECTED_HOST_COUNT):
            site.openapi.delete_host(f"test-host-pt{i + 1}")
        for i in range(EXPECTED_FOLDER_COUNT):
            site.openapi.delete_folder(f"/test_folder_pt{i + 1}")
        site.openapi.activate_changes_and_wait_for_completion()
...
def mock_https_server(site: Site, tmp_path: Path) -> Iterator[MockServer]:
    ...
    with MockServer(https=True, cert_dir=str(tmp_path)) as mock_server:
        ...
        site.write_text_file(ca_bundle_path, original_ca_bundle + "\n" + mock_cert)
...
def test_successful_https_product_usage_data_transmission(
    site: Site,
    mock_https_server: MockServer,  # pylint: disable=redefined-outer-name
) -> None:
    """Test successful HTTPS transmission of product usage analytics data."""
    run_product_usage_collection(site)
...
def test_unsuccessful_https_product_usage_data_transmission(
    ...
    """Test that failed HTTPS transmission keeps product usage analytics files."""
    run_product_usage_collection(site)
