...
def test_update_package_code(tmp_path: Path) -> None:
    ...
    try:
        ...
        msi_engine.update_package_code(str(tgt))
        ...
        msi_engine.update_package_code(
            str(tgt), package_code="{01234567-1234-1234-1234-012345678901}"
        )
        tgt_content = tgt.read_bytes()
        ...
        assert not tgt.exists()
        assert msi_engine.copy_file_safe(src, tgt)
        msi_engine.update_package_code(str(tgt), package_code="012")
        tgt_content = tgt.read_bytes()
