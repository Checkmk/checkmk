...
def assert_predictive_levels_structure(
    ...
    for element in required_elements:
        assert element in stored_value, f"Rule should contain required element '{element}'"

    if expected_checks:
        for check in expected_checks:
            assert check in stored_value, f"Rule should contain expected element '{check}'"


def assert_fixed_levels_structure(stored_value: str, expected_checks: list[str]) -> None:
    """Assert fixed levels structure is present."""
    assert "fixed" in stored_value, "Rule should contain required element 'fixed'"
    for check in expected_checks:
        assert check in stored_value, f"Rule should contain expected element '{check}'"
