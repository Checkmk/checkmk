...
def inject_otel_dumps(otel_site: Site) -> Iterator[None]:
    ...
    """
    dump_rule_id = inject_dumps(otel_site, DUMP_SOURCE_DIR, check_dumps_up_to_date=False)
...
def test_otel_service_monitoring_rules(
    ...
    try:
        ...
        if dump_update_required:
            ...
            otel_site.reschedule_services(host_name, max_count=3, strict=True)

        logger.info(f"Retrieving services for host {host_name}")
        services = otel_site.get_host_services(
            host_name, pending=False, extra_columns=["performance_data"]
        )
        assert len(services) == expected_services_count, (
