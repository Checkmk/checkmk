...
def test_datasource_ok(site: Site, redfish_hosts: Hosts) -> None:
    checkmk_service = get_service(site, redfish_hosts.dell_ok, "Check_MK")
...
def test_host_labels(site: Site, redfish_hosts: Hosts) -> None:
    raw = site.openapi.service_discovery.get_discovery_result(redfish_hosts.dell_ok)["extensions"]
    ...
    assert {k: v["value"] for k, v in discovered_labels.items()} == {
        ...
        "cmk/os_name": "iDRAC",
        "cmk/os_platform": "Dell",
        "cmk/os_type": "redfish",
        "cmk/os_version": "7.00.60.00",
    }


def test_no_agent_service(site: Site, redfish_hosts: Hosts) -> None:
    assert "check_mk-checkmk_agent" not in {
        s.check_command for s in get_services(site, redfish_hosts.dell_ok)
    }
...
def test_plugins(site: Site, redfish_hosts: Hosts) -> None:
    ...
    """
    services = get_services(site, redfish_hosts.dell_ok)
