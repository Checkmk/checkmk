...
class _AutomationHTTPHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format: str, *args: object) -> None:  # noqa: A002
        logger.debug("mock-http: " + format, *args)
...
class _LivestatusHandler(socketserver.StreamRequestHandler):
    """Minimal livestatus server: answers GET queries with canned status data."""
    ...
    def _handle_query(self, request_lines: list[str]) -> bool:
        verb = request_lines[0].split(" ", 1)[0]
        headers = {}
...
class MockRemoteSiteCluster:
    ...
    def __init__(
        ...
        self._http_server = _MockHTTPServer(
            http_port, self._sites, cmk_version, cmk_edition, activate_delay
        )
        status_values: dict[str, object] = {
            "livestatus_version": cmk_version,
            "program_version": f"Check_MK {cmk_version}",
