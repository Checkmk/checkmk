...
from tests.system.gui.testlib.playwright.pom.setup.otel.open_telemetry_collector_prometheus_scraping import (
...
class AddOpenTelemetryCollectorPrometheusScraping(CmkPage):
    ...
    def job_name_textfield(self) -> Locator:
        return self.main_area.locator().get_by_label("Job name")

    @property
    def scrape_interval_textfield(self) -> Locator:
        return self.main_area.locator().get_by_label("Scrape interval")

    @property
    def encrypt_communication_with_tls_checkbox(self) -> Locator:
        return self.main_area.locator().get_by_label("Encrypt communication with TLS")

    @property
    def metrics_path_textfield(self) -> Locator:
        return self.main_area.locator().get_by_label("Metrics path")
    ...
    def ip_address_or_hostname_textfield(self) -> Locator:
        return self.main_area.locator().get_by_label("IP address or hostname")

    @property
    def port_textfield(self) -> Locator:
        return self.main_area.locator().get_by_label("Port")
    ...
    def host_name_rules_dropdown(self) -> Locator:
        return self.main_area.locator().get_by_label("Host name rules")
