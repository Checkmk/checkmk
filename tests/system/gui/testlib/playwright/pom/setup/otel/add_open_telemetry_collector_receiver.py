...
from tests.system.gui.testlib.playwright.helpers import DropdownListNameToID, LocatorHelper
from tests.system.gui.testlib.playwright.pom.page import CmkPage
from tests.system.gui.testlib.playwright.pom.setup.otel.open_telemetry_collector_receiver import (
...
class AddOpenTelemetryCollectorReceiver(CmkPage):
    ...
    def receiver_protocol_endpoint_checkbox(self, receiver_type: str) -> Locator:
        return self._receiver(receiver_type).get_by_label("Endpoint")
    ...
    def ip_address_or_hostname_textfield(self, receiver_type: str) -> Locator:
        return self._receiver(receiver_type).get_by_label("IP address or hostname")

    def port_textfield(self, receiver_type: str) -> Locator:
        return self._receiver(receiver_type).get_by_label("Port")
    ...
    def password_dropdown(self, receiver_type: str) -> Locator:
        return self._receiver(receiver_type).get_by_label("Password")
    ...
    def send_logs_to_event_console_checkbox(self, receiver_type: str) -> Locator:
        return self._receiver(receiver_type).get_by_label("Send log messages to event")
    ...
    def add_new_password(self, receiver_type: str, password_data: dict[str, str]) -> None:
        ...
        self.new_password_slide_in.editable_by_dropdown.click()
        self.new_password_slide_in.editable_by_option("Administrators").click()
...
class NewPasswordSlideIn(LocatorHelper):
    ...
    def locator(self, selector: str | None = None) -> Locator:
        _loc = self.page.frame_locator("iframe[name='main']").locator(
            "div[class*='slide-in__container']"
        )
        if selector is None:
            return _loc
    ...
    def title(self) -> Locator:
        return self.locator("h2[class*='slide-in__title']")
    ...
    @property
    def editable_by_dropdown(self) -> Locator:
        return self.locator().get_by_label("Editable by")

    def editable_by_option(self, option: str) -> Locator:
        return self.locator().get_by_role("option", name=option)
