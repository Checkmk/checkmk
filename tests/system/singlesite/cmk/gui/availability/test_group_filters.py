...
def fixture_grouped_hosts(site: Site) -> Iterator[None]:
    ...
    A host without an agent and without discovered services gets the fallback PING service,
    so no host check command rule is needed. That ruleset is not even available in the cloud
    edition, where the REST API only accepts an allow-listed set of rulesets.
    """
    rule_ids: list[str] = []
...
def _session_user(web: CMKWebSession) -> str:
    """Name of the user the web session acts as

    The cloud edition logs in through the SaaS login instead of as cmkadmin, so the user
    is only known at runtime. The auth cookie starts with their name.
    """
    cookie = web.get_auth_cookie()
    assert cookie is not None and cookie.value, "The web session is not logged in"
    return unquote(cookie.value).split(":")[0]
...
def _group_titles(
    ...
    """
    site.write_file(
        f"var/check_mk/web/{_session_user(web)}/avoptions.mk",
