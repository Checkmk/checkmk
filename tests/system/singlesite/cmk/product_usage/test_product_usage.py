...
def default_cfg(site: Site) -> Generator[None]:
    """Setup site and create resources for product usage analytics tests."""
    site.ensure_running()
...
def mock_https_server(site: Site, tmp_path: Path) -> Iterator[MockServer]:
    ...
    with MockServer(https=True, cert_dir=str(tmp_path)) as mock_server:
        # Read the mock server's self-signed certificate and append it to the site's CA bundle.
        mock_cert = Path(mock_server.cert_file).read_text() if mock_server.cert_file else ""
...
def run_product_usage_collection(site: Site) -> None:
    ...
    result = site.execute(["cmk-product-usage", "--collection"])
    assert result.wait() == 0, f"Product usage collection failed: {result.stderr}"
    assert site.is_dir(
        PRODUCT_USAGE_DATA_PATH
...
def run_product_usage_upload(site: Site, mock_url: str) -> None:
    """Set CMK_PRODUCT_USAGE_URL to the mock server URL and run product usage upload."""
    result = site.execute(
        ["env", f"{PRODUCT_USAGE_URL_ENV_VAR}={mock_url}", "cmk-product-usage", "--upload"]
    )
...
def test_product_usage_collection_writes_one_json_file_and_id_file(site: Site) -> None:
    """Product usage collection writes exactly one JSON file and one site_id file."""
    run_product_usage_collection(site)

    files = site.listdir(PRODUCT_USAGE_DATA_PATH)
    json_files = [f for f in files if f.endswith(".json")]
    assert len(json_files) == 1, f"Expected 1 JSON file, found {len(json_files)}: {json_files}"
    assert "site_id" in files, f"site_id file not found. Found: {files}"


@pytest.mark.usefixtures("default_cfg")
def test_site_id_file_does_not_contain_site_id(site: Site) -> None:
    """site_id file must not contain the site.id."""
    run_product_usage_collection(site)

    site_id = site.read_file(f"{PRODUCT_USAGE_DATA_PATH}/site_id").strip()
    logger.info(f"Product usage site ID: {site_id}")
    assert site.id not in site_id, f"site.id ({site.id}) should not be present in site_id file"
...
def test_product_usage_json_has_expected_structure(site: Site) -> None:
    ...
    json_content = get_product_usage_json_content(site)
    product_usage_data = json.loads(json_content)
    assert "metadata" in product_usage_data
    assert "data" in product_usage_data


@pytest.mark.usefixtures("default_cfg")
def test_product_usage_json_id_matches_site_id_file(site: Site) -> None:
    """Product usage JSON data.id matches site_id file content."""
    run_product_usage_collection(site)

    json_content = get_product_usage_json_content(site)
    product_usage_data = json.loads(json_content)
    site_id = site.read_file(f"{PRODUCT_USAGE_DATA_PATH}/site_id").strip()
    assert product_usage_data["data"]["id"] == site_id
...
def test_product_usage_json_counts_and_version(site: Site) -> None:
    ...
    json_content = get_product_usage_json_content(site)
    product_usage_data = json.loads(json_content)["data"]
    assert product_usage_data["count_hosts"] == EXPECTED_HOST_COUNT
    assert product_usage_data["count_folders"] == EXPECTED_FOLDER_COUNT
    assert product_usage_data["edition"] == site.version.edition.short
    assert product_usage_data["cmk_version"] == f"{site.version.version}"
