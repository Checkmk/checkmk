...
def test_run_omd_reload(site: Site) -> None:
    ...
    p = site.run(["omd", "reload"], check=False)
...
def test_run_omd_reload_service(site: Site) -> None:
    ...
    for service, status in expected.items():
        p = site.run(["omd", "reload", service], check=False)
...
def test_run_omd_backup_and_omd_restore_empty() -> None:
    ...
    try:
        # run the backup
        restored_site = site_factory.get_site(restored_site_name)
