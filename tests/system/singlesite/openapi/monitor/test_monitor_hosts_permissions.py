...
def fixture_contact_group_and_hosts(site: Site) -> Iterator[None]:
    site.openapi.post(
        ...
        json={"name": _CONTACT_GROUP, "alias": _CONTACT_GROUP},
    ).raise_for_status()
...
def fixture_restricted_user(site: Site, contact_group_and_hosts: None) -> Iterator[None]:
...
def _list_hosts(site: Site, value: str) -> requests.Response:
    url = (
        f"http://{site.openapi.host}:{site.openapi.port}/{site.openapi.site}"
        f"/check_mk/api/{APIVersion.INTERNAL}/monitor/hosts"
    )
