...
class CMKOpenApiSession(requests.Session):
    def __init__(
        self, host: str, user: str, password: str, site: str = "heute", api_version: str = "1.0"
    ):
        super().__init__()
        self.host = host
        self.user = user
    ...
    def request(self, method: str, url: str, *args, **kwargs) -> requests.Response:  # type: ignore
        ...
        """
        url = f"http://{self.host}/{self.site}/check_mk/api/{self.api_version}/{url.strip('/')}"
    ...
    def activate_changes(
        self, sites: Optional[list[str]] = None, force_foreign_changes: bool = False
    ) -> bool:
        """
        Returns
        ...
        if response.status_code == 404:
            # no running activation found
            #  this is caused when the activation is finished before the redirection to the
            #  wait for completion endpoint happens
            return True
