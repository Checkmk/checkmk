...
setUp() {
    # section_job() reads the job files below ${MK_VARDIR}/job, so the agent has to be
    # pointed at the temporary directory the tests fill. Without this the cd in
    # section_job() fails and the whole section comes out empty - which lets every
    # assertion on absent output pass without the agent ever having been exercised.
