...
def test_discover_3par_capacity(
    ...
    parse_3par_capacity = fix_register.agent_sections[SectionName("3par_capacity")].parse_function
    assert list(check.discovery_function(parse_3par_capacity(section))) == expected_discovery_result
    ...
    [
        ...
        pytest.param(
            ...
            [
                ...
                Metric("fs_size", 100.0, boundaries=(0.0, None)),
                Result(state=State.WARN, summary="3 MB / 3.0% failed (warn/crit at 2.0%/5.0%)"),
            ],
        ...
        pytest.param(
            ...
            [
                ...
                Metric("fs_used_percent", 20.0, levels=(80.0, 90.0), boundaries=(0.0, 100.0)),
                Metric("fs_size", 100.0, boundaries=(0.0, None)),
                Result(state=State.CRIT, summary="6 MB / 6.0% failed (warn/crit at 2.0%/5.0%)"),
            ],
    ...
    assert (
        list(
            check.check_function(
                item=item,
                params=parameters,
                section=parse_3par_capacity(section),
            )
        )
