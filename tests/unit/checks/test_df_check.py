...
def test_df_discovery_with_parse(check_manager, monkeypatch, info, expected_result,
    ...
    assert checktestlib.DiscoveryResult(check.run_discovery(
        check.run_parse(info))) == expected_result
...
@pytest.mark.parametrize("item,params,info,expected_result", [
    (
        ...
        [(
            0,
            '75.8% used (103.92 of 137.13 GB), trend: 0.00 B / 24 hours',
            [
    ...
    (
        ...
        [(
            0,
            '21.1% used (4.23 of 20.00 GB), trend: 0.00 B / 24 hours',
            [
...
def test_df_check_with_parse(check_manager, item, params, info, expected_result):
    ...
    actual = checktestlib.CheckResult(check.run_check(item, params, check.run_parse(info)))
    expected = checktestlib.CheckResult(expected_result)
    checktestlib.assertCheckResultsEqual(actual, expected)
