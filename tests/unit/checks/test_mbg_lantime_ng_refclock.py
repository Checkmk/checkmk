...
@pytest.mark.parametrize("info,item,params,expected", [
    (meinberg_lantime_2, '1', (3, 3), CheckResult([
...
@pytest.mark.parametrize("info,item,params,expected", [
    (meinberg_lantime_1, '1', (3, 3), CheckResult([
        BasicCheckResult(1, 'Type: gps170, Usage: primary, State: not synchronized (GPS antenna disconnected)', None),
        BasicCheckResult(0, 'Next leap second: 2012-06-30', None),
        BasicCheckResult(2, 'Satellites: 0/12 (warn/crit below 3/3)', None)
    ])),
    (meinberg_lantime_5, '1', (3, 3), CheckResult([
