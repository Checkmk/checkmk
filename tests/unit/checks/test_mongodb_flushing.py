...
@pytest.mark.parametrize(
    ...
    [
        ([("average_ms", "1.28893335892"), ("last_ms", "0"),
          ("flushed", "36479")], 0, 'Average flush time over 60 minutes: 0.00 ms', [], 0,
         'Last flush time: 0.00 s', 'flush_time', 0.0),
        ([("average_ms", "5"), ("last_ms", "121"),
          ("flushed", "10000")], 0, 'Average flush time over 60 minutes: 0.00 ms', [], 1,
         'Last flush time: 0.12 s (warn/crit at 0.10 s/0.20 s)', 'flush_time', 0.121),
        ([("last_ms", "120"),
          ("flushed", "10000")], 3, 'missing data: average_ms', [], -1, '', '', -1.0),
        ([("average_ms", "5"),
          ("flushed", "10000")], 3, 'missing data: last_ms', [], -1, '', '', -1.0),
        ([("average_ms", "5"),
          ("last_ms", "120")], 3, 'missing data: flushed', [], -1, '', '', -1.0),
        ([("last_ms", "120")], 3, 'missing data: average_ms and flushed', [], -1, '', '',
         -1.0),
...
def test_check_function(check_manager, info, state_expected, info_expected, perf_expected,
    ...
    if len(check_result.subresults) == 1:
        ...
        check_result_flush_time(check_result.subresults[1], state_expected_flush,
                                info_expected_flush, perf_expected_flush_key,
                                perf_expected_flush_value)
...
def check_result_flush_time(result, state_expected_flush, info_expected_flush,
                            perf_expected_flush_key, perf_expected_flush_value):
