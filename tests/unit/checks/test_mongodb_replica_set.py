...
def check_plugin_fixutre() -> CheckPlugin:
    plugin = agent_based_register.get_check_plugin(CheckPluginName("mongodb_replica_set"))
    assert plugin
    return plugin


@pytest.fixture(name="section", scope="module")
def section_fixture() -> Mapping[str, Any]:
    plugin = agent_based_register.get_section_plugin(SectionName("mongodb_replica_set"))
    assert isinstance(plugin, AgentSectionPlugin)
    return plugin.parse_function(_STRING_TABLE)
