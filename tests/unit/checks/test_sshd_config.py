...
def _fixture_section_plugin() -> AgentSectionPlugin:
    plugin = agent_based_register.get_section_plugin(SectionName("sshd_config"))
    assert isinstance(plugin, AgentSectionPlugin)
    return plugin


@pytest.fixture(name="check_plugin")
def _fixture_check_plugin() -> CheckPlugin:
    plugin = agent_based_register.get_check_plugin(CheckPluginName("sshd_config"))
    assert plugin
    return plugin
...
def test_discovery(
    ...
    assert list(check_plugin.discovery_function(
        section_plugin.parse_function(string_table))) == [Service()]
        ...
        pytest.param(
            ...
            [
                ...
                Result(
                    state=State.CRIT,
                    summary=
                    "Use pluggable authentication module: not present in SSH daemon configuration",
                ),
            ],
        ),
        pytest.param(
            ...
            [
                Result(
                    state=State.CRIT,
                    summary=
                    "Allow keyboard-interactive/challenge-response authentication: not present in SSH daemon configuration",
                )
            ],
        ),
    ],
