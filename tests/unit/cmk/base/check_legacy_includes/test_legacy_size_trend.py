...
def test_size_trend_growing(monkeypatch: MonkeyPatch) -> None:
    ...
    state, infotext, perfdata = size_trend.size_trend(
        "somecheck", "someitem", "some resource", {
            "trend_range": 1,
            "trend_perfdata": True,
    ...
    assert state == 2
    assert infotext == ", trend: +100.00 MB / 1 hours - growing too fast (warn/crit at 50.00 MB/100.00 MB per 1.0 h)(!!)"\
                        ", growing too fast (warn/crit at 10.0%/20.0% per 1.0 h)(!)"\
                        ", only 9 hours until some resource full(!!)"
    assert perfdata == [('growth', 2400.0), ('trend', 2400.0, 1200.0, 2400.0, 0, 1000.0),
                        ('trend_hoursleft', 9.0)]
...
def test_size_trend_shrinking(monkeypatch: MonkeyPatch) -> None:
    ...
    assert state == 2
    assert infotext == ", trend: -100.00 MB / 1 hours - shrinking too fast (warn/crit at 50.00 MB/100.00 MB per 1.0 h)(!!)"\
