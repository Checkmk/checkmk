...
def test_size_trend_growing(monkeypatch: MonkeyPatch) -> None:
    ...
    assert (
        infotext
        == ", trend: +100.00 MB / 1 hours - growing too fast (warn/crit at 50.00 MB/100.00 MB per 1.0 h)(!!)"
...
def test_size_trend_shrinking(monkeypatch: MonkeyPatch) -> None:
    ...
    assert (
        infotext
        == ", trend: -100.00 MB / 1 hours - shrinking too fast (warn/crit at 50.00 MB/100.00 MB per 1.0 h)(!!)"
...
def test_size_trend_negative_free_space(monkeypatch: MonkeyPatch) -> None:
    ...
    assert (
        infotext
        == ", trend: +100.00 MB / 1 hours - growing too fast (warn/crit at 50.00 MB/100.00 MB per 1.0 h)(!!), "
        "growing too fast (warn/crit at 10.0%/20.0% per 1.0 h)(!), only 0 hours until some resource full(!!)"
    )
    assert perfdata == [
        ("growth", 2400.0),
        ("trend", 2400.0, 1200.0, 2400.0, 0, 1000.0),
