...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            [
                "--config",
                "{'connection': 'prometheus-server', 'verify-cert': True, 'auth_basic': "
                "('auth_token', {'token': ('password', 'token')}), 'protocol': 'http', "
                "'exporter': [('node_exporter', {'entities': ['df', 'diskstat', "
        ...
        pytest.param(
            ...
            [
                "--config",
                "{'connection': 'prometheus-server', 'verify-cert': True, 'auth_basic': "
                "('auth_login', {'username': 'user', 'password': ('password', "
                "'password')}), 'protocol': 'https', 'exporter': [('node_exporter', "
                "{'host_mapping': 'abc123', 'entities': ['df', 'diskstat', 'kernel']}), "
                "('cadvisor', {'entity_level': ('both', {'container_id': 'long', "
                "'prepend_namespaces': 'use_namespace'}), 'namespace_include_patterns': "
                "['p1', 'p2'], 'entities': ['diskio', 'cpu', 'df', 'if', 'memory']})], "
                "'promql_checks': [{'service_description': 'my-service', "
                "'metric_components': [{'metric_label': 'my-metric', 'promql_query': "
                "'my-query'}]}, {'service_description': 'my-second-service', 'host_name': "
