...
def test_inventory_checkpoint_tunnels() -> None:
    assert list(inventory_checkpoint_tunnels([["peer1", "3"], ["peer2", "131"]])) == [
        ("peer1", {}),
        ("peer2", {}),
    ]
...
def test_check_checkpoint_tunnels_known_status() -> None:
    assert check_checkpoint_tunnels("peer1", _DEFAULT_PARAMS, [["peer1", "3"]]) == (
        0,
        "Active",
    )
...
def test_check_checkpoint_tunnels_unknown_status() -> None:
    ...
    assert check_checkpoint_tunnels("DELAUTNWFWC01", _DEFAULT_PARAMS, section) == (
        3,
