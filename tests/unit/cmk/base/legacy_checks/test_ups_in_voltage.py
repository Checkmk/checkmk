...
def test_check_ups_in_voltage_ok() -> None:
    assert check_ups_in_voltage("1", {"levels_lower": (210.0, 180.0)}, [["1", "230"]]) == (
        0,
        "in voltage: 230V",
        [("in_voltage", 230, 210.0, 180.0, 150)],
    )


def test_check_ups_in_voltage_warn() -> None:
    assert check_ups_in_voltage("1", {"levels_lower": (240, 200)}, [["1", "230"]]) == (
        1,
        "in voltage: 230V, (warn/crit below 240V/200V)",
        [("in_voltage", 230, 240, 200, 150)],
    )


def test_check_ups_in_voltage_crit() -> None:
    assert check_ups_in_voltage("1", {"levels_lower": (250, 240)}, [["1", "230"]]) == (
        2,
        "in voltage: 230V, (warn/crit below 250V/240V)",
        [("in_voltage", 230, 250, 240, 150)],
    )


def test_check_ups_in_voltage_missing_item() -> None:
    assert check_ups_in_voltage("9", {"levels_lower": (210.0, 180.0)}, [["1", "230"]]) == (
        3,
