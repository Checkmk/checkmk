...
@pytest.mark.parametrize('string_table, item, result', [
    (DATA, "JL086A 1", [
...
@pytest.mark.parametrize('string_table, item, result', [
    ...
    (DATA, "JL086A 3", [
        ...
        Result(state=State.WARN, summary='Temperature: 51.0°C (warn/crit at 50.0°C/60.0°C)'),
        Result(state=State.OK, notice='Configuration: only use user levels'),
    ]),
    (DATA, "JL086A 4", [
        Metric('temp', 61.0, levels=(50.0, 60.0)),
        Result(state=State.CRIT, summary='Temperature: 61.0°C (warn/crit at 50.0°C/60.0°C)'),
        Result(state=State.OK, notice='Configuration: only use user levels'),
    ]),
])
