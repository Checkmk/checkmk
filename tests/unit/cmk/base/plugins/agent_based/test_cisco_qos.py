...
def test_discover_cisco_qos(fix_register: FixRegister) -> None:
    assert list(
        fix_register.check_plugins[CheckPluginName("cisco_qos")].discovery_function(_STRING_TABLE)
    ) == [
    ...
    assert (
        list(
            fix_register.check_plugins[CheckPluginName("cisco_qos")].check_function(
                ...
                params=params,
                section=_STRING_TABLE_ZERO_SPEED,
            )
        )
        == expected_result
    )
