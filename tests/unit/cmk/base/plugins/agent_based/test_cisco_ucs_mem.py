...
def test_inventory_cisco_ucs_mem() -> None:
    assert list(inventory_cisco_ucs_mem(STRING_TABLE)) == [
        ("mem-1", None),
        ("mem-2", None),
        ("mem-3", None),
        ("mem-4", None),
...
def test_check_cisco_ucs_mem(item: str, expected_output: Sequence[object]) -> None:
    assert list(check_cisco_ucs_mem(item, None, STRING_TABLE)) == expected_output
