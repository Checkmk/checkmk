...
def test_fileinfo_min_max_age_levels():
    ...
    # minage matches
    output_minage = fileinfo_plugin.check_fileinfo(item, {
        'minage': (5, 1),
    }, parsed)
    ...
    # maxage matches
    output_maxage = fileinfo_plugin.check_fileinfo(item, {
        'maxage': (1, 2),
    }, parsed)
    ...
    # generic check_levels function.
    output_both = fileinfo_plugin.check_fileinfo(item, {
        'minage': (5, 1),
        'maxage': (1, 2),
    }, parsed)
...
@pytest.mark.parametrize('info, params, expected_result', [
    (
        [
            ['1536557964'],
    ...
    (
        ...
        {
            "group_patterns": [("log", ("*syslog*", "")), ("today", ("/tmp/$DATE:%Y%m%d$.txt", ""))]
        },
        [
    ...
    (
        [
            ...
            [u'1611065406'],
            [
                u'/SAP HANA PHT 00/daemon_wwfcsunil286s.dc.ege.ds.30000.336.trc', u'11072435',
                u'1609376403'
            ],
            [u'/SAP HANA PHT 00/backup.log.3.gz', u'425', u'1609773942'],
            [u'/SAP HANA PHT 00/daemon_wwfcsunil286s_065255__children.trc', u'1923', u'1607339609'],
...
@pytest.mark.parametrize('info, item, params, expected_result', [
    (
        [
            ['1536557964'],
            ['regular.txt', '4242', '1536421281'],
...
@pytest.mark.parametrize('info, params, expected_result', [
    (
        ...
        [
            ...
            Service(item='log', parameters={'group_patterns': [('*syslog*', '')]}),
            Service(item='today', parameters={'group_patterns': [('/tmp/$DATE:%Y%m%d$.txt', '')]}),
        ],
    ),
])
...
@pytest.mark.parametrize('info, item, params, expected_result', [
    (
        ...
        [
            ...
            Result(state=State.OK, notice="[/var/log/syslog] Age: 4 seconds, Size: 1,307,632 B"),
            Result(state=State.OK,
                   notice="[/var/log/syslog.1] Age: 7 hours 59 minutes, Size: 1,235,157 B"),
            Result(state=State.OK, summary="Count: 2"),
            Metric("count", 2),
            Result(state=State.CRIT, summary="Size: 2,542,789 B (warn/crit at 2 B/2,097,152 B)"),
            ...
            Result(
                state=State.CRIT,
                summary=
                "Oldest age: 7 hours 59 minutes (warn/crit at 1 hour 0 minutes/5 hours 0 minutes)"),
    ...
    (
        [
            ...
            [u"name", u"status", u"size", u"time"],
            [u"[[[content]]]"],
            [u"/var/log/syslog", u"ok", u"1307632", u""],
            [u"/var/log/syslog.1", u"ok", u"1235157", u"1563259976"],
            [u"/var/log/aptitude", u"ok", u"0", u"1543826115"],
            [u"/var/log/aptitude.2.gz", u"ok", u"3234", u"1539086721"],
            [u"/tmp/20190716.txt", u"ok", u"1235157", u"1563259976"],
