...
def test_check_heartbeat_crm_resources_simple(section_3: Section) -> None:
    assert list(check_heartbeat_crm_resources("grp_omd", {"expected_node": None}, section_3,)) == [
...
def test_check_heartbeat_crm_resources_only() -> None:
    ...
    assert list(_check_heartbeat_crm_resources(resources["checkmk"], {"expected_node": None},)) == [
        Result(
            state=State.OK,
