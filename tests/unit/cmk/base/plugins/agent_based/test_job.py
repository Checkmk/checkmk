...
def test__job_parse_real_time(timestr, expected_result):
...
def test__process_start_time(warn, crit, result):
    assert job._process_start_time(1547301201, warn, crit) == result


def test__normal_result():
    summary = 'summary'
...
@freeze_time(datetime.datetime(2020, 7, 9, 13, 17, 00, tzinfo=pytz.timezone('UTC')))
@pytest.mark.parametrize(
    "item, params, section, expected_results",
    [
        (
