...
from cmk.base.plugins.agent_based import k8s_cpu_load
...
def check_plugin(fix_register):
    for name, plugin in fix_register.check_plugins.items():
        if str(name) == "k8s_cpu_resources":
            return plugin
    assert False, "Should be able to find the plugin"


def test_register_agent_section_calls(agent_section):
    assert str(agent_section.name) == "k8s_cpu_resources_v1"
    assert str(agent_section.parsed_section_name) == "k8s_cpu_resources"
    assert agent_section.parse_function == k8s_cpu_load.parse


def test_register_check_plugin_calls(check_plugin):
    assert str(check_plugin.name) == "k8s_cpu_resources"
    assert check_plugin.service_name == "CPU Load"
    assert check_plugin.discovery_function.__wrapped__ == k8s_cpu_load.discovery
    assert check_plugin.check_function.__wrapped__ == k8s_cpu_load.check
    assert check_plugin.check_default_parameters == {}


def test_parse(string_table, limit, requests):
    section = k8s_cpu_load.parse(string_table)
    assert section.limit == limit
    assert section.requests == requests


def test_discovery_returns_an_iterable(string_table):
    parsed = k8s_cpu_load.parse(string_table)
    assert list(k8s_cpu_load.discovery(parsed))


@pytest.fixture
def check_result(section):
    return k8s_cpu_load.check({}, section)
