...
def test_check_k8s_node_count_default_params() -> None:
    ...
    assert result == [
        Result(state=State.CRIT, summary="Number of worker nodes: 0 (warn/crit below 3/1)"),
        Metric("k8s_node_count_worker", 0.0, boundaries=(0.0, None)),
        Result(state=State.CRIT, summary="Number of control plane nodes: 0 (warn/crit below 3/1)"),
