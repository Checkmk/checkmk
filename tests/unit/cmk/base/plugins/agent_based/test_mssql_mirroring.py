...
@pytest.mark.parametrize('string_table, parsed_config', [
    (
        [
            ['MBEAN'],
            [
...
@pytest.mark.parametrize('string_table, discovered_services', [
    (
        [
            ['MBEAN'],
            [
...
def test_discover_mssql_mirroring(string_table, discovered_services):
    assert list(discover_mssql_mirroring(
        parse_mssql_mirroring(string_table))) == discovered_services


@pytest.mark.parametrize('item, params, section, check_result', [
    (
        'TRAVELER',
        {
        ...
        [
            ...
            Result(state=State.OK, notice='Mirroring partner name: TCP://server-guatemala.net:22'),
            Result(state=State.OK, notice='Mirroring witness name: TCP://server-vientam.net:22'),
        ],
    ),
    (
        'FARMER',
