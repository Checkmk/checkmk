...
SECTION: Section = {
    "ipv6.google.com": MtrResult(hops=[]),
    "mathias-kettner.de": MtrResult(
        hops=[
            Hop(
                name="1.2.3.4",
                pl=0.0,
    ...
    "broken.example.com": MtrResult(
        hops=[],
        error="mtr: Failed to resolve host: broken.example.com: Name or service not known",
    ),
    "stale.example.com": MtrResult(
        hops=[
            Hop(
                name="1.2.3.4",
                pl=0.0,
        ...
        error="mtr: Unable to get raw sockets.",
    ),
}
...
def test_discover_mtr() -> None:
    assert list(discover_mtr(SECTION)) == [
        ...
        Service(item="broken.example.com"),
        Service(item="stale.example.com"),
    ]
        ...
        pytest.param(
            ...
            [
                Result(
                    state=State.UNKNOWN,
                    summary=(
                        "mtr failed: mtr: Failed to resolve host: broken.example.com: "
                        "Name or service not known"
                    ),
                ),
            ],
            id="no hops, but mtr said why",
        ),
        pytest.param(
            "stale.example.com",
            {
            ...
            [
                Result(
                    state=State.OK,
                    notice=(
                        "Data is from the last successful run, the latest one failed: "
                        "mtr: Unable to get raw sockets."
                    ),
                ),
                Result(
                    state=State.OK,
                    summary="Number of Hops: 1",
                    details="Hop 1: 1.2.3.4",
                ),
                Metric("hops", 1.0),
                Result(state=State.OK, summary="Packet loss: 0%"),
                Metric("hop_1_pl", 0.0, levels=(10.0, 25.0)),
                Result(state=State.OK, summary="Round trip average: 2 milliseconds"),
                Metric("hop_1_rta", 0.0022, levels=(0.15, 0.25)),
                Result(state=State.OK, summary="Standard deviation: 2 milliseconds"),
                Metric("hop_1_rtstddev", 0.0016, levels=(0.15, 0.25)),
                Metric("hop_1_rtmin", 0.0012),
                Metric("hop_1_rtmax", 0.007),
                Metric("hop_1_response_time", 0.0013),
            ],
            id="stale hops plus the reason why they are stale",
        ),
    ],
)
def test_check_mtr(
