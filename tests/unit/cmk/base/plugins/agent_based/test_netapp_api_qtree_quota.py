...
def test_get_item_names(qtree: Qtree, expected_result: tuple[str, str]) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            "QUOTA-10.user",
            [
                Metric("fs_used", 0.0, levels=(4.8, 5.4)),
                Metric("fs_free", 6.0, boundaries=(0.0, None)),
                Metric("fs_used_percent", 0.0, levels=(80.0, 90.0), boundaries=(0.0, 100.0)),
                Result(state=State.OK, summary="Used: 0% - 0 B of 6.00 MiB"),
                Metric("fs_size", 6.0, boundaries=(0.0, None)),
                Metric("growth", 0.0),
