...
@pytest.mark.parametrize("string_table,expected_parsed_data", [
    ([['[heute]'], ['cmc', '0'], ['apache', '0'], ['OVERALL', '0'], ['[stable]'], ['cmc', '0'],
      ['apache', '0'], ['OVERALL', '0']], {
          "heute": {
              'stopped': [],
              'existing': ['cmc', 'apache'],
...
def test_parse_omd_status(string_table, expected_parsed_data):
    assert parse_omd_status(string_table) == expected_parsed_data


@pytest.mark.parametrize("item,section_omd_status,section_omd_info,result", [
    ("stable", {
        'heute': {
            'stopped': [],
...
def test_check_omd_status(item, section_omd_status, section_omd_info, result):
