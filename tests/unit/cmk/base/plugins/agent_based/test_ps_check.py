...
PS_DISCOVERED_ITEMS = [
    ...
    Service(item="firefox is on fire", parameters={
        "process": "~.*(fire)fox",
        "process_info": "text",
        "user": None,
        'cpu_rescale_max': 'cpu_rescale_max_unspecified',
    ...
    Service(item="Checkhelpers heute", parameters={
    ...
    Service(item="Checkhelpers Overall", parameters={
        "process": "~/omd/sites/\\w+/lib/cmc/checkhelper",
        "process_info": "text",
        "user": None,
        'match_groups': (),
    ...
    Service(item="svchost", parameters={
        "cpulevels": (90.0, 98.0),
        "handle_count": (1000, 2000),
        "levels": (1, 1, 99999, 99999),
        "max_age": (3600, 7200),
...
cpu_config = namedtuple(
    "cpu_config",
    "name agent_info cputime cpu_cores exp_load cpu_rescale_max",
)
cpu_util_data = [
...
def test_parse_ps_windows(mocker: MockerFixture):
    section_ps = ps_section.parse_ps(splitter(
    ...
    service = next(iter(ps_utils.discover_ps(params=[{"descr": "CPUSTRES64", "match": "CPUSTRES64.EXE", "default_params": {}}, {}], section_ps=section_ps, section_mem=section_mem, section_mem_used=section_mem_used, section_cpu=section_cpu)))
    ...
    with value_store.context(CheckPluginName("ps"), "unit-test"):
        ...
        results = list(ps_check.check_ps(item=item, params=params, section_ps=section_ps, section_mem=section_mem, section_mem_used=section_mem_used, section_cpu=section_cpu))
        single_process_result = next(r for r in results if isinstance(r, Result) and "PID 2080" in r.summary)
        assert single_process_result == Result(state=state.CRIT, summary="CPUSTRES64.EXE with PID 2080 CPU: 20.00% (warn/crit at 0%/0%)")
