...
def test_parse_site_object_counts():
    assert site_object_counts.parse_site_object_counts([
        ['[[[heute]]]'],
        ['Tags', 'snmp', '1'],
...
def test_check_site_object_counts():
    assert list(site_object_counts.check_site_object_counts(SECTION)) == [
        ...
        Metric('service_check_commands_hr_cpu', 1.0, levels=(None, None), boundaries=(None, None)),
        Metric('tags_snmp', 3.0, levels=(None, None), boundaries=(None, None)),
        Metric('tags_prod', 2.0, levels=(None, None), boundaries=(None, None)),
        Result(
            state=state.OK,
...
def test_cluster_check_site_object_counts():
    assert list(
        ...
        Metric('service_check_commands_omd_apache',
               ...
               levels=(None, None),
               boundaries=(None, None)),
        Metric('service_check_commands_hr_cpu', 4.0, levels=(None, None), boundaries=(None, None)),
        Metric('tags_snmp', 3.0, levels=(None, None), boundaries=(None, None)),
        Metric('tags_prod', 4.0, levels=(None, None), boundaries=(None, None)),
        Metric('tags_tcp', 5.0, levels=(None, None), boundaries=(None, None)),
        Result(
            state=state.OK,
            summary=
            'Service Check Commands: 5 lnx_if, 4 omd_apache, 4 hr_cpu, Tags: 3 snmp, 4 prod, 5 tcp',
            details=
            'Service Check Commands: 5 lnx_if, 4 omd_apache, 4 hr_cpu, Tags: 3 snmp, 4 prod, 5 tcp'
        ),
    ]
