...
SECTION_HASHES: veritas_vcs.Section = {
    ...
    'cluster': {
        'c7dbacpt': [Vcs(attr='ClusState', value='RUNNING', cluster=None)]
    },
    'group': {
        'lan': [
...
@pytest.mark.parametrize('states, expected_state', [
    (
        ['a'],
        'a',
    ),
...
def test_check_veritas_vcs_group():
    assert list(veritas_vcs.check_veritas_vcs_group(
        'nepharius',
        PARAMS,
        SECTION_HASHES,
    )) == [
...
def test_check_veritas_vcs_resource():
    assert list(veritas_vcs.check_veritas_vcs_resource(
        'bob3-dg',
        PARAMS,
        SECTION,
    )) == [
