...
@pytest.mark.parametrize("info, section", [
    (ZERO_INSTANCES_INFO, {}),
    (INFO, SECTION),
...
def test_discovery(section, services):
    assert list(
        winperf_msx_queues.discover_winperf_msx_queues(
            winperf_msx_queues._DEFAULT_DISCOVERY_PARAMETERS, section)) == services
...
def test_check(item, params, result):
    assert list(
        winperf_msx_queues.check_winperf_msx_queues(item, params,
                                                    EXCHANGE_2013_SERVER_SECTION)) == result
