...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [Plugin(base_os=OS.LINUX, source=Path("mk_suseconnect"), interval=None)],
        ),
        (
            {"deployment": ("cached", 3600.0)},
            [Plugin(base_os=OS.LINUX, source=Path("mk_suseconnect"), interval=3600)],
        ),
        (
