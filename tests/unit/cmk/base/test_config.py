...
def test_discovery_check_parameters(
    ...
    ts = Scenario()
    ts.add_host(hostname)
    ts.set_option("use_new_descriptions_for", {"cmk_inventory": True})
    ts.set_option(
        "periodic_discovery",
        [
            {
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("cisco_mem_asa64", "Mem used test_item", id="previously configured"),
    ],
)
def test_correct_description_used_for_plugins_not_configured_via__use_new_descriptions_for(
    monkeypatch: MonkeyPatch, plugin_name: str, expected_descr: str
) -> None:
    test_host = HostAddress("test_host")
    plugins = {
        CheckPluginName(plugin_name): _make_check_plugin(plugin_name, "New service description %s")
    }
    monkeypatch.setattr(agent_based_register._config, "registered_check_plugins", plugins)

    ts = Scenario()
    ts.add_host(test_host)
    ts.set_option("use_new_descriptions_for", {})
    config_cache = ts.apply(monkeypatch)
