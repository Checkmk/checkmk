...
import cmk.base.data_sources.snmp
import cmk.base.data_sources.tcp
import cmk.base.ip_lookup as ip_lookup
import cmk.base.config as config
import cmk.base.exceptions


def test_data_source_cache_default(monkeypatch):
    Scenario().add_host("hostname").apply(monkeypatch)
    source = cmk.base.data_sources.snmp.SNMPDataSource("hostname", "ipaddress")
...
def test_mgmt_board_data_source_is_ip_address():
    _is_ipaddress = cmk.base.data_sources.abstract.ManagementBoardDataSource._is_ipaddress
...
def test_mgmt_board_data_source_management_board_ipaddress(monkeypatch, result, address,
    ...
    if resolvable:
        ...
        def raise_exc(h):
            raise cmk.base.exceptions.MKIPAddressLookupError("Failed to...")
...
def test_normalize_ip(ip_in, ips_out):
    assert cmk.base.data_sources.abstract._normalize_ip_addresses(ip_in) == ips_out
...
def test_tcpdatasource_only_from(monkeypatch, result, reported, rule):
    ...
    source = cmk.base.data_sources.tcp.TCPDataSource("hostname", "ipaddress")
...
@pytest.mark.parametrize("info_func_result,expected", [
    ...
    (
        cmk.base.data_sources.programs.SpecialAgentConfiguration("arg0", None),
        ("arg0", None),
    ),
    (
        cmk.base.data_sources.programs.SpecialAgentConfiguration("arg0 arg1", None),
        ("arg0 arg1", None),
    ),
    (
        cmk.base.data_sources.programs.SpecialAgentConfiguration(["list0", "list1"], None),
        ("'list0' 'list1'", None),
    ),
    (
        cmk.base.data_sources.programs.SpecialAgentConfiguration("arg0 arg1", "stdin_blob"),
        ("arg0 arg1", "stdin_blob"),
    ),
    (
        cmk.base.data_sources.programs.SpecialAgentConfiguration(["list0", "list1"], "stdin_blob"),
...
def test_get_command_line_and_stdin(monkeypatch, info_func_result, expected):
    ...
    ds = cmk.base.data_sources.programs.SpecialAgentDataSource("testhost", "127.0.0.1",
...
def test_data_sources_of_hosts(monkeypatch, hostname, settings):
    ...
    sources = cmk.base.data_sources.DataSources(hostname, "127.0.0.1")
