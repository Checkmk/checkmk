...
class TestHostSections:
    ...
    @pytest.fixture
    def identical_host_sections(self):
        return self._get_some_section()

    def _get_some_section(self):
    ...
    def test_add_self_extends_sections(self, host_sections, identical_host_sections):
        ...
        for section in host_sections.sections:
            assert result.sections[section] == 2 * identical_host_sections.sections[section]
        ...
        for host_name in host_sections.piggybacked_raw_data:
            assert (
                ...
                == 2 * identical_host_sections.piggybacked_raw_data[host_name]
            )
