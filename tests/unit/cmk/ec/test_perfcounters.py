...
def test_perfcounters_column_default_values():
    ...
    for column_name, default_value in c.status_columns():
        if column_name.startswith("status_average_") and column_name.endswith("_time"):
            ...
            assert type(default_value) == int, "Wrong column type %r: %s" % (column_name, type(default_value))
            assert default_value == 0, "Wrong column default value %r: %d" % (column_name, default_value)
...
def test_perfcounters_correct_status_values():
    ...
    for column_name, column_value in zip([ n for n, d in c.status_columns() ], c.get_status()):
        if column_name.startswith("status_average_") and column_name.endswith("_time"):
            ...
            assert column_value == c._counters[counter_name], "Invalid value %r: %r" % (column_name, c._counters[counter_name])
