...
def _create_dashboard_payload(
    ...
    icon_config: dict[str, object] | None = None,
) -> dict[str, object]:
...
class TestDashboardIcon:
    @pytest.mark.parametrize(
        "icon_config",
        [
            pytest.param({"name": "action_menu"}, id="name_only"),
            pytest.param(
                {"name": "action_menu", "emblem": "agent_deployment"}, id="name_with_emblem"
            ),
        ],
    )
    def test_create_with_icon(
        self, clients: ClientRegistry, icon_config: dict[str, object]
    ) -> None:
        resp = clients.DashboardClient.create(
