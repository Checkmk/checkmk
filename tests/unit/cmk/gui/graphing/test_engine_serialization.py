...
from cmk.gui.graphing._engine_serialization import (
    deserialize_resolved_graphs,
    serialize_resolved_graphs,
)
...
def _rich_graphs() -> Sequence[ResolvedGraph]:
...
def test_template_round_trip_is_lossless() -> None:
    resolved = _rich_graphs()
    payload = serialize_resolved_graphs(resolved)
    ...
    assert serialize_resolved_graphs(deserialize_resolved_graphs(payload)) == payload
