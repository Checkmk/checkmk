...
from cmk.gui.graphing._template_graphs_via_engine import (
    ...
    resolve_template_graphs,
    update_template_graph_via_engine,
)
...
def test_template_lifecycle_discover_and_update() -> None:
    ...
    rrd = _FakeRRD()
    resolved = resolve_template_graphs(service=_SERVICE, rrd=rrd)
    [fallback] = [graph for graph in resolved if graph.name == _METRIC]
    assert [
    ...
    evaluated = update_template_graph_via_engine(
        resolved=resolved,
        rrd=rrd,
    ...
    assert len(evaluated) == len(resolved)
