...
def test_page_renders_registered_client_details(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with output_funnel.plugged():
        ModeRegisteredOAuthClients().page(Config())
...
@pytest.mark.usefixtures("request_context")
def test_page_renders_empty_table_without_error() -> None:
    with output_funnel.plugged():
        ModeRegisteredOAuthClients().page(Config())
        written = "".join(output_funnel.drain())
...
class TestModeRegisteredOAuthClientsAction:
    def test_deletes_single_client(self, flask_app: Flask) -> None:
        with get_client_store() as store:
        ...
        with flask_app.test_request_context(
            ...
            ModeRegisteredOAuthClients().action(Config())

        with get_client_store() as store:
            assert store.get(registered.ok.client_id) is None

    def test_bulk_deletes_checked_clients_only(self, flask_app: Flask) -> None:
        with get_client_store() as store:
    ...
    def test_does_not_delete_when_transaction_is_invalid(self, flask_app: Flask) -> None:
        with get_client_store() as store:
            registered = store.register(["https://client.example/callback"], "Example")
        assert registered.is_ok()
