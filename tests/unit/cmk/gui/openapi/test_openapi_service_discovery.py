...
def test_openapi_update_service_phase_refuses_without_manage_services_or_edit(
    ...
    no longer disable a service through this endpoint -- it is refused and writes nothing, exactly
    as the read-only endpoints of the family already were (they demand both in their handlers, see
    `RO_PERMISSIONS`). Before the fix this endpoint alone demanded neither, so which permission a
    client needed depended on which endpoint it happened to use.

    The refusal is a `401` here: the remap of a failed permission check to `403` (werk 18241) is
    2.5.0 and newer, on 2.4.x `MKAuthException.status` still reaches the response unchanged.
    """
