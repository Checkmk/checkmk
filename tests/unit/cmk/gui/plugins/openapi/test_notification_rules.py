...
from cmk.utils.notify_types import CustomPluginName
...
def test_update_notification_with_invalid_custom_plugin(clients: ClientRegistry) -> None:
    config = notification_rule_request_example()
    resp1 = clients.RuleNotification.create(rule_config=config)

    custom_plugin_name = CustomPluginName("plugin_a")
    config["notification_method"]["notify_plugin"] = {
        "option": "create_notification_with_the_following_custom_parameters",
        "plugin_params": {
    ...
    resp2 = clients.RuleNotification.edit(
        rule_id=resp1.json["id"],
        rule_config=config,
        expect_ok=False,
    )
    resp2.assert_status_code(400)
    assert resp2.json["title"] == "Plugin doesn't exist"
    assert resp2.json["detail"] == "The plugin 'plugin_a' is not a valid notification plugin."
