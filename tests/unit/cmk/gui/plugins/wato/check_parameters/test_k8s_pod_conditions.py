...
from cmk.gui.plugins.wato.check_parameters import k8s_pod_conditions
...
def test_parameter_valuespec_returns_a_dictionary():
    parameters = k8s_pod_conditions._parameter_valuespec()
...
@pytest.fixture
def rulespec():
    for r in k8s_pod_conditions.rulespec_registry.get_by_group("static/applications"):
        if r.name == "static_checks:k8s_pod_conditions":
            return r
...
def test_rulespec_registry_parameter_valuespec(rulespec):
    assert rulespec._parameter_valuespec == k8s_pod_conditions._parameter_valuespec
