...
def test_page_download_product_usage_register() -> None:
    ...
    page_class = page_registry.get("download_product_usage")
    assert page_class is not None
    assert page_class == PageDownloadProductUsage


@pytest.mark.usefixtures("request_context")
def test_page_download_product_usage_success(
    mocker: MockerFixture,
    mock_product_usage_data: PageDownloadProductUsage,
) -> None:
...
@pytest.mark.usefixtures("request_context")
def test_page_download_product_usage_permission_denied(mocker: MockerFixture) -> None:
    mock_user = mocker.patch("cmk.gui.product_usage_analytics.download.user")
