...
def test_form_spec_recap() -> None:
    ...
    form_data: ParsedFormData = {FormSpecId("wrapper"): {"test_dict_element": "I am a test string"}}
    recap = list(recaps_form_spec(form_data, build_expected_formspec_map(setup_stages)))

    assert len(recap) == 1
    assert isinstance(recap[0], FormSpecRecap)
    assert recap[0].form_spec.data == form_data[FormSpecId("wrapper")]  # type: ignore[attr-defined]
