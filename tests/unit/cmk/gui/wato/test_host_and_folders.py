...
class TestFolderSiteStats:
    ...
    def _setup_fs(self, folder: Folder) -> None:
        folder.create_hosts([(HostName("main_host"), {}, None)])
        sub = folder.create_subfolder(name="sub", title="", attributes={})
        sub.create_hosts([(HostName("sub_host"), {}, None)])
        nest = sub.create_subfolder(name="sub/nested", title="", attributes={})
        nest.create_hosts([(HostName("nested_host"), {}, None)])
