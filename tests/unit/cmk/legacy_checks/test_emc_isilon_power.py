...
def test_upper_levels_warn() -> None:
    ...
    ) == [Result(state=State.WARN, summary="245.0 V (warn/crit at or above 240.0/260.0 V)")]
...
def test_upper_levels_crit() -> None:
    ...
    ) == [Result(state=State.CRIT, summary="265.0 V (warn/crit at or above 240.0/260.0 V)")]
...
def test_lower_crit_takes_precedence_over_upper() -> None:
    ...
    ) == [Result(state=State.CRIT, summary="150.0 V (warn/crit below 200.0/180.0 V)")]
...
def test_lower_warn_triggers_with_only_lower_configured() -> None:
    ...
    ) == [Result(state=State.WARN, summary="190.0 V (warn/crit below 200.0/180.0 V)")]
