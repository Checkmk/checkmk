...
from cmk.agent_based.v2 import GetRateError, StringTable
...
def test_check_sophos_messages_reports_from_third_cycle_on() -> None:
    ...
    assert _check_sophos_messages("Legit", {}, _section(152, 14), value_store, NOW + 120) == (
        0,
        "1.1 Inbounds and Outbounds/s, 1.0 Inbounds/s, 0.1 Outbounds/s",
        [("messages_inbound", 1.0), ("messages_outbound", 0.1)],
    )
...
def test_check_sophos_messages_item_is_renamed() -> None:
    ...
    assert _check_sophos_messages(
        "Invalid Recipient", {}, _section(152, 14), value_store, NOW + 120
    ) == (
        0,
        "1.1 Inbounds and Outbounds/s, 1.0 Inbounds/s, 0.1 Outbounds/s",
        [("messages_inbound", 1.0), ("messages_outbound", 0.1)],
...
def test_check_sophos_messages_unknown_item() -> None:
    assert _check_sophos_messages("Not In Section", {}, _section(92, 8), {}, NOW) is None
