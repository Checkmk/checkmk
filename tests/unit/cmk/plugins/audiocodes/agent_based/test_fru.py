...
_STRING_TABLE = [
    ...
    [["67387393", "4", "7"], ["67907585", "4", "7"]],
]


def test_discovery_function() -> None:
    section = parse_audiocodes_fru(_STRING_TABLE)
    assert section is not None
    assert list(discover_audiocodes_fru(section)) == [
...
def test_check_function(
    ...
    section = parse_audiocodes_fru(string_table)
    assert section
    assert list(check_audiocodes_fru(item, section)) == expected
