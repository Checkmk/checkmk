...
from cmk.plugins.aws.agent_based.aws_dynamodb_table_write_capacity import (
    check_aws_dynamodb_write_capacity,
    discover_aws_dynamodb_table_write_capacity,
)

test_section = {
    "Sum_ConsumedWriteCapacityUnits": 50.0,
    "provisioned_WriteCapacityUnits": 100.0,
    "Minimum_ConsumedWriteCapacityUnits": 15.0,
    "Maximum_ConsumedWriteCapacityUnits": 90.0,
}
...
def test_discover_aws_dynamodb_table_write_capacity():
    inventory = list(discover_aws_dynamodb_table_write_capacity(test_section))
...
def test_check_aws_dynamodb_write_capacity():
    results = list(check_aws_dynamodb_write_capacity(test_params, test_section))
    ...
    assert (
        len(
            [
                ...
                and (res.name.endswith("_wcu") or res.name.endswith("_perc"))
            ]
        )
        == 4
    )
    ...
    assert " WCU" in results[0].summary
    ...
    assert " WCU" in results[4].summary
    ...
    assert " WCU" in results[6].summary


def test_check_aws_dynamodb_latency_no_data():
    with pytest.raises(IgnoreResultsError):
        list(check_aws_dynamodb_write_capacity(test_params, {}))
