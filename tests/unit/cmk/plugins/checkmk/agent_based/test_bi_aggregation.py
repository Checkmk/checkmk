...
def test_check_bi_aggregation() -> None:
    assert list(check_bi_aggregation("Host test", TEST_INFO)) == [
        ...
        Result(
            state=State.OK,
            notice="Error during aggregation",
            details="Host test (!) [affects state]",
        ),
        Result(
            state=State.OK,
            notice="Error during aggregation",
            details="+-- General State (!) [affects state]",
        ),
        Result(
            state=State.OK,
            notice="Error during aggregation",
            details="| +-- Check_MK (!) [affects state]",
        ),
        Result(
            state=State.OK,
            notice="Error during aggregation",
            details="| | +-- Check_MK Discovery, Services unmonitored: 1 (cpu_loads: 1)(!), Host labels: all up to date (!) [affects state]",
        ),
        Result(
            state=State.OK,
            notice="Error during aggregation",
