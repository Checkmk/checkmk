...
def test_discover_checkpoint_powersupply_filters_indices(
    section: StringTable, expected: list[Service]
) -> None:
    assert list(discover_checkpoint_powersupply(parse_checkpoint_powersupply(section))) == expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            DEFAULT_PARAMS,
            [["1", "No Redundancy"]],
            Result(state=State.WARN, summary="No Redundancy"),
        ),
