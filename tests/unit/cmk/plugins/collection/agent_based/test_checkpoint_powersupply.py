...
from cmk.plugins.collection.agent_based.checkpoint_powersupply import (
    ...
    inventory_checkpoint_powersupply,
...
def test_inventory_checkpoint_powersupply() -> None:
    section = parse_checkpoint_powersupply(STRING_TABLE)
    assert list(inventory_checkpoint_powersupply(section)) == [
        Service(item="1"),
        Service(item="2"),
        Service(item="3"),
    ]
...
def test_inventory_checkpoint_powersupply_empty() -> None:
...
def test_check_checkpoint_powersupply_item_not_found() -> None:
    ...
    assert not list(check_checkpoint_powersupply("99", DEFAULT_PARAMS, STRING_TABLE))
