...
def test_inventory_domino_info_empty() -> None:
    assert list(inventory_domino_info([])) == []


def test_inventory_domino_info() -> None:
    section: StringTable = [
        ["1", "MEDEMA", "CN=HH-BK4/OU=SRV/O=MEDEMA/C=DE", "Release 8.5.3FP5 HF89"]
    ]
    assert list(inventory_domino_info(section)) == [Service()]
