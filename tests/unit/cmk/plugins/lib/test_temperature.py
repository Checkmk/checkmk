...
def test_check_trend_time_period_ok() -> None:
    ...
    assert results == [
        ...
        Result(
            state=State.OK,
            summary="Time until temperature limit reached: 6 minutes 0 seconds",
        ),
    ]
...
def test_check_temperature_user_levels_warn_upper() -> None:
    ...
    assert results == [
        ...
        Result(
            state=State.WARN,
            summary="Temperature: 23.0 °C (warn/crit at 23.0 °C/30.0 °C)",
        ),
        Result(
            state=State.OK,
            notice="Configuration: prefer user levels over device levels (used user levels)",
...
def test_check_temperature_user_levels_warn_lower() -> None:
    ...
    assert results == [
        ...
        Result(
            state=State.WARN,
            summary="Temperature: -1.0 °C (warn/crit below 0.0 °C/-15.0 °C)",
        ),
        Result(
            state=State.OK,
            notice="Configuration: prefer user levels over device levels (used user levels)",
...
def test_check_temperature_device_levels_warn_upper() -> None:
    ...
    assert results == [
        ...
        Result(
            state=State.WARN,
            summary="Temperature: 10.0 °C (warn/crit at 10.0 °C/15.0 °C)",
        ),
        Result(
            state=State.OK,
            notice="Configuration: prefer user levels over device levels (used device levels)",
...
def test_check_temperature_device_levels_warn_lower() -> None:
    ...
    assert results == [
        ...
        Result(
            state=State.WARN,
            summary="Temperature: 0.0 °C (warn/crit below 1.0 °C/-15.0 °C)",
        ),
        Result(
            state=State.OK,
            notice="Configuration: prefer user levels over device levels (used device levels)",
