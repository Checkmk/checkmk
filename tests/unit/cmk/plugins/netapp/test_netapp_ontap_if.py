...
def test_missing_broadcast_domain_sets_flag(failover_policy: str) -> None:
    ...
    (_, extra_data) = _merge_if_counters_sections(
        {iface_no_bd.name: iface_no_bd},
        {port_no_bd.item_name(): port_no_bd},
        None,
    )
...
def test_sfo_partners_only_failover_ports() -> None:
    ...
    (_, extra_data) = _merge_if_counters_sections(
        ...
        {port.item_name(): port for port in ports},
        None,
    )

    # node1 (home) and node2 (HA partner) are eligible; node3 is excluded.
    assert sorted(
        (fop["node"], fop["port"]) for fop in extra_data["lif_sfo"]["failover_ports"]
    ) == [
        ("node1", "port1"),
        ("node2", "port2"),
    ]
