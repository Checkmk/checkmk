...
def test_check_power_consumption_with_metrics() -> None:
    parsed = parse_redfish_multiple(_make_string_table(_make_power_entry()))
    results = list(check_redfish_power_consumption("0", parsed))
    assert Result(state=State.OK, summary="Current power: 421 W") in results
...
def test_check_power_consumption_item_not_found() -> None:
    parsed = parse_redfish_multiple(_make_string_table(_make_power_entry()))
    results = list(check_redfish_power_consumption("99", parsed))
    assert results == []
