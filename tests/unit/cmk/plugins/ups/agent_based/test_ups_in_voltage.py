...
from cmk.plugins.ups.agent_based.ups_in_voltage import discover_ups_in_voltage
from cmk.plugins.ups.lib import check_ups_in_voltage, parse_ups_voltage
...
def test_check_ups_in_voltage_zero_is_crit() -> None:
    ...
    assert results[0] == Result(
        state=State.CRIT, summary="In voltage: 0V (warn/crit below 240V/200V)"
    )
    assert Metric("in_voltage", 0.0, boundaries=(150.0, None)) in results
