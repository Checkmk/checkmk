...
from cmk.plugins.vsphere.agent_based.esx_vsphere_hostsystem import (
    check_esx_vsphere_hostsystem_multipath,
)
...
def test_check_esx_vsphere_hostsystem_multipath(
    section: HostSystemSection, item: str, check_results: CheckResult
) -> None:
    assert (
        list(
            check_esx_vsphere_hostsystem_multipath(item, params={"levels_map": {}}, section=section)
        )
        == check_results
    )
