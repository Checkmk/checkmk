...
def test_collect_when_grafana_not_used() -> None:
    data = grafana_collector.collect(Path(var_dir))
...
def test_grafana_data_collector(
    ...
    grafana_collector.store_usage_data(
        headers=Headers(headers), var_dir=Path(var_dir), logger=mock.Mock()
    )
...
def test_grafana_data_collector_unexpected_error() -> None:
    ...
    grafana_collector.store_usage_data(
        headers=headers_mock, var_dir=Path(var_dir), logger=logger_mock
    )
...
def test_remove_grafana_usage_data() -> None:
    ...
    grafana_collector.remove_grafana_usage_data(Path(var_dir))
