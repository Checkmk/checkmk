...
def wato_path() -> Path:
    ...
    wato_fp = Path(check_mk_config_dir) / "wato"
...
def mock_checkmk_info(mocker: MockerFixture) -> None:
    mocker.patch(
        "cmk.utils.version.get_general_version_infos",
        return_value={"version": "2.2.0", "edition": "pro"},
    )


@pytest.mark.usefixtures("mock_instance_id", "mock_checkmk_info")
def test_collect_and_number_of_folders(local_connection: mock.Mock, wato_path: Path) -> None:
    ...
    data = site_info_collector.collect(Path(check_mk_config_dir), Path(var_dir))
...
@pytest.mark.usefixtures("mock_instance_id", "mock_checkmk_info")
def test_collect_too_many_columns(local_connection: mock.Mock) -> None:
    local_connection.query.return_value = [[0, 0, "unexpected"]]
    with pytest.raises(SiteInfoItemsInvalidError):
        site_info_collector.collect(Path(check_mk_config_dir), Path(var_dir))


@pytest.mark.usefixtures("mock_instance_id", "mock_checkmk_info")
def test_collect_too_few_columns(local_connection: mock.Mock) -> None:
...
@pytest.mark.usefixtures("mock_instance_id", "mock_checkmk_info")
def test_collect_no_site_info(local_connection: mock.Mock) -> None:
    local_connection.query.return_value = []
    with pytest.raises(SiteInfoInvalidError):
        site_info_collector.collect(Path(check_mk_config_dir), Path(var_dir))


def test_get_or_create_site_id() -> None:
    site_id_fp = site_info_collector.site_id_file_path(Path(var_dir))

    assert site_info_collector.get_site_id(site_id_fp) is None

    site_id = site_info_collector.get_or_create_site_id(site_id_fp)

    assert isinstance(site_id, UUID)

    # We get the ID again to verify it was stored correctly
    assert site_info_collector.get_site_id(site_id_fp) == site_id
