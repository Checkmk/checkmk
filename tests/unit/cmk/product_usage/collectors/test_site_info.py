...
def test_collect_and_number_of_folders(local_connection: MockType, wato_path: Path) -> None:
    ...
    data = site_info_collector.collect(Path(check_mk_config_dir), Path(var_dir), omd_root)
...
def test_collect_too_many_columns(local_connection: MockType) -> None:
    ...
    with pytest.raises(SiteInfoItemsInvalidError):
        site_info_collector.collect(Path(check_mk_config_dir), Path(var_dir), omd_root)
...
def test_get_or_create_product_usage_site_id() -> None:
    site_id_fp = site_info_collector.product_usage_site_id_file_path(Path(var_dir))

    assert site_info_collector.get_product_usage_site_id(site_id_fp) is None

    site_id = site_info_collector.get_or_create_product_usage_site_id(site_id_fp)

    assert isinstance(site_id, UUID)

    # We get the ID again to verify it was stored correctly
    assert site_info_collector.get_product_usage_site_id(site_id_fp) == site_id
