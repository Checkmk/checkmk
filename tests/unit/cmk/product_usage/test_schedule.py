...
def test_get_and_store_next_run_ts() -> None:
    ts_file_path = next_run_file_path(Path(var_dir))
...
def test_should_run_collection_on_schedule() -> None:
    now = datetime.now()
    ts_file_path = next_run_file_path(Path(var_dir))

    assert should_run_collection_on_schedule(Path(var_dir), now) is False

    store_next_run_ts(ts_file_path, 1)

    assert should_run_collection_on_schedule(Path(var_dir), now) is True
