...
@pytest.mark.parametrize("old_config,expected_config", [
    (("worst", ()), {
        "type": "worst",
        "count": 1,
        "restrict_state": 2,
...
def test_aggregation_function_conversion(old_config, expected_config):
    ...
    assert schema_converter.convert_aggr_func_old_to_new(
        None, old_config) == expected_config  # type: ignore[arg-type]  # irrelevant for test
