...
def test_try_update_license_usage() -> None:
    ...
    try_update_license_usage(
        ...
        lambda *args, **kwargs: LicenseUsageSample(
            ...
            num_services_synthetic=1,
            num_services_synthetic_excluded=2,
            extension_ntop=True,
        ),
    )
    assert (
...
def test_license_usage_report(
    ...
    for sample, expected_sample in zip(history, expected_history):
        ...
        assert sample.num_services_synthetic == expected_sample.num_services_synthetic
        assert (
            sample.num_services_synthetic_excluded
            == expected_sample.num_services_synthetic_excluded
        )
...
def test_history_try_add_sample_from_same_day() -> None:
    ...
    history.add_sample(
        LicenseUsageSample(
            ...
            num_services_synthetic=5,
            num_services_synthetic_excluded=6,
            extension_ntop=True,
        )
    )
