...
class TestVersionedConfigPath:
    ...
    def test_str(self, config_path) -> None:
        assert str(config_path) == str(config_path.serial)

    def test_repr(self, config_path) -> None:
        assert isinstance(repr(config_path), str)

    def test_eq(self, config_path) -> None:
    ...
    def test_hash(self, config_path) -> None:
    ...
    def test_fspath(self, config_path) -> None:
    ...
    def test_create_success(self, config_path, is_cmc) -> None:
    ...
    @pytest.mark.parametrize("is_cmc", (True, False))
    def test_create_no_latest_link_update_on_failure(self, config_path, is_cmc) -> None:
        assert not Path(config_path).exists()
        assert not Path(LATEST_CONFIG).exists()
