...
def test_main_help(site_context, capsys):
    omdlib.main.main_help(site_context)
    stdout = capsys.readouterr()[0]
    assert "omd COMMAND -h" in stdout


def test_main_version_of_current_site(site_context, capsys, monkeypatch):
    monkeypatch.setattr(omdlib, "__version__", "1.2.3p4")
    global_opts = omdlib.main.default_global_options()
    args = []
    options = []
    omdlib.main.main_version(site_context, global_opts, args, options)

    stdout = capsys.readouterr()[0]
    assert stdout == "OMD - Open Monitoring Distribution Version 1.2.3p4\n"


def test_main_version_root(capsys, monkeypatch):
    monkeypatch.setattr(omdlib, "__version__", "1.2.3p4")
    global_opts = omdlib.main.default_global_options()
    args = []
    options = []
    omdlib.main.main_version(omdlib.main.RootContext(), global_opts, args, options)

    stdout = capsys.readouterr()[0]
    assert stdout == "OMD - Open Monitoring Distribution Version 1.2.3p4\n"


def test_main_version_root_not_existing_site():
    with pytest.raises(SystemExit, match="No such site: testsite"):
        omdlib.main.main_version(omdlib.main.RootContext(), omdlib.main.default_global_options(),
                                 ["testsite"], [])


def test_main_version_root_specific_site_broken_version(fs):
...
def test_main_version_root_specific_site(fs, capsys, monkeypatch):
    fs.create_dir("/omd/sites/testsite")
...
def test_main_version_root_specific_site_bare(fs, capsys, monkeypatch):
    fs.create_dir("/omd/sites/testsite")
    fs.create_symlink("/omd/sites/testsite/version", "../../versions/1.2.3p4")
    fs.create_dir("/omd/versions/1.2.3p4")
    omdlib.main.main_version(omdlib.main.RootContext(), omdlib.main.default_global_options(),
...
def test_main_versions(fs, capsys, monkeypatch):
    fs.create_dir("/omd/versions/1.2.3p4")
    fs.create_dir("/omd/versions/1.6.0p4")
    fs.create_dir("/omd/versions/1.6.0p14")
    fs.create_symlink("/omd/versions/default", "1.6.0p4")
    omdlib.main.main_versions(omdlib.main.RootContext(), omdlib.main.default_global_options(), [],
                              [])
...
def test_main_versions_bare(fs, capsys, monkeypatch):
    fs.create_dir("/omd/versions/1.2.3p4")
    fs.create_dir("/omd/versions/1.6.0p4")
    fs.create_dir("/omd/versions/1.6.0p14")
    fs.create_symlink("/omd/versions/default", "1.6.0p4")
...
def test_main_sites(fs, capsys, monkeypatch):
    fs.create_dir("/omd/versions/1.2.3p4")
    fs.create_dir("/omd/versions/1.6.0p4")
    fs.create_dir("/omd/versions/1.6.0p14")
    fs.create_symlink("/omd/versions/default", "1.6.0p4")
    ...
    omdlib.main.main_sites(omdlib.main.RootContext(), omdlib.main.default_global_options(), [], [])
