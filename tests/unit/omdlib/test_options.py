...
def test_parse_command_options_help_argument_position(
    ...
    with pytest.raises(SystemExit) as pytest_wrapped_e:
        _parse_command_options(command.description, arguments, command.options)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            Root(),
            ["-f", "rm", "monitoring_test"],
            RmCommand(
                target_site="monitoring_test",
                global_opts=GlobalOptions(version=None, verbose=False, force=True),
                options={},
            ),
        ),
        (
...
def test_parse_args_or_exec_other_omd_no_version_link_rm_warns(
    ...
    result = parse_args_or_exec_other_omd(Root(), ["-f", "rm", "broken_site"], tmp_path)
