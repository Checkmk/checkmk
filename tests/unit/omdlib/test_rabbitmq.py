...
from omdlib.rabbitmq import _write_rabbitmq_management_port_conf, write_rabbitmq_default_conf
...
def test_write_rabbitmq_default_conf_single_addr(tmp_path: Path) -> None:
    ...
    write_rabbitmq_default_conf(
        "mysite", tmp_path, {"RABBITMQ_ONLY_FROM": "192.168.1.1", "RABBITMQ_PORT": "5671"}
    )
...
def test_write_rabbitmq_default_conf_multiple_addrs(tmp_path: Path) -> None:
    (tmp_path / "etc/rabbitmq/conf.d").mkdir(parents=True)
    write_rabbitmq_default_conf(
        "mysite", tmp_path, {"RABBITMQ_ONLY_FROM": "10.0.0.1 10.0.0.2", "RABBITMQ_PORT": "5671"}
    )
    content = (tmp_path / "etc/rabbitmq/conf.d/01-default.conf").read_text()
...
def test_write_rabbitmq_default_conf_empty_only_from(tmp_path: Path) -> None:
    (tmp_path / "etc/rabbitmq/conf.d").mkdir(parents=True)
    write_rabbitmq_default_conf(
        "mysite", tmp_path, {"RABBITMQ_ONLY_FROM": "", "RABBITMQ_PORT": "5671"}
    )
    content = (tmp_path / "etc/rabbitmq/conf.d/01-default.conf").read_text()
...
def test_write_rabbitmq_default_conf_overwrites(tmp_path: Path) -> None:
    ...
    write_rabbitmq_default_conf(
        "mysite", tmp_path, {"RABBITMQ_ONLY_FROM": "192.168.1.1", "RABBITMQ_PORT": "5672"}
    )
    content = (tmp_path / "etc/rabbitmq/conf.d/01-default.conf").read_text()
...
def test__write_rabbitmq_management_port_conf(tmp_path: Path) -> None:
    (tmp_path / "etc/rabbitmq/conf.d").mkdir(parents=True)
    _write_rabbitmq_management_port_conf("mysite", tmp_path, {"RABBITMQ_MANAGEMENT_PORT": "15671"})
...
def test__write_rabbitmq_management_port_conf_overwrites(tmp_path: Path) -> None:
    (tmp_path / "etc/rabbitmq/conf.d").mkdir(parents=True)
    _write_rabbitmq_management_port_conf("mysite", tmp_path, {"RABBITMQ_MANAGEMENT_PORT": "15671"})
    _write_rabbitmq_management_port_conf("mysite", tmp_path, {"RABBITMQ_MANAGEMENT_PORT": "15672"})
    content = (tmp_path / "etc/rabbitmq/conf.d/02-management-port.conf").read_text()
