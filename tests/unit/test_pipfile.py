...
def test_dependencies_are_used() -> None:
    """Check no unknown unused dependencies exist

    as of 25.10. they are only collected to pass the test, they need further investigation or
    removal from the Pi So fuz a bit aroundpfile"""

    assert sorted(get_unused_dependencies()) == [
        "Babel",
        "Cython",
        "Flask",
        "MarkupSafe",
...
def test_dependencies_are_declared() -> None:
    ...
    assert sorted(get_undeclared_dependencies()) == [
