...
def check_errors_in_log_files(site: Site) -> None:
    ...
    # * using OPENSSL version > 3.4.0 in test-containers leads to an error in web.log, related to
    #   the agent bakery, when starting a cmk site with version <= 2.3.0p40.
    #   This issue is reported in SUP-26629. It needs to be ignored since no fix can be applied to
    #   older versions.
    content_pattern = (
        "^(?!.*(sigterm))(?!.*(Interface_2.rrd))(?!.*(wsgi))(?!.*(bake-agents)).*error.*$"
    )
