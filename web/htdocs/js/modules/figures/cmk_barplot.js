...
class BarplotFigure extends cmk_figures.FigureBase {
    ...
    resize() {
        ...
        this.plot.select("g.x_axis").attr("transform", "translate(0," + this.plot_size.height + ")");
    }
    ...
    _update_plot_definitions(plot_definitions) {
        ...
        plot_definitions.forEach(plot_definition=>{
            if (plot_definition.plot_type != "single_value")
                return;
    ...
    update_gui(data) {
        ...
        this.plot.selectAll("g.x_axis").call(d3.axisBottom(this.scale_x))
            .selectAll("text")
        ...
        this.plot.selectAll("g.y_axis").call(d3.axisLeft(this.scale_y)
            .tickFormat(d=>{
                if (d > 999)
    ...
    _render_bar_containers() {
        this.plot.selectAll("g.bar")
            .data(this._plot_definitions, d=>d.id)
            .join(
                enter=>enter.append("g").classed("bar", true)
            )
            .attr("transform", d=>{
    ...
    _render_values() {
        this.plot.selectAll("g.bar").selectAll("rect.value").data(d=>{
    ...
    _render_levels() {
        let levels = this.plot.selectAll("g.bar").selectAll("rect.level").data(d=>{
            let levels = [];
            ...
            if (metrics == undefined)
                return levels;
            [{what: "warn", color: "#fffe44"}, {what: "crit", color: "#ff3232"}].forEach(level=>{
                if (metrics[level.what] == undefined)
                    return;
    ...
    _render_bar_tilers() {
        ...
        this.plot.selectAll("g.bar").selectAll("rect.tiler").data(tiles).join(
            enter=>enter.append("rect")
                .classed("tiler", true)
    ...
    _render_bar_gradients() {
        ...
        this.plot.selectAll("g.bar").selectAll("linearGradient").data((d, idx)=>[[d, idx]])
            .join("linearGradient")
            .attr("id", d=>"color-gradient-bar-" + d[1])
            .attr("gradientUnits", "userSpaceOnUse")
            .attr("x1", 0).attr("y1", 0)
            .attr("x2", this.scale_x.bandwidth()).attr("y2", 0)
            .selectAll("stop")
            .data(()=>{
                return [
