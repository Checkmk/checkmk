...
class TimeseriesFigure extends cmk_figures.FigureBase {
    ...
    _setup_legend() {
        this._legend = this._div_selection.select("div.figure_content")
    ...
    resize() {
        ...
        this.plot_size = {
            ...
            height: new_size.height - this.margin.top - this.margin.bottom - this._get_legend_height(),
        };
    ...
    _update_subplots(plot_definitions) {
        ...
        this._subplots.forEach(subplot=>{subplot.marked_for_removal = true;});
        ...
        this._subplots.forEach(subplot=>{
            if (subplot.marked_for_removal)
                this.remove_plot(subplot);
        });
    ...
    render_legend() {
        ...
        let items = this._legend.selectAll(".legend_item").data(this._subplots, d=>d.definition.id);
        items.exit().remove();
        ...
        new_items.merge(items).selectAll("div")
...
class AverageScatterplotFigure extends TimeseriesFigure {
    ...
    _mouse_move() {
        ...
        if (!["svg", "path"].includes(ev.target.tagName) ||
...
class SubPlot {
    ...
    get_color() {
        if (this.definition.color)
            return d3.color(this.definition.color);
        return;
    }

    get_opacity() {
        if (this.definition.opacity)
            return this.definition.opacity;
        return 1;
    }
    ...
    get_domains() {
        ...
        if (this.definition.is_scalar)
            return;

        return {
            x: [d3.min(this.transformed_data, d=>d.date), d3.max(this.transformed_data, d=>d.date)],
    ...
    get_legend_data(start, end) {
        ...
        if (this.definition.is_scalar)
            return {data: this.transformed_data};

        let data = this.transformed_data.filter(d=>{
            return d.timestamp >= start && d.timestamp <= end;
        });

        let value_accessor = this.definition.stack_on && this.definition.stack_values ? "unstacked_value" : "value";
        return {
    ...
    prepare_render() {
        ...
        this.main_g = this._renderer.g.selectAll("g.subplot_main_g." + this.definition.id).data([null])
            .join("g")
        ...
        this.svg = this.main_g.selectAll("svg.subplot").data([null])
            .join("svg")
    ...
    migrate_to(other_renderer) {
        ...
        if (delta) {
            other_renderer.g.select(".subplot_main_g." + this.definition.id)
...
class LinePlot extends SubPlot {
    ...
    render() {
        ...
        let path = this.svg.selectAll("g.graph_data path").data([this.transformed_data])
            .join(enter=>enter
                .append("g")
...
class AreaPlot extends SubPlot {
    ...
    _compute_quantiles() {
        ...
        this._quantiles = {
            top_20_perc: d3.quantile(all_values, 0.80)
        };
    }
    ...
    prepare_render() {
        ...
        let y2 = 0;
        if (this._quantiles)
            y2 = this._renderer.scale_y(this._quantiles.top_20_perc);

        this.svg.selectAll("linearGradient").data([null])
        ...
        this.svg.selectAll("linearGradient")
            .attr("x1", 0).attr("y1", plot_size.height)
    ...
    render() {
        ...
        let color = this.get_color();
        let gradient_color = "url(#area-gradient-" + this.definition.id + this._renderer.get_id() + ")";
        let opacity = this.get_opacity();
        let stroke_width = this.get_stroke_width();
        if (this.definition.stack_on)
...
class ScatterPlot extends SubPlot {
    ...
    prepare_render() {
        SubPlot.prototype.prepare_render.call(this);
        let plot_size = this._renderer.plot_size;
        let fo = this.main_g.selectAll("foreignObject.canvas_object")
            .data([plot_size])
            .join("foreignObject")
        ...
        let body = fo.selectAll("xhtml").data([null])
            .join("xhtml")
            .style("margin", "0px");

        this.canvas = body.selectAll("canvas").data([plot_size])
            .join("canvas")
            .classed("subplot", true)
            .attr("x", 0)
            .attr("y", 0)
    ...
    redraw_canvas() {
        ...
        let g = color.g;
        this.transformed_data.forEach(point=>{
            if (point.scaled_x > plot_size.width || point.scaled_x < 0)
                return;
    ...
    get_color() {
        let color = SubPlot.prototype.get_color.call(this);
        return color != undefined ? color : d3.color(this._get_css("fill", "circle", ["scatterdot"]));
    }
}
...
class BarPlot extends SubPlot {
    ...
    render() {
        ...
        this._bars = bars.enter().append("a")
            ...
            .attr("width", d=>this._renderer.scale_x(new Date(d.ending_timestamp * 1000)) - this._renderer.scale_x(d.date))
            .each((d, idx, nodes)=>{
                // Update classes
...
class CmkGraphTimeseriesFigure extends TimeseriesFigure {
    ...
    _get_legend_height() {
        if (!this._legend || this._small_legend)
            return 0;
    ...
    render_legend() {
        ...
        this._div_selection.selectAll("div.toggle").data([null]).enter()
            .append("div")
    ...
    _render_legend(subplot_data, small) {
        ...
        let table = this._legend.selectAll("tbody").data([null])
            .join(enter=>enter
                .append("table")
                .classed("legend", true)
                .style("width", "100%")
        ...
        table.selectAll("tr.headers").data([["", "MINIMUM", "MAXIMUM", "AVERAGE", "LAST"]])
            .join("tr").classed("headers", true)
            .selectAll("th").data(d=>d)
            .join("th").text(d=>d);

        // Metrics
        let rows = table.selectAll("tr.metric").data(subplot_data.filter(d=>d.definition.id.startsWith("metric_"))).join("tr").classed("metric", true);
        rows.selectAll("td.name").data(d=>[d]).enter().append("td").classed("name small", true).each((d, idx, nodes)=>{
        ...
        rows.selectAll("td.min").data(d=>[d]).join("td").classed("scalar min", true).text(d=>d.data.data.length == 0 ? "NaN" : d.data.y[0].toFixed(2));
        rows.selectAll("td.max").data(d=>[d]).join("td").classed("scalar max", true).text(d=>d.data.data.length == 0 ? "NaN" : d.data.y[1].toFixed(2));
        rows.selectAll("td.average").data(d=>[d]).join("td").classed("scalar average", true).text(d=>d.data.data.length == 0 ? "NaN" : d3.mean(d.data.data, d=>d.value).toFixed(2));
        rows.selectAll("td.last").data(d=>[d]).join("td").classed("scalar last", true).text(d=>{
            if (d.data.data.length == 0)
                return "NaN";

            if (d.data.data[0].value == null)
                return "NaN";

            if (d.data.data[0].unstacked_value)
                return d.data.data[0].unstacked_value.toFixed(2);
            else
                return d.data.data[0].value.toFixed(2);
        });

        // Levels
        rows = table.selectAll("tr.level").data(subplot_data.filter(d=>d.definition.id.startsWith("level_")))
            .join(enter=>enter
                .append("tr")
        ...
        rows.selectAll("td.name").data(d=>[d]).enter().append("td").classed("name", true).each((d, idx, nodes)=>{
            let td = d3.select(nodes[idx]);
            td.classed("name", true);
            td.append("div")
                .classed("color", true)
        ...
        rows.selectAll("td.min").data(d=>[d]).join("td").classed("scalar min", true).text("");
        rows.selectAll("td.max").data(d=>[d]).join("td").classed("scalar max", true).text("");
        rows.selectAll("td.average").data(d=>[d]).join("td").classed("scalar average", true).text("");
        rows.selectAll("td.last").data(d=>[d]).join("td").classed("scalar last", true).text(d=>d.data.data.length == 0 ? "NaN" : d.data.data[0].value.toFixed(2));
...
class CmkGraphShifter extends CmkGraphTimeseriesFigure {
    ...
    _apply_shift_config(data) {
        ...
        this._shifts.forEach(config=>{
            if (config.seconds === 0)
                return;
            let shift = JSON.parse(JSON.stringify(this._subplots_by_id[config.shifted_id].definition));
    ...
    _setup_cutter_options_panel() {
        this._cutter_div = this._div_selection.select("div.figure_content")
            .selectAll("div.cutter_options").data([null])
        ...
        this._cutter_div.append("label")
            ...
            .text("Shift data").on("click", (d, idx, nodes)=>{
                let node = d3.select(nodes[idx]);
                let active = !node.classed("active");
                node.classed("active", active);
                this._cutter_div.select("div.options").transition().style("width", active ? null : "0px");
                node.style("background", active ? "green" : null);
            });
        let options = [{"id": "Hours", "min": 0, "max": 24}, {"id": "Days", "min": 0, "max": 31}];
        let div_options = this._cutter_div.append("div")
            .style("overflow", "hidden")
            .style("width", "0px")
            .classed("options", true);
        let new_table = div_options.selectAll("table").data([null]).enter()
            .append("table");

        let new_rows = new_table.selectAll("tr.shift_option").data(options).enter().append("tr").classed("shift_option", true);
        new_rows.append("td").text(d=>d.id);
        new_rows.append("td").text("0").classed("value", true).attr("id", d=>d.id);
        new_rows.append("td").append("input")
            .attr("type", "range")
            .attr("min", d=>d.min)
            .attr("max", d=>d.max)
            .attr("value", 0)
    ...
    _update_cutter_options_panel() {
        let table = this._cutter_div.select("div.options").selectAll("table.metrics").data([null]).join("table").classed("metrics", true);
        let rows = table.selectAll("tr.metric").data(
            this._subplots.filter(d=>!(d.definition.is_scalar || d.definition.is_shift || d.definition.hidden)), d=>d.definition.id)
            .join("tr").classed("metric", true);
        rows.selectAll("input").data(d=>[d]).join("input").attr("type", "checkbox").style("display", "inline-block").on("change", ()=>this._update_shifts());
        let metric_td = rows.selectAll("td.color").data(d=>[d]).enter().append("td").classed("color", true);
        metric_td.append("div").classed("color", true).style("background", d=>d.definition.color);
    ...
    _update_shifts() {
        ...
        checked_metrics.each(d=>{
            this._shifts.push({shifted_id:d.definition.id,
