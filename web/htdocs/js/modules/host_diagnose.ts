...
export function start_test(ident, hostname, transid) {
    ...
    vars +=
        "&ipaddress=" +
        encodeURIComponent(getFirstElementByNameAsInput("vs_host_p_ipaddress").value);
    ...
    if (getFirstElementByNameAsInput("vs_host_p_snmp_v3_credentials_USE").checked) {
        ...
        if (v3_use == "0") {
            vars +=
                ...
                encodeURIComponent(
                    getFirstElementByNameAsInput("vs_host_p_snmp_v3_credentials_0_1").value
                );
        } else if (v3_use == "1") {
            vars +=
                "&snmpv3_auth_proto=" +
                encodeURIComponent(
                    getFirstElementByNameAsInput("vs_host_p_snmp_v3_credentials_1_1").value
                );
            vars +=
                "&snmpv3_security_name=" +
                encodeURIComponent(
                    getFirstElementByNameAsInput("vs_host_p_snmp_v3_credentials_1_2").value
                );
            vars +=
                "&snmpv3_security_password=" +
                encodeURIComponent(
                    getFirstElementByNameAsInput("vs_host_p_snmp_v3_credentials_1_3").value
                );
        } else if (v3_use == "2") {
            vars +=
                "&snmpv3_auth_proto=" +
                encodeURIComponent(
                    getFirstElementByNameAsInput("vs_host_p_snmp_v3_credentials_2_1").value
                );
            vars +=
                "&snmpv3_security_name=" +
                encodeURIComponent(
                    getFirstElementByNameAsInput("vs_host_p_snmp_v3_credentials_2_2").value
                );
            vars +=
                "&snmpv3_security_password=" +
                encodeURIComponent(
                    getFirstElementByNameAsInput("vs_host_p_snmp_v3_credentials_2_3").value
                );
            vars +=
                "&snmpv3_privacy_proto=" +
                encodeURIComponent(
                    getFirstElementByNameAsInput("vs_host_p_snmp_v3_credentials_2_4").value
                );
            vars +=
                "&snmpv3_privacy_password=" +
                encodeURIComponent(
                    getFirstElementByNameAsInput("vs_host_p_snmp_v3_credentials_2_5").value
                );
        }
    }
    ...
    vars +=
        "&agent_port=" +
        encodeURIComponent(getFirstElementByNameAsInput("vs_rules_p_agent_port").value);
    vars +=
        "&tcp_connect_timeout=" +
...
function handle_host_diag_result(data, response_json) {
    ...
    var retry = document.getElementById(data.ident + "_retry") as HTMLImageElement;
