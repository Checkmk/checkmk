...
export class DatasourceManager {
    ...
    _register_datasource(datasource_class) {
        if (datasource_class.id() in this.datasources)
            return
...
export class AggregationsDatasource extends AbstractDatasource {
    ...
    fetch_aggregations(list_of_aggregations, use_layout_id) {
        let url = "ajax_fetch_aggregation_data.py?aggregations=" + JSON.stringify(list_of_aggregations)
        if (use_layout_id)
            url += "&layout_id=" + use_layout_id
        this.fetch(url)
    }
}
