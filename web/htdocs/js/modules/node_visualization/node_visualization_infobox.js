...
export class InfoBox {
    ...
    feed_data(data) {
        ...
        if (this._selection.style("display") == "none") {
            ...
            node_visualization_utils.DefaultTransition.add_transition(this._selection.style("opacity", "0"))
                                        .style("opacity", "1")
        }
        ...
        if (formatter) {
            ...
            node_visualization_utils.DefaultTransition.add_transition(this._content_selection.style("height", old_height))
    ...
    _find_formatter(data) {
        for (let idx in this._formatters) {
            ...
            if (formatter.supports_data(data))
                return formatter
        }
    }
}
...
class AbstractInfoboxFormatter {
    ...
    supports_data(data) {return false}
...
class InfoboxNodeFormatter extends AbstractInfoboxFormatter {
    ...
    render_into_selection(selection, data) {
        ...
        table.selectAll("tr").data(entries).enter()
            .append("tr")
            .append("td")
        ...
        this.main_instance.viewport.current_viewport.get_all_nodes().forEach(node=>{
            if (node.data.name == node_id) {
                node_visualization_utils.DefaultTransition.add_transition(
                    ...
                    .call(this.main_instance.viewport.current_viewport.main_zoom.transform, ()=>this._transform(node))
                return
            }
        })
    }
    ...
    _transform(node) {
      ...
      .translate(this.main_instance.viewport.current_viewport.width / 2, this.main_instance.viewport.current_viewport.height / 2)
